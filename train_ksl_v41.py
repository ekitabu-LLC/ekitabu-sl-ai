#!/usr/bin/env python3
"""
KSL Training v41 - Recover-and-Resample Temporal Augmentation

Builds on v37 (GroupNorm + SupCon + Projection Head) and adds:
  5. Recover-and-Resample (R&R): temporal augmentation inspired by
     "Recover-and-Resample" (NeurIPS 2024, Liu et al.)

The R&R augmentation addresses cross-signer temporal variation:
  - Different signers perform signs at different speeds and with different
    temporal offsets (some start earlier, some hold the end pose longer)
  - RECOVER: Linearly extrapolates beyond the observed start/end frames
    by 5-15%, simulating a "wider temporal window" around the sign
  - RESAMPLE: Randomly crops a sub-window (70-100% length) from the
    recovered sequence and resamples back to original T via interpolation
  - SEGMENT WARP: Divides the sequence into 3-5 segments and applies
    random per-segment speed factors (0.7-1.3x), simulating within-sign
    temporal variation between signers

This replaces v37's speed_aug (simple stretch to [130,220] frames) with
a more principled temporal domain augmentation. All other settings
(GroupNorm, SupCon, projection head, spatial augmentations) are identical.

Changes from v37:
  1. REPLACED: speed_aug → Recover-and-Resample (recover + crop-resample + segment-warp)
  2. NEW hyperparams: rr_prob, rr_extend_min/max, rr_crop_min, rr_num_segments, rr_speed_range
  3. Model class renamed KSLGraphNetV41 (architecture identical to V37)

Usage:
    python train_ksl_v41.py --model-type numbers
    python train_ksl_v41.py --model-type words
"""
...
# ---------------------------------------------------------------------------
# Graph Topology (identical to v37)
# ---------------------------------------------------------------------------
...
V41_CONFIG = {
    ...
    "grl_start_epoch": 0,
    "grl_ramp_epochs": 100,
    # Recover-and-Resample temporal augmentation (NEW in v41)
    # Replaces v37's speed_aug with a more principled temporal domain augmentation.
    "rr_prob": 0.7,             # probability of applying R&R
    "rr_extend_min": 0.05,     # min boundary extension ratio (5%)
    "rr_extend_max": 0.15,     # max boundary extension ratio (15%)
    "rr_crop_min": 0.7,        # min crop ratio (70% of extended sequence)
    "rr_num_segments": 4,      # number of segments for within-sign speed variation
    "rr_speed_min": 0.7,       # min per-segment speed factor
    "rr_speed_max": 1.3,       # max per-segment speed factor
    # Spatial augmentation (identical to v37)
    "rotation_prob": 0.5,
...
# ---------------------------------------------------------------------------
# Adjacency Matrix (identical to v37)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Data Deduplication (identical to v37)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Normalization (identical to v37)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Augmentation Suite (spatial augmentations identical to v37)
# ---------------------------------------------------------------------------
...
def augment_recover_resample(data_list, config):
    ...
    Inspired by "Recover-and-Resample" (NeurIPS 2024, Liu et al.) adapted
    for sign language skeleton sequences.

    Three stages applied in sequence:
      1. RECOVER: Linearly extrapolate start/end frames to simulate a wider
         temporal window (as if recording started earlier / ended later).
      ...
      3. SEGMENT WARP: Divide into segments and apply per-segment speed
         variation to simulate within-sign temporal differences.

    Args:
        data_list: list of numpy arrays, each shape (T, ...), same T
        config: dict with rr_* hyperparameters

    Returns:
        list of augmented arrays, same shapes as input
    """
    T = data_list[0].shape[0]
    if T < 4:
        return data_list
    ...
    # Estimate velocity at boundaries from first/last few frames
    n_ref = min(5, T // 4)  # number of reference frames for velocity estimation
    ...
    for arr in data_list:
        ...
        # Extrapolate backward (before start)
        # Velocity at start: average of first n_ref frame differences
        if n_ref >= 2:
    ...
    # Randomly crop a sub-window and resample back to original T
    ...
    # Resample back to original T using linear interpolation
    ...
    # Build cumulative timestamp mapping
    seg_len = T / num_seg
    timestamps = []
    for i in range(T):
        seg_idx = min(int(i / seg_len), num_seg - 1)
        local_t = (i - seg_idx * seg_len) / seg_len  # 0 to 1 within segment
    ...
    # Resample using these warped timestamps
...
# ---------------------------------------------------------------------------
# Feature Computation (identical to v37)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Signer-Balanced Batch Sampler (identical to v37)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Temporal CutMix (identical to v37)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Dataset (v41: replaces speed_aug with R&R)
# ---------------------------------------------------------------------------
...
class KSLMultiStreamDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Temporal warp (kept from v37 — complements R&R)
        ...
        # NEW v41: Recover-and-Resample (replaces v37's speed_aug)
        if self.aug and np.random.random() < self.config["rr_prob"]:
            h, velocity, bones, joint_angles, fingertip_dists, hand_body_feats = \
...
# ---------------------------------------------------------------------------
# Model Components (identical to v37 — GroupNorm throughout)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Main Model (v41: identical architecture to v37)
# ---------------------------------------------------------------------------

class KSLGraphNetV41(nn.Module):
    """ST-GCN v41: GroupNorm + SupCon projection head + R&R augmentation."""
...
# ---------------------------------------------------------------------------
# Loss Functions (identical to v37)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Train One Stream (identical to v37 except model class name)
# ---------------------------------------------------------------------------
...
def train_stream(stream_name, name, classes, config, train_dirs, val_dir,
    ...
    print(f"[{ts()}] Normalization: GroupNorm (v41)")
    print(f"[{ts()}] SupCon: weight={config['supcon_weight']}, "
          f"temp={config['supcon_temperature']}, proj_dim={proj_dim}")
    print(f"[{ts()}] R&R: prob={config['rr_prob']}, extend=[{config['rr_extend_min']}, "
          f"{config['rr_extend_max']}], crop_min={config['rr_crop_min']}, "
          f"segments={config['rr_num_segments']}, speed=[{config['rr_speed_min']}, {config['rr_speed_max']}]")
...
# ---------------------------------------------------------------------------
# Fusion Weight Learning (identical to v37)
# ---------------------------------------------------------------------------
...
def main():
    parser = argparse.ArgumentParser(
        description="KSL ST-GCN v41 - GroupNorm + SupCon + Recover-and-Resample",
    ...
    parser.add_argument("--checkpoint-dir", type=str,
                        default=os.path.join(base_data, "checkpoints", "v41"))
    ...
    print("=" * 70)
    print("KSL Training v41 - GroupNorm + SupCon + Recover-and-Resample")
    print(f"Started: {ts()}")
    ...
    print(f"\nv41 Changes from v37:")
    print(f"  1. REPLACED: speed_aug -> Recover-and-Resample (R&R)")
    print(f"     - RECOVER: linear extrapolation at boundaries ({V41_CONFIG['rr_extend_min']}-{V41_CONFIG['rr_extend_max']} ratio)")
    print(f"     - CROP-RESAMPLE: random crop ({V41_CONFIG['rr_crop_min']}-1.0) + resize to original T")
    print(f"     - SEGMENT WARP: {V41_CONFIG['rr_num_segments']} segments, speed [{V41_CONFIG['rr_speed_min']}-{V41_CONFIG['rr_speed_max']}]")
    print(f"  2. All other settings identical (GroupNorm, SupCon, proj head, spatial aug)")
    print(f"\n[{ts()}] Config:")
    print(json.dumps(V41_CONFIG, indent=2, default=str))
    ...
    for cat_name, classes, use_focal in categories:
        ...
        stream_results = {}
        for stream_name in V41_CONFIG["streams"]:
            result = train_stream(stream_name, cat_name, classes, V41_CONFIG,
        ...
        fusion_weights, fused_acc = learn_fusion_weights(
            cat_name, classes, V41_CONFIG, val_dir, ckpt_dir, device
        )
        results[cat_key] = {
    ...
    results_path = os.path.join(
        args.results_dir,
        f"v41_{args.model_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    )
    with open(results_path, "w") as f:
        json.dump({
            ...
            "timestamp": ts(),
            "changes_from_v37": [
                "1. REPLACED: speed_aug -> Recover-and-Resample (R&R) temporal augmentation",
                "2. R&R Stage 1 (Recover): linear extrapolation at start/end boundaries (5-15%)",
                "3. R&R Stage 2 (Crop-Resample): random crop (70-100%) + resize to T",
                "4. R&R Stage 3 (Segment Warp): per-segment speed variation (0.7-1.3x, 4 segments)",
            ],
