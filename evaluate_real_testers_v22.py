#!/usr/bin/env python3
"""
Evaluate v22 model on real-world test signers.

V22 changes from v21:
- No ArcFace head (single-stage CE + SupCon)
- Bigger auxiliary branch (64-dim output with temporal conv1d)
- Larger signer head (64 hidden units)
- Model forward: (gcn_input, aux_input, grl_lambda=0.0) -> (logits, signer_logits, embedding)

Usage:
    python evaluate_real_testers_v22.py
"""
...
# ---------------------------------------------------------------------------
# Constants (matching train_ksl_v22.py)
# ---------------------------------------------------------------------------
...
# Joint angle topology (same as train_ksl_v22.py)
...
# ---------------------------------------------------------------------------
# Preprocessing (matching v22 dataset pipeline, no augmentation)
# ---------------------------------------------------------------------------
...
def normalize_wrist_palm(h):
    """Wrist-centric + palm-size normalization (matching v22 training)."""
    lh = h[:, :21, :]
...
def preprocess_landmarks(raw, max_frames=MAX_FRAMES):
    """Preprocess raw (num_frames, 225) landmarks to v22 format.
...
# ---------------------------------------------------------------------------
# Test-Time Augmentation (TTA) - adapted for v22
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Inference (v22: no use_arcface, no labels in forward)
# ---------------------------------------------------------------------------
...
def run_inference(model, raw, device, classes, use_tta=True):
    ...
    with torch.no_grad():
        # v22: forward(gcn_input, aux_input, grl_lambda=0.0)
        logits, _, _ = model(
...
def main():
    ...
    print("=" * 70)
    print("V22 Real-World Evaluation (with TTA + Confidence Analysis)")
    ...
    # Import v22 model class
    from train_ksl_v22 import KSLGraphNetV22, build_adj

    results = {}

    # ===== NUMBERS =====
    numbers_ckpt = os.path.join(ckpt_dir, "v22_numbers", "best_model.pt")
    ...
    # ===== WORDS =====
    words_ckpt = os.path.join(ckpt_dir, "v22_words", "best_model.pt")
    ...
    # ===== FINAL SUMMARY =====
    print(f"\n{'='*70}")
    print("FINAL SUMMARY - V22 Real-World Evaluation")
    print(f"{'='*70}")
    ...
    results_path = os.path.join(results_dir, f"v22_real_testers_{ts_str}.json")
    ...
    results_out = {
        ...
        "features": "GCN: xyz(3)+velocity(3)+bone(3)=9ch | AUX: angles+fingertip_dists (64-dim with temporal conv1d)",
        "model": "KSLGraphNetV22 (ST-GCN + bigger aux MLP + signer-adversarial, no ArcFace)",
