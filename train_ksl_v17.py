#!/usr/bin/env python3
"""
KSL Training v17 (Alpine) - Simplified ST-GCN + SWA

Changes from v16:
- Numbers capacity matched to words (hidden_dim=128, 6 layers, same augmentation)
- Simple CE loss only (removed focal, contrastive, anti-attractor, mixup)
- SWA (Stochastic Weight Averaging) in last 20% of training epochs
- Words config unchanged from v16 (91.3% baseline)

Usage:
    python train_ksl_v17.py --model-type numbers
    python train_ksl_v17.py --model-type words
    python train_ksl_v17.py --model-type both
"""
...
NUMBERS_CONFIG = {
    ...
    "num_nodes": 48,
    "in_channels": 3,
    "hidden_dim": 128,       # was 96 in v16, now matched to words
    "num_layers": 6,          # was 5 in v16, now matched to words
    "temporal_kernel": 9,
    "batch_size": 32,
    "epochs": 300,            # was 250 in v16, more room for convergence
    "learning_rate": 5e-4,    # was 3e-4 in v16, now matched to words
    "min_lr": 1e-6,
    "weight_decay": 0.003,    # was 0.01 in v16, reduced
    "dropout": 0.4,           # was 0.35 in v16, now matched to words
    "label_smoothing": 0.15,  # was 0.1 in v16, now matched to words
    "patience": 60,           # was 50 in v16, more patience
    "warmup_epochs": 10,
    # Augmentation (strong, matching words)
    "hand_dropout_prob": 0.5,       # was 0.15
    "hand_dropout_min": 0.3,        # was 0.05
    "hand_dropout_max": 0.7,        # was 0.2
    "complete_hand_drop_prob": 0.1,  # was 0.0
    "noise_std": 0.015,             # was 0.006
    "noise_prob": 0.4,              # was 0.3
    # Hard class boosting
...
# Words config: identical to v16 (91.3% baseline, don't touch)
WORDS_CONFIG = {
    "max_frames": 90,
    "num_nodes": 48,
    "in_channels": 3,
    ...
    "noise_std": 0.015,
    "noise_prob": 0.4,
    # No hard class boosting for words
...
def train_split(name, classes, config, train_dir, val_dir, ckpt_dir, device,
                writer=None):
    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] Training {name} ({len(classes)} classes) - v17")
    print(f"[{ts()}] {'=' * 70}")
    ...
    if config.get("use_hard_class_boosting", False):
        ...
        for lbl in train_ds.labels:
            ...
            sw.append(base_w)
        print(f"[{ts()}] Hard-class boosting enabled")
    else:
    ...
    print(f"[{ts()}] Config: hidden_dim={config['hidden_dim']}, layers={config['num_layers']}, "
          f"dropout={config['dropout']}, wd={config['weight_decay']}, ls={config['label_smoothing']}")
    ...
    swa_active = False
    print(f"[{ts()}] SWA starts at epoch {swa_start}")
    ...
    for ep in range(config["epochs"]):
        ...
        for d, t in train_ld:
            ...
            logits = model(d)
            loss = F.cross_entropy(logits, t, weight=w, label_smoothing=config["label_smoothing"])
        ...
        # Validation (use SWA model if active)
        ...
        if va > best:
            ...
            save_state = swa_model.module.state_dict() if swa_active else model.state_dict()
            torch.save(
                {
                    "model": save_state,
                    "val_acc": va,
    ...
    # Update BN stats for SWA model before final eval
    if swa_active:
        print(f"[{ts()}] Updating SWA batch norm statistics...")
        ...
        with torch.no_grad():
            for d, t in val_ld:
                ...
                vt_swa += t.size(0)
                vc_swa += p.eq(t).sum().item()
        swa_final_acc = 100.0 * vc_swa / vt_swa
        print(f"[{ts()}] SWA final accuracy (BN updated): {swa_final_acc:.1f}%")
...
def main():
    parser = argparse.ArgumentParser(
        description="KSL ST-GCN Training v17 (Alpine HPC)",
    ...
    print("=" * 70)
    print(f"KSL Training v17 - Simplified ST-GCN + SWA (Alpine HPC)")
    print(f"Started: {ts()}")
    print("=" * 70)
    ...
    if args.tensorboard:
        try:
            ...
            tb_dir = os.path.join(args.tb_logdir, f"v17_{args.model_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
    ...
    if args.model_type in ("numbers", "both"):
        ...
        ckpt_dir = os.path.join(args.checkpoint_dir, "v17_numbers")
    ...
    if args.model_type in ("words", "both"):
        ...
        ckpt_dir = os.path.join(args.checkpoint_dir, "v17_words")
    ...
    # Summary
    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] SUMMARY v17")
    print(f"[{ts()}] {'=' * 70}")
    if results.get("numbers"):
        print(f"[{ts()}] Numbers: {results['numbers']['overall']:.1f}% "
              f"(best epoch {results['numbers']['best_epoch']}, params {results['numbers']['params']:,})")
    if results.get("words"):
        print(f"[{ts()}] Words:   {results['words']['overall']:.1f}% "
              f"(best epoch {results['words']['best_epoch']}, params {results['words']['params']:,})")
    ...
    results_path = os.path.join(
        args.results_dir,
        f"v17_{args.model_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    )
    results_out = {
        "version": "v17",
