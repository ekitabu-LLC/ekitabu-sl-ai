#!/usr/bin/env python3
"""
KSL Training v31 Exp5 - Cross-Signer Contrastive Learning

Based on v28 multi-stream architecture. Adds cross-signer supervised contrastive loss
to explicitly pull together same-class embeddings from different signers.

Key changes from v28:
  1. **Batch size = 64** (increased from 32) to ensure diverse signers/classes per batch
  2. **SupCon weight = 0.05** (reduced from 0.1) for controlled regularization
  3. Total loss = CE_loss + 0.05 * SupCon_loss
  4. SupCon groups same-class samples as positives (regardless of signer)

This is a minimal ablation test: only changing SupCon weight and batch size.
All other training settings identical to v28.

Usage:
    python train_ksl_v31_exp5.py --model-type numbers
    python train_ksl_v31_exp5.py --model-type words
"""
...
# ---------------------------------------------------------------------------
# Config (v31_exp5 - cross-signer contrastive)
# ---------------------------------------------------------------------------
...
V31_EXP5_CONFIG = {
    ...
    "batch_size": 64,  # INCREASED from 32 for cross-signer diversity
    "epochs": 500,
    "learning_rate": 1e-3,
    "min_lr": 1e-6,
    "weight_decay": 1e-3,
    ...
    "patience": 80,
    "warmup_epochs": 10,
    # SupCon - REDUCED weight for exp5
    "supcon_weight": 0.05,  # CHANGED from 0.1
...
# ---------------------------------------------------------------------------
# Model (KSLGraphNetV25 architecture)
# ---------------------------------------------------------------------------
...
def main():
    ...
    parser.add_argument("--checkpoint-dir", type=str, default=os.path.join(base_data, "checkpoints", "v31_exp5"))
    ...
    print("=" * 70)
    print(f"KSL Training v31 Exp5 - Cross-Signer Contrastive Learning")
    print(f"Started: {ts()}")
    ...
    print(f"  Streams: {V31_EXP5_CONFIG['streams']}")
    print(f"  In channels per stream: {V31_EXP5_CONFIG['in_channels']}")
    print(f"  Batch size: {V31_EXP5_CONFIG['batch_size']} (INCREASED from 32)")
    print(f"  SupCon weight: {V31_EXP5_CONFIG['supcon_weight']} (REDUCED from 0.1)")

    print(f"\n[{ts()}] v31 Exp5 Config:")
    print(json.dumps(V31_EXP5_CONFIG, indent=2, default=str))
    ...
    for cat_name, classes, use_focal in categories:
        ...
        # Train each stream
        stream_results = {}
        for stream_name in V31_EXP5_CONFIG["streams"]:
            result = train_stream(
                stream_name, cat_name, classes, V31_EXP5_CONFIG,
        ...
        fusion_weights, fused_acc = learn_fusion_weights(
            cat_name, classes, V31_EXP5_CONFIG, val_dir, ckpt_dir, device
        )
    ...
    # Summary
    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] SUMMARY v31 Exp5 - Cross-Signer Contrastive")
    print(f"[{ts()}] {'=' * 70}")
    ...
    results_path = os.path.join(
        args.results_dir,
        f"v31_exp5_{args.model_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    )
    results_out = {
        "version": "v31_exp5",
        "model_type": args.model_type,
        "seed": args.seed,
        "config": V31_EXP5_CONFIG,
        "results": results,
        ...
        "timestamp": ts(),
        "changes_from_v28": [
            "1. BATCH SIZE = 64 (increased from 32) for cross-signer diversity",
            "2. SUPCON WEIGHT = 0.05 (reduced from 0.1) for controlled regularization",
            "3. Total loss = CE_loss + 0.05 * SupCon_loss",
            "4. SupCon groups same-class samples as positives (cross-signer)",
        ],
    }
