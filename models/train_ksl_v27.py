#!/usr/bin/env python3
"""
KSL Training v27 (Alpine) - MediaPipe fix + 12 signers + Focal Loss for words

Based on v26 (v25 architecture on ksl-alpha). Three changes:

  1. MEDIAPIPE FIX: Training data re-extracted with MediaPipe 0.10.5 on Alpine,
     matching the evaluation environment. No more version mismatch.
  2. 12 TRAINING SIGNERS: Combines train_alpha (signers 1-10) + val_alpha (11-12)
     for training (~895 samples). Validation uses test_alpha (signers 13-15, 225 samples).
  3. FOCAL LOSS FOR WORDS: Words model uses focal loss (gamma=2, class-balanced alpha)
     to address Market sink class absorption. Numbers model keeps CE with label_smoothing.

Architecture, augmentation, and all other hyperparameters are IDENTICAL to v25/v26.

Usage:
    python train_ksl_v27.py --model-type numbers
    python train_ksl_v27.py --model-type words
    python train_ksl_v27.py --model-type both --seed 42
    python train_ksl_v27.py --model-type numbers --loso
"""
...
def check_mediapipe_version():
    ...
    V27: Training data was re-extracted with MediaPipe 0.10.5 on Alpine,
    matching the evaluation environment. No more version mismatch.
    """
    try:
...
# ---------------------------------------------------------------------------
# Config (v27 - identical to v25/v26, 12 signers + focal loss for words)
# ---------------------------------------------------------------------------
...
def extract_signer_id(filename):
    ...
    Also handles Signer_XX pattern as fallback (e.g., Signer_01_rep1.npy).
    ksl-alpha uses the same CLASS-SIGNER-REP.npy format as v2 data.
    """
    import re
    base = os.path.splitext(filename)[0]
    # Primary: CLASS-SIGNER-REP format (e.g., Apple-5-1.npy)
    parts = base.split("-")
    if len(parts) >= 2:
        return parts[1]
    # Fallback: Signer_XX pattern (e.g., Signer_01_rep1.npy)
...
# ---------------------------------------------------------------------------
# Dataset (v27 - v25/v26 pipeline + multi-directory support)
# ---------------------------------------------------------------------------
...
class KSLGraphDataset(Dataset):
    def __init__(self, data_dirs, classes, config, aug=False):
        """
        Args:
            data_dirs: str or list of str. One or more data directories to load from.
                       v27: supports multiple dirs to combine train_alpha + val_alpha.
        ...
        # v27: Accept single dir (backward compatible) or list of dirs
        if isinstance(data_dirs, str):
            data_dirs = [data_dirs]

        # Collect all sample paths from all directories
        all_paths = []
        all_labels = []
...
# ---------------------------------------------------------------------------
# Focal Loss (v27 NEW - for words model to address Market sink class)
# ---------------------------------------------------------------------------

class FocalLoss(nn.Module):
    """Focal Loss with optional per-class alpha weighting and label smoothing.

    Applies (1 - p_t)^gamma weighting to standard cross-entropy, focusing
    training on hard-to-classify examples. Per-class alpha weights compensate
    for any class imbalance in the training set.
...
def train_split(name, classes, config, train_dir, val_dir, ckpt_dir, device,
                use_focal=False):
    """Train one split (numbers or words) with v27.

    Args:
        use_focal: If True, use focal loss with class-balanced alpha weights
                   instead of standard cross-entropy. Used for words model
                   to address Market sink class absorption.
    """

    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] Training {name} ({len(classes)} classes) - v27")
    print(f"[{ts()}] {'=' * 70}")
    ...
    # v27: Focal loss for words model (class-balanced alpha weights)
    if use_focal:
        # Compute class-balanced alpha weights from training distribution
        class_counts = Counter(train_ds.labels)
        num_classes = len(classes)
        total_samples = len(train_ds)
        # Alpha = total / (num_classes * count_per_class), then normalize to mean=1
        ...
        alpha_weights = alpha_weights / alpha_weights.mean()  # normalize so mean=1
        alpha_weights = alpha_weights.to(device)
        cls_loss_fn = FocalLoss(
            gamma=2.0, alpha=alpha_weights,
            label_smoothing=config["label_smoothing"],
        )
        print(f"[{ts()}] Loss: FocalLoss(gamma=2.0, class-balanced alpha, ls={config['label_smoothing']})")
        print(f"[{ts()}]   Alpha weights: {dict(zip(classes, alpha_weights.cpu().tolist()))}")
    else:
        cls_loss_fn = None  # Use inline F.cross_entropy (unchanged from v26)
        print(f"[{ts()}] Loss: CE(ls={config['label_smoothing']}) [no focal]")
    ...
    for ep in range(config["epochs"]):
        ...
        for gcn_data, aux_data, targets, signer_targets in train_ld:
            ...
            if use_cutmix:
                ...
                # v27: Use focal loss if enabled, else standard CE
...
def run_loso(name, classes, config, train_dir, val_dir, ckpt_dir, results_dir, device):
    ...
    """
    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] LOSO Cross-Validation: {name} ({len(classes)} classes) - v27")
    print(f"[{ts()}] {'=' * 70}")
...
def evaluate_test_set(name, classes, config, test_dir, ckpt_path, device):
    ...
    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] Test Set Evaluation: {name} ({len(classes)} classes) - v27")
...
def main():
    parser = argparse.ArgumentParser(
        description="KSL ST-GCN Training v27 (Alpine HPC) - 12 signers + focal loss",
    ...
    print("=" * 70)
    print(f"KSL Training v27 - 12 signers + Focal Loss (words) + MediaPipe fix [{mode_str}]")
    print(f"Started: {ts()}")
    ...
    # v27: Train on 12 signers (train_alpha + val_alpha), validate on 3 (test_alpha)
    ...
    print(f"\n[{ts()}] v27 Config:")
    print(json.dumps(V27_CONFIG, indent=2, default=str))
    ...
    if args.loso:
        ...
        if args.model_type in ("numbers", "both"):
            ckpt_dir = os.path.join(args.checkpoint_dir, "v27_loso_numbers")
            results["numbers_loso"] = run_loso(
                "Numbers", NUMBER_CLASSES, V27_CONFIG,
                args.train_dir, args.val_dir, ckpt_dir, args.results_dir, device,
            )

        if args.model_type in ("words", "both"):
            ckpt_dir = os.path.join(args.checkpoint_dir, "v27_loso_words")
            results["words_loso"] = run_loso(
                "Words", WORD_CLASSES, V27_CONFIG,
                args.train_dir, args.val_dir, ckpt_dir, args.results_dir, device,
            )
    else:
        # Normal training mode — v27: 12 signers train, 3 val, focal for words
        if args.model_type in ("numbers", "both"):
            ckpt_dir = os.path.join(args.checkpoint_dir, "v27_numbers")
            results["numbers"] = train_split(
                "Numbers", NUMBER_CLASSES, V27_CONFIG,
                train_dirs, val_dir, ckpt_dir, device,
                use_focal=False,  # Numbers: keep CE with label_smoothing
            )

        if args.model_type in ("words", "both"):
            ckpt_dir = os.path.join(args.checkpoint_dir, "v27_words")
            results["words"] = train_split(
                "Words", WORD_CLASSES, V27_CONFIG,
                train_dirs, val_dir, ckpt_dir, device,
                use_focal=True,  # Words: focal loss to address Market sink
            )
    ...
    # Summary
    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] SUMMARY v27 [{mode_str}]")
    print(f"[{ts()}] {'=' * 70}")
    ...
    results_path = os.path.join(
        args.results_dir,
        f"v27{loso_tag}_{args.model_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    )
    results_out = {
        "version": "v27",
        "mode": mode_str,
        ...
        "changes_from_v26": [
            "1. MEDIAPIPE FIX: Training data re-extracted with MediaPipe 0.10.5 on Alpine "
               "(matches evaluation environment, no more version mismatch).",
            "2. 12 TRAINING SIGNERS: Combines train_alpha (1-10) + val_alpha (11-12). "
               "Validation on test_alpha (signers 13-15).",
            "3. FOCAL LOSS FOR WORDS: gamma=2, class-balanced alpha weights. "
               "Numbers model keeps standard CE with label_smoothing=0.1.",
        ],
    }
