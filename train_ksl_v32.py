#!/usr/bin/env python3
"""
KSL Training v32 (Alpine) - GroupNorm + SupCon (Signer-Agnostic)

Combines the two best techniques from v31:
  - GroupNorm (from v31 exp1): No BN adaptation needed at inference
  - SupCon loss (from v31 exp5): Cross-signer contrastive learning

Key config changes vs v31 exp1:
  - batch_size: 64 (up from 32, for cross-signer diversity in SupCon)
  - supcon_weight: 0.05 (down from 0.1, proven optimal in exp5)

This model is fully signer-agnostic: no AdaBN/Alpha-BN needed at inference.

Usage:
    python train_ksl_v32.py --model-type numbers
    python train_ksl_v32.py --model-type words
"""
...
class KSLGraphNetV32(nn.Module):
...
def train_stream(stream_name, name, classes, config, train_dir, val_dir,
    ...
    print(f"[{ts()}] Normalization: GroupNorm + SupCon (v32)")
    if use_focal:
...
def main():
    ...
    parser.add_argument("--epochs", type=int, default=None,
                        help="Override number of epochs (default: use config)")
    ...
    parser.add_argument("--checkpoint-dir", type=str, default=os.path.join(base_data, "checkpoints/v32b"))
    parser.add_argument("--results-dir", type=str, default=os.path.join(base_data, "results"))

    args = parser.parse_args()

    if args.epochs is not None:
        V32_CONFIG["epochs"] = args.epochs

    set_seed(args.seed)

    print("=" * 70)
    print(f"KSL Training v32 - GroupNorm + SupCon (Signer-Agnostic)")
    print(f"Started: {ts()}")
    ...
    print(f"  Streams: {V32_CONFIG['streams']}")
    print(f"  In channels per stream: {V32_CONFIG['in_channels']}")
    print(f"  Normalization: GroupNorm + SupCon (signer-agnostic)")

    print(f"\n[{ts()}] v32 Config:")
    print(json.dumps(V32_CONFIG, indent=2, default=str))
    ...
    for cat_name, classes, use_focal in categories:
        ...
        # Train each stream
        stream_results = {}
        for stream_name in V32_CONFIG["streams"]:
            result = train_stream(
                stream_name, cat_name, classes, V32_CONFIG,
        ...
        fusion_weights, fused_acc = learn_fusion_weights(
            cat_name, classes, V32_CONFIG, val_dir, ckpt_dir, device
        )
    ...
    results_path = os.path.join(
        args.results_dir,
        f"v32_{args.model_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    )
    results_out = {
        "version": "v32",
