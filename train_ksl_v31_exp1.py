#!/usr/bin/env python3
"""
KSL Training v31 Exp1 (Alpine) - GroupNorm replacing BatchNorm

Based on v28 multi-stream late fusion but with ALL BatchNorm layers replaced
by GroupNorm (num_groups=8 for channels>=8, num_groups=1 for smaller channels).

GroupNorm provides domain-invariant normalization (no signer-specific statistics).
Keeps the signer-adversarial head to help with domain invariance during training.

Changes from v28:
  1. Replace ALL nn.BatchNorm1d and nn.BatchNorm2d with nn.GroupNorm
  2. Keep signer-adversarial head (domain invariance training signal)
  3. Save checkpoint with metadata {"norm_type": "groupnorm"}

Usage:
    python train_ksl_v31_exp1.py --model-type numbers
    python train_ksl_v31_exp1.py --model-type words
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="KSL ST-GCN Training v31 Exp1 - GroupNorm instead of BatchNorm",
    ...
    parser.add_argument("--checkpoint-dir", type=str, default=os.path.join(base_data, "checkpoints/v31_exp1"))
    ...
    print("=" * 70)
    print(f"KSL Training v31 Exp1 - GroupNorm instead of BatchNorm")
    print(f"Started: {ts()}")
    ...
    print(f"  Streams: {V31_EXP1_CONFIG['streams']}")
    print(f"  In channels per stream: {V31_EXP1_CONFIG['in_channels']}")
    print(f"  Normalization: GroupNorm (replacing BatchNorm)")

    print(f"\n[{ts()}] v31 Exp1 Config:")
    print(json.dumps(V31_EXP1_CONFIG, indent=2, default=str))
    ...
    for cat_name, classes, use_focal in categories:
        ...
        # Train each stream
        stream_results = {}
        for stream_name in V31_EXP1_CONFIG["streams"]:
            result = train_stream(
                stream_name, cat_name, classes, V31_EXP1_CONFIG,
        ...
        fusion_weights, fused_acc = learn_fusion_weights(
            cat_name, classes, V31_EXP1_CONFIG, val_dir, ckpt_dir, device
        )
    ...
    # Summary
    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] SUMMARY v31 Exp1 - GroupNorm")
    print(f"[{ts()}] {'=' * 70}")
