#!/usr/bin/env python3
"""
KSL Training v44_expr3 - Skeleton-CutMix Bone Exchange Augmentation

Based on v43 (exp5 arch: BatchNorm, SupCon weight=0.05, R&R for numbers only).

Key idea: Exchange hand sub-skeletons between samples of the SAME class to
create plausible new training examples with mixed signer characteristics.
For each sample, with probability cutmix_prob, randomly pick another sample
of the same class and swap either the left or right hand skeleton.

IMPORTANT: CutMix is applied to the joint stream; bone and velocity streams
are RECOMPUTED from the augmented joints for consistency.

Changes from v43:
  1. **Skeleton-CutMix** augmentation added in training collate_fn
  2. New arg: --skeleton_cutmix_prob (default 0.5)
  3. Checkpoint dir: v44_expr3

Expected: Improved cross-signer generalization via hand-skeleton exchange.

Usage:
    python train_ksl_v44_expr3.py --model-type numbers
    python train_ksl_v44_expr3.py --model-type words
"""
...
# ---------------------------------------------------------------------------
# Joint groups for Skeleton-CutMix
# ---------------------------------------------------------------------------

JOINT_GROUPS = {
    'left_hand':  list(range(0, 21)),   # joints 0-20
    'right_hand': list(range(21, 42)),  # joints 21-41
    'pose':       list(range(42, 48)),  # joints 42-47
}

# ---------------------------------------------------------------------------
# Config (v44_expr3 - v43 base + Skeleton-CutMix)
# ---------------------------------------------------------------------------
...
V44_EXPR3_CONFIG = {
    ...
    "rr_speed_min": 0.7,
    "rr_speed_max": 1.3,
    # NEW: Skeleton-CutMix (applied via collate_fn after batching)
    "skeleton_cutmix_prob": 0.5,
}
...
def augment_recover_resample(data_list, config):
    ...
    Three stages applied in sequence:
      1. RECOVER: Linearly extrapolate start/end frames (5-15%) to simulate
         a wider temporal window around the sign.
      2. CROP-RESAMPLE: Randomly crop a sub-window (70-100%) from the
         recovered sequence and resample back to original length.
      3. SEGMENT WARP: Divide into segments and apply per-segment speed
         variation (0.7-1.3x) to simulate within-sign temporal differences.
...
# ---------------------------------------------------------------------------
# Skeleton-CutMix: exchange hand sub-skeletons between same-class samples
# ---------------------------------------------------------------------------

def skeleton_cutmix_batch(streams_batch, labels, mix_prob=0.5):
    """
    Apply Skeleton-CutMix to a batch: exchange left or right hand joints
    between samples of the SAME class.

    CutMix is applied to the joint stream; bone and velocity streams are
    RECOMPUTED from the augmented joint data for physical consistency.

    Args:
        streams_batch: dict with 'joint', 'bone', 'velocity' tensors of shape (B, 3, T, 48)
        labels: (B,) tensor of class labels
        mix_prob: probability of applying CutMix per sample

    Returns:
        Updated streams_batch dict with augmented tensors.
    """
    joint_data = streams_batch["joint"]  # (B, 3, T, 48)
    B, C, T, V = joint_data.shape

    augmented_joint = joint_data.clone()
    ...
    for i in range(B):
        ...
        if len(same_class) < 2:
            continue
        j_idx = same_class[random.randint(0, len(same_class) - 1)].item()
        if j_idx == i:
            continue

        # Randomly choose which body part to exchange
        group_name = random.choice(['left_hand', 'right_hand'])
        indices = JOINT_GROUPS[group_name]

        # Exchange: sample i gets sample j's hand skeleton
        augmented_joint[i, :, :, indices] = joint_data[j_idx, :, :, indices]

    # Recompute bone stream from augmented joint data
    # bone[child] = joint[child] - joint[parent]
    augmented_bone = torch.zeros_like(augmented_joint)
    for child in range(48):
        parent = PARENT_MAP[child]
        if parent >= 0:
            augmented_bone[:, :, :, child] = (
                augmented_joint[:, :, :, child] - augmented_joint[:, :, :, parent]
            )

    # Recompute velocity stream from augmented joint data
    # velocity[t] = joint[t] - joint[t-1]
    augmented_velocity = torch.zeros_like(augmented_joint)
    augmented_velocity[:, :, 1:, :] = (
        augmented_joint[:, :, 1:, :] - augmented_joint[:, :, :-1, :]
    )

    return {
        "joint": augmented_joint,
        "bone": augmented_bone,
        "velocity": augmented_velocity,
    }
...
class KSLMultiStreamDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # R&R: Recover-and-Resample (only when apply_rr=True, i.e., numbers)
...
# ---------------------------------------------------------------------------
# Model (KSLGraphNetV44Expr3 architecture — identical to V43/exp5)
# ---------------------------------------------------------------------------
...
class KSLGraphNetV44Expr3(nn.Module):
    """ST-GCN with aux branch, GRL, SupCon. Same architecture as V43/exp5."""
...
def train_stream(stream_name, name, classes, config, train_dir, val_dir,
    ...
    print(f"[{ts()}] Skeleton-CutMix prob: {skeleton_cutmix_prob}")
    print(f"[{ts()}] {'=' * 70}")
    ...
    for ep in range(config["epochs"]):
        ...
        for streams_batch, aux_data, targets, signer_targets in train_ld:
            # --- NEW: Apply Skeleton-CutMix to the batch ---
            if skeleton_cutmix_prob > 0:
                streams_batch = skeleton_cutmix_batch(
                    streams_batch, targets, mix_prob=skeleton_cutmix_prob
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="KSL ST-GCN Training v44_expr3 - Skeleton-CutMix + SupCon",
    ...
    parser.add_argument("--skeleton-cutmix-prob", type=float, default=0.5,
                        help="Per-sample probability of applying Skeleton-CutMix")
    ...
    parser.add_argument("--checkpoint-dir", type=str, default=os.path.join(base_data, "checkpoints", "v44_expr3"))
    ...
    V44_EXPR3_CONFIG["skeleton_cutmix_prob"] = args.skeleton_cutmix_prob

    print("=" * 70)
    print(f"KSL Training v44_expr3 - Skeleton-CutMix + SupCon")
    print(f"Started: {ts()}")
    print(f"Seed: {args.seed}")
    print(f"Skeleton-CutMix prob: {args.skeleton_cutmix_prob}")
    print("=" * 70)
    ...
    # Same data split as v28 (12 train, 3 val)
    train_dirs = [args.train_dir, args.val_dir]  # signers 1-12
    val_dir = args.test_dir                       # signers 13-15

    print(f"\nV44_expr3 Data Split:")
    print(f"  Train dirs: {train_dirs}  (signers 1-12)")
    print(f"  Val dir:    {val_dir}  (signers 13-15)")
    print(f"  Streams: {V44_EXPR3_CONFIG['streams']}")
    print(f"  In channels per stream: {V44_EXPR3_CONFIG['in_channels']}")
    print(f"  Batch size: {V44_EXPR3_CONFIG['batch_size']}")
    print(f"  SupCon weight: {V44_EXPR3_CONFIG['supcon_weight']}")
    print(f"  Skeleton-CutMix prob: {V44_EXPR3_CONFIG['skeleton_cutmix_prob']}")
    print(f"  R&R: applied to NUMBERS only (not words)")

    print(f"\n[{ts()}] V44_expr3 Config:")
    print(json.dumps(V44_EXPR3_CONFIG, indent=2, default=str))
    ...
    if args.model_type in ("numbers", "both"):
        categories.append(("Numbers", NUMBER_CLASSES, False, True))   # apply_rr=True
    if args.model_type in ("words", "both"):
        categories.append(("Words", WORD_CLASSES, True, False))        # apply_rr=False
    ...
    for cat_name, classes, use_focal, apply_rr in categories:
        ...
        # Train each stream
        stream_results = {}
        for stream_name in V44_EXPR3_CONFIG["streams"]:
            result = train_stream(
                stream_name, cat_name, classes, V44_EXPR3_CONFIG,
                train_dirs, val_dir, ckpt_dir, device,
                use_focal=use_focal, apply_rr=apply_rr,
                skeleton_cutmix_prob=args.skeleton_cutmix_prob,
            )
            if result:
        ...
        fusion_weights, fused_acc = learn_fusion_weights(
            cat_name, classes, V44_EXPR3_CONFIG, val_dir, ckpt_dir, device
        )
    ...
    # Summary
    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] SUMMARY V44_expr3 - Skeleton-CutMix + SupCon")
    print(f"[{ts()}] {'=' * 70}")
    ...
    results_path = os.path.join(
        args.results_dir,
        f"v44_expr3_{args.model_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    )
    results_out = {
        "version": "v44_expr3",
        ...
        "timestamp": ts(),
        "changes_from_v43": [
            "1. Skeleton-CutMix augmentation: exchange hand sub-skeletons between same-class samples",
            "2. CutMix applied to joint stream, bone/velocity recomputed for consistency",
            "3. New arg: --skeleton-cutmix-prob (default 0.5)",
            "4. Checkpoint dir: v44_expr3",
        ],
    }
