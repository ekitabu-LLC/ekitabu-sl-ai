#!/usr/bin/env python3
"""
KSL Training v44_expr7 - JMDA (Joint Mixing Data Augmentation)

Based on v43 (exp5 arch: BatchNorm, SupCon weight=0.05, R&R for numbers only).

Key addition: SpatialMix + TemporalMix data augmentation for cross-signer diversity.

SpatialMix: Mix joint positions from different samples by body part group
  (left hand, right hand, or pose) to create virtual signers with mixed
  hand shapes/positions. Preserves within-group spatial coherence.

TemporalMix: Mix temporal segments between samples at a random cut point
  to create sequences with mixed temporal patterns.

Both augmentations:
  - Operate on raw joint coordinates BEFORE bone/velocity computation
  - Use soft cross-entropy loss: lam * CE(pred, label1) + (1-lam) * CE(pred, label2)
  - Are applied with configurable probabilities (default 0.3 each)
  - Are applied after all other spatial augmentations but before temporal operations

Usage:
    python train_ksl_v44_expr7.py --model-type numbers
    python train_ksl_v44_expr7.py --model-type words
"""
...
# ---------------------------------------------------------------------------
# Config (v44_expr7 - JMDA: SpatialMix + TemporalMix)
# ---------------------------------------------------------------------------
...
V44_EXPR7_CONFIG = {
    ...
    "cutmix_alpha": 1.0,
    "cutmix_prob": 0.3,
    # R&R augmentation (applied to numbers only via apply_rr flag)
    "rr_prob": 0.7,
    "rr_extend_min": 0.05,
    "rr_extend_max": 0.15,
    "rr_crop_min": 0.7,
    ...
    "rr_speed_min": 0.7,
    "rr_speed_max": 1.3,
    # NEW: JMDA augmentation
    "spatial_mix_prob": 0.3,
    "temporal_mix_prob": 0.3,
}
...
# ---------------------------------------------------------------------------
# Recover-and-Resample Temporal Augmentation
# ---------------------------------------------------------------------------

def augment_recover_resample(data_list, config):
    T = data_list[0].shape[0]
    if T < 4:
        return data_list

    n_ref = min(5, T // 4)
    extend_ratio = np.random.uniform(config["rr_extend_min"], config["rr_extend_max"])
    n_extend = max(1, int(T * extend_ratio))
    ...
    for arr in data_list:
        ...
        if n_ref >= 2:
            start_vel = np.mean(arr[1:n_ref] - arr[:n_ref-1], axis=0)
        else:
            start_vel = np.zeros(shape_rest, dtype=np.float32)

        pre_frames = []
        for k in range(n_extend, 0, -1):
            pre_frames.append(arr[0] - k * start_vel)
        pre = np.stack(pre_frames, axis=0) if pre_frames else np.empty((0,) + shape_rest, dtype=np.float32)

        if n_ref >= 2:
            end_vel = np.mean(arr[-n_ref:] - arr[-n_ref-1:-1], axis=0)
        else:
            end_vel = np.zeros(shape_rest, dtype=np.float32)

        post_frames = []
        for k in range(1, n_extend + 1):
            post_frames.append(arr[-1] + k * end_vel)
        post = np.stack(post_frames, axis=0) if post_frames else np.empty((0,) + shape_rest, dtype=np.float32)

        recovered.append(np.concatenate([pre, arr, post], axis=0))

    T_ext = recovered[0].shape[0]

    crop_ratio = np.random.uniform(config["rr_crop_min"], 1.0)
    crop_len = max(4, int(T_ext * crop_ratio))
    max_start = T_ext - crop_len
    crop_start = np.random.randint(0, max(1, max_start + 1))

    cropped = [arr[crop_start:crop_start + crop_len] for arr in recovered]

    crop_T = cropped[0].shape[0]
    if crop_T != T:
        indices_float = np.linspace(0, crop_T - 1, T)
        indices_low = np.floor(indices_float).astype(int)
        indices_high = np.minimum(indices_low + 1, crop_T - 1)
        frac = (indices_float - indices_low).astype(np.float32)

        resampled = []
        for arr in cropped:
            frac_shape = [T] + [1] * (arr.ndim - 1)
            frac_bc = frac.reshape(frac_shape)
            resampled.append(
                arr[indices_low] * (1.0 - frac_bc) + arr[indices_high] * frac_bc
            )
    else:
        resampled = cropped

    num_seg = config["rr_num_segments"]
    speed_min = config["rr_speed_min"]
    speed_max = config["rr_speed_max"]

    speeds = np.random.uniform(speed_min, speed_max, num_seg)

    seg_len = T / num_seg
    timestamps = []
    for i in range(T):
        seg_idx = min(int(i / seg_len), num_seg - 1)
        local_t = (i - seg_idx * seg_len) / seg_len
        timestamps.append(seg_idx + local_t * speeds[seg_idx])

    ts_arr = np.array(timestamps)
    ts_arr = (ts_arr - ts_arr[0]) / max(ts_arr[-1] - ts_arr[0], 1e-8) * (T - 1)

    idx_low = np.floor(ts_arr).astype(int)
    idx_high = np.minimum(idx_low + 1, T - 1)
    idx_low = np.clip(idx_low, 0, T - 1)
    frac = (ts_arr - idx_low).astype(np.float32)
    ...
    for arr in resampled:
        ...
        frac_bc = frac.reshape(frac_shape)
        warped.append(
            arr[idx_low] * (1.0 - frac_bc) + arr[idx_high] * frac_bc
        )
...
# ---------------------------------------------------------------------------
# JMDA: SpatialMix + TemporalMix (NEW in v44_expr7)
# ---------------------------------------------------------------------------

# Body part joint groups for SpatialMix
BODY_PART_GROUPS = {
    'left_hand': list(range(0, 21)),
    'right_hand': list(range(21, 42)),
    'pose': list(range(42, 48)),
}


def spatial_mix_batch(h_batch, labels_batch, spatial_mix_prob):
    """
    SpatialMix: mix joint positions by body part group between paired samples.

    For each sample in the batch, with probability spatial_mix_prob:
      1. Pick a random partner from the batch
      2. Pick a random body part group (left_hand, right_hand, or pose)
      3. Linearly interpolate that body part's joints with a random ratio lam in [0.3, 0.7]

    Args:
        h_batch: list of (T_i, 48, 3) numpy arrays (raw joint positions, variable length)
        labels_batch: list of int labels
        spatial_mix_prob: probability of applying spatial mix per sample

    Returns:
        h_mixed: list of (T_i, 48, 3) numpy arrays
        mix_labels: list of int (partner labels, or same label if not mixed)
        lam_list: list of float (mixing ratios; 1.0 = no mixing applied)
    """
    B = len(h_batch)
    h_mixed = [h.copy() for h in h_batch]
    mix_labels = list(labels_batch)
    lam_list = [1.0] * B

    for i in range(B):
        if random.random() > spatial_mix_prob:
            continue

        # Pick a random partner (different sample)
        j = random.randint(0, B - 1)
        if j == i:
            j = (i + 1) % B

        # Pick a random body part group
        group_name = random.choice(list(BODY_PART_GROUPS.keys()))
        joints = BODY_PART_GROUPS[group_name]

        # Mixing ratio
        lam = random.uniform(0.3, 0.7)

        # Need to align temporal lengths for mixing
        Ti = h_mixed[i].shape[0]
        Tj = h_batch[j].shape[0]

        if Tj != Ti:
            # Resample partner to match this sample's length
            indices_float = np.linspace(0, Tj - 1, Ti)
            idx_low = np.floor(indices_float).astype(int)
            idx_high = np.minimum(idx_low + 1, Tj - 1)
            frac = (indices_float - idx_low).astype(np.float32)
            frac_bc = frac.reshape(Ti, 1, 1)
            partner_resampled = (h_batch[j][idx_low] * (1.0 - frac_bc) +
                                 h_batch[j][idx_high] * frac_bc)
        else:
            partner_resampled = h_batch[j]

        # Mix the selected body part joints
        h_mixed[i][:, joints, :] = (
            lam * h_mixed[i][:, joints, :] +
            (1 - lam) * partner_resampled[:, joints, :]
        )

        # lam = proportion from original sample
        # Weight by number of joints mixed vs total
        joint_ratio = len(joints) / 48.0
        effective_lam = 1.0 - joint_ratio * (1.0 - lam)
        lam_list[i] = effective_lam
        mix_labels[i] = labels_batch[j]

    return h_mixed, mix_labels, lam_list


def temporal_mix_batch(h_batch, labels_batch, temporal_mix_prob):
    """
    TemporalMix: mix temporal segments between paired samples.

    For each sample in the batch, with probability temporal_mix_prob:
      1. Pick a random partner from the batch
      2. Choose a cut point t in [T//4, 3*T//4]
      3. Replace frames after cut point with partner's frames

    Args:
        h_batch: list of (T_i, 48, 3) numpy arrays
        labels_batch: list of int labels
        temporal_mix_prob: probability of applying temporal mix per sample

    Returns:
        h_mixed: list of (T_i, 48, 3) numpy arrays
        mix_labels: list of int (partner labels)
        lam_list: list of float (proportion from original)
    """
    B = len(h_batch)
    h_mixed = [h.copy() for h in h_batch]
    mix_labels = list(labels_batch)
    ...
    for i in range(B):
        if random.random() > temporal_mix_prob:
            continue
        ...
        # Cut point
        t_cut = random.randint(Ti // 4, 3 * Ti // 4)

        # Resample partner to match this sample's length if needed
        if Tj != Ti:
            indices_float = np.linspace(0, Tj - 1, Ti)
            idx_low = np.floor(indices_float).astype(int)
            idx_high = np.minimum(idx_low + 1, Tj - 1)
        ...
        # Replace frames after cut point
        h_mixed[i][t_cut:, :, :] = partner_resampled[t_cut:, :, :]

        lam_list[i] = t_cut / Ti  # proportion from original
        mix_labels[i] = labels_batch[j]
...
# ---------------------------------------------------------------------------
# Multi-Stream Dataset with JMDA support
# ---------------------------------------------------------------------------

class KSLMultiStreamDatasetJMDA(Dataset):
    """
    Returns per-stream tensors (joint/bone/velocity).

    JMDA mixing happens at collate time (batch-level), not per-sample.
    This dataset returns raw joint data (pre-mixing) plus the standard
    augmented/processed streams for when mixing is not applied.

    For JMDA: we store the raw joint array before bone/velocity computation
    so mixing can be applied and streams recomputed.
    """

    def __init__(self, data_dirs, classes, config, aug=False, apply_rr=False):
        self.mf = config["max_frames"]
        self.aug = aug
        self.config = config
        self.apply_rr = apply_rr
    ...
    def _load_and_augment_joints(self, idx):
        """Load raw data and apply per-sample spatial augmentations. Returns (T, 48, 3) joints."""
        ...
        # Hand-body features BEFORE normalization (from raw coordinates)
        hand_body_feats = compute_hand_body_features(h)

        # Normalization
        h = normalize_wrist_palm(h)

        # Bone-length perturbation
        ...
        return h, hand_body_feats

    def _process_joints_to_streams(self, h, hand_body_feats):
        """
        Given augmented joints (T, 48, 3) and hand_body_feats (T, 8),
        compute all streams and aux features, apply temporal augmentations,
        and return final tensors.
        """
        # Compute velocity
        ...
        # R&R (only when apply_rr=True, i.e., numbers)
        if self.aug and self.apply_rr and np.random.random() < self.config["rr_prob"]:
            h, velocity, bones, joint_angles, fingertip_dists, hand_body_feats = \
                augment_recover_resample(
                    [h, velocity, bones, joint_angles, fingertip_dists, hand_body_feats],
                    self.config,
                )
    ...
    def __getitem__(self, idx):
        h, hand_body_feats = self._load_and_augment_joints(idx)
        streams, aux_tensor = self._process_joints_to_streams(h, hand_body_feats)
...
# ---------------------------------------------------------------------------
# JMDA Collate Function (applies mixing at batch level)
# ---------------------------------------------------------------------------

class JMDACollator:
    """
    Custom collate function that applies SpatialMix or TemporalMix at batch level.

    When JMDA is triggered:
    1. Load raw joints for all samples in batch
    2. Apply SpatialMix OR TemporalMix to the raw joints
    3. Recompute bone/velocity streams from mixed joints
    4. Return mixed streams + mixing info (partner labels, lambdas)

    When JMDA is NOT triggered (or for validation):
    Falls back to standard collation.
    """

    def __init__(self, dataset, spatial_mix_prob=0.3, temporal_mix_prob=0.3, enabled=True):
        self.dataset = dataset
        self.spatial_mix_prob = spatial_mix_prob
        self.temporal_mix_prob = temporal_mix_prob
        self.enabled = enabled

    def __call__(self, batch_indices_or_items):
        """
        batch_indices_or_items: list of (streams, aux, label, signer) tuples
        from dataset.__getitem__
        """
        # Standard collation - the dataset already processed everything
        streams_list, aux_list, labels_list, signer_list = zip(*batch_indices_or_items)
        stream_names = list(streams_list[0].keys())
        streams_batch = {
            name: torch.stack([s[name] for s in streams_list])
            for name in stream_names
        ...
        if not self.enabled:
            return streams_batch, aux_batch, labels, signers, labels, torch.ones(len(labels))

        # Decide whether to apply JMDA on this batch
        # Apply SpatialMix OR TemporalMix (not both) per batch
        roll = random.random()
        apply_spatial = roll < self.spatial_mix_prob
        apply_temporal = (not apply_spatial) and (roll < self.spatial_mix_prob + self.temporal_mix_prob)

        if not apply_spatial and not apply_temporal:
            return streams_batch, aux_batch, labels, signers, labels, torch.ones(len(labels))

        # For JMDA: we apply mixing on the already-processed joint stream
        # and recompute bone/velocity from mixed joints.
        # This is simpler and more correct than re-loading raw data.
        B, C, T, V = streams_batch["joint"].shape  # (B, 3, 90, 48)

        # Convert joint stream back to (B, T, V, C) for mixing
        joints_np = streams_batch["joint"].permute(0, 2, 3, 1).numpy()  # (B, T, 48, 3)

        labels_np = labels.numpy().tolist()

        if apply_spatial:
            joints_list = [joints_np[i] for i in range(B)]
            mixed_joints, mix_labels, lam_list = spatial_mix_batch(
                joints_list, labels_np, spatial_mix_prob=1.0  # already decided to apply
            )
            mix_type = "spatial"
        else:
            joints_list = [joints_np[i] for i in range(B)]
            mixed_joints, mix_labels, lam_list = temporal_mix_batch(
                joints_list, labels_np, temporal_mix_prob=1.0
            )
            mix_type = "temporal"

        # Recompute streams from mixed joints
        mixed_joint_tensors = []
        mixed_bone_tensors = []
        mixed_velocity_tensors = []
        mixed_aux_list = []

        for i in range(B):
            h = mixed_joints[i].astype(np.float32)

            # Recompute velocity from mixed joints
            velocity = np.zeros_like(h)
            velocity[1:] = h[1:] - h[:-1]

            # Recompute bones from mixed joints
            bones = compute_bones(h)
            ...
            mixed_joint_tensors.append(
                torch.FloatTensor(h_clipped).permute(2, 0, 1))
            mixed_bone_tensors.append(
                torch.FloatTensor(bones_clipped).permute(2, 0, 1))
            mixed_velocity_tensors.append(
                torch.FloatTensor(velocity_clipped).permute(2, 0, 1))

            # Recompute aux features from mixed joints
            joint_angles = compute_joint_angles(h)
            fingertip_dists = compute_fingertip_distances(h)
            # Hand-body features: use original (mixing doesn't change global position much)
            # Actually recompute for correctness
            # Note: hand_body_features needs raw (non-normalized) coords, but after
            # normalization the features are different. Since we mixed normalized joints,
            # we just keep the original aux features for mixed samples.
            mixed_aux_list.append(aux_list[i])

        streams_batch["joint"] = torch.stack(mixed_joint_tensors)
        streams_batch["bone"] = torch.stack(mixed_bone_tensors)
        streams_batch["velocity"] = torch.stack(mixed_velocity_tensors)
        # Keep original aux (hand-body features from original samples)
        # aux_batch stays the same

        mix_labels_tensor = torch.tensor(mix_labels, dtype=torch.long)
        lam_tensor = torch.tensor(lam_list, dtype=torch.float32)

        return streams_batch, aux_batch, labels, signers, mix_labels_tensor, lam_tensor
...
# ---------------------------------------------------------------------------
# Model (same architecture as V43/exp5)
# ---------------------------------------------------------------------------
...
class KSLGraphNetV44Expr7(nn.Module):
    """ST-GCN with aux branch, GRL, SupCon. Same arch as V43/exp5."""
...
def train_stream(stream_name, name, classes, config, train_dir, val_dir,
                 ckpt_dir, device, use_focal=False, apply_rr=False):
    stream_ckpt_dir = os.path.join(ckpt_dir, stream_name)

    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] Training {name} - Stream: {stream_name} ({len(classes)} classes)")
    print(f"[{ts()}] R&R augmentation: {'YES (numbers)' if apply_rr else 'NO (words)'}")
    print(f"[{ts()}] JMDA: SpatialMix prob={config['spatial_mix_prob']}, "
          f"TemporalMix prob={config['temporal_mix_prob']}")
    print(f"[{ts()}] {'=' * 70}")

    adj = build_adj(config["num_nodes"]).to(device)

    train_ds = KSLMultiStreamDatasetJMDA(train_dir, classes, config, aug=True, apply_rr=apply_rr)
    val_ds = KSLMultiStreamDatasetJMDA(val_dir, classes, config, aug=False)
    ...
    # JMDA collator for training
    jmda_collator = JMDACollator(
        dataset=train_ds,
        spatial_mix_prob=config["spatial_mix_prob"],
        temporal_mix_prob=config["temporal_mix_prob"],
        enabled=True,
    )
    ...
    train_ld = DataLoader(
        ...
        collate_fn=jmda_collator,
    )

    # Validation uses standard collate (no JMDA)
    val_jmda_collator = JMDACollator(
        dataset=val_ds,
        spatial_mix_prob=0,
        temporal_mix_prob=0,
        enabled=False,
    ...
    val_ld = DataLoader(
        ...
        collate_fn=val_jmda_collator,
    )

    tk = tuple(config.get("temporal_kernels", [3, 5, 7]))
    model = KSLGraphNetV44Expr7(
        nc=len(classes),
    ...
    print(f"[{ts()}] Config: ic={config['in_channels']}, hd={config['hidden_dim']}, "
          f"nl={config['num_layers']}, dr={config['dropout']}, batch={config['batch_size']}")
    print(f"[{ts()}] SupCon weight: {config['supcon_weight']}")
    if use_focal:
        print(f"[{ts()}] Loss: FocalLoss(gamma=2.0)")
    else:
    ...
    jmda_applied_count = 0
    jmda_total_batches = 0
    ...
    for ep in range(config["epochs"]):
        ...
        for streams_batch, aux_data, targets, signer_targets, mix_labels, lam_weights in train_ld:
            ...
            lam_weights = lam_weights.to(device)
            opt.zero_grad()

            jmda_total_batches += 1

            # Check if JMDA was applied (any lam != 1.0)
            jmda_active = (lam_weights < 0.999).any().item()
            if jmda_active:
                jmda_applied_count += 1

            # When JMDA is active, use soft labels
            if jmda_active:
                logits, signer_logits, embedding = model(
                    gcn_data, aux_data, grl_lambda=grl_lambda,
                )

                # Soft CE loss: lam * CE(pred, label_a) + (1-lam) * CE(pred, label_b)
                if cls_loss_fn is not None:
                    # Per-sample weighted focal loss
                    loss_a = F.cross_entropy(logits, targets, reduction='none',
                                             label_smoothing=config["label_smoothing"])
                    loss_b = F.cross_entropy(logits, mix_labels, reduction='none',
                                             label_smoothing=config["label_smoothing"])
                    cls_loss = (lam_weights * loss_a + (1 - lam_weights) * loss_b).mean()
                else:
                    loss_a = F.cross_entropy(logits, targets, reduction='none',
                                             label_smoothing=config["label_smoothing"])
                    loss_b = F.cross_entropy(logits, mix_labels, reduction='none',
                ...
                # SupCon uses original labels (unmixed)
                ...
                _, p = logits.max(1)
                tc += (lam_weights * p.eq(targets).float() +
                       (1 - lam_weights) * p.eq(mix_labels).float()).sum().item()

            else:
                # Standard training (no JMDA this batch) - may still use cutmix/mixup
        ...
        if ep < 10 or (ep + 1) % 10 == 0 or va > best:
            jmda_rate = 100.0 * jmda_applied_count / max(jmda_total_batches, 1)
            print(
                f"[{ts()}] [{stream_name}] Ep {ep + 1:3d}/{config['epochs']} | "
                f"Loss: {tl / max(len(train_ld), 1):.4f} | Train: {ta:.1f}% | Val: {va:.1f}% | "
                f"SignerAcc: {signer_acc:.1f}% | JMDA: {jmda_rate:.0f}% | "
...
def learn_fusion_weights(category_name, classes, config, val_dir, ckpt_dir, device):
    ...
    val_ds = KSLMultiStreamDatasetJMDA(val_dir, classes, config, aug=False)
    val_collator = JMDACollator(dataset=val_ds, enabled=False)
    val_ld = DataLoader(val_ds, batch_size=config["batch_size"], shuffle=False,
                        num_workers=2, pin_memory=True, collate_fn=val_collator)
...
def main():
    parser = argparse.ArgumentParser(
        description="KSL Training v44_expr7 - JMDA (SpatialMix + TemporalMix)",
    ...
    parser.add_argument("--spatial-mix-prob", type=float, default=0.3)
    parser.add_argument("--temporal-mix-prob", type=float, default=0.3)
    ...
    parser.add_argument("--checkpoint-dir", type=str, default=os.path.join(base_data, "checkpoints", "v44_expr7"))
    parser.add_argument("--results-dir", type=str, default=os.path.join(base_data, "results"))

    args = parser.parse_args()

    # Override config with CLI args
    V44_EXPR7_CONFIG["spatial_mix_prob"] = args.spatial_mix_prob
    V44_EXPR7_CONFIG["temporal_mix_prob"] = args.temporal_mix_prob

    set_seed(args.seed)

    print("=" * 70)
    print(f"KSL Training v44_expr7 - JMDA (SpatialMix + TemporalMix)")
    print(f"Started: {ts()}")
    print(f"Seed: {args.seed}")
    print(f"SpatialMix prob: {args.spatial_mix_prob}")
    print(f"TemporalMix prob: {args.temporal_mix_prob}")
    print("=" * 70)
    ...
    print(f"\nv44_expr7 Data Split:")
    print(f"  Train dirs: {train_dirs}  (signers 1-12)")
    print(f"  Val dir:    {val_dir}  (signers 13-15)")
    print(f"  Streams: {V44_EXPR7_CONFIG['streams']}")
    print(f"  In channels per stream: {V44_EXPR7_CONFIG['in_channels']}")
    print(f"  Batch size: {V44_EXPR7_CONFIG['batch_size']}")
    print(f"  SupCon weight: {V44_EXPR7_CONFIG['supcon_weight']}")
    print(f"  R&R: applied to NUMBERS only (not words)")
    print(f"  JMDA: SpatialMix={V44_EXPR7_CONFIG['spatial_mix_prob']}, "
          f"TemporalMix={V44_EXPR7_CONFIG['temporal_mix_prob']}")

    print(f"\n[{ts()}] v44_expr7 Config:")
    print(json.dumps(V44_EXPR7_CONFIG, indent=2, default=str))
    ...
    if args.model_type in ("numbers", "both"):
        categories.append(("Numbers", NUMBER_CLASSES, False, True))
    if args.model_type in ("words", "both"):
        categories.append(("Words", WORD_CLASSES, True, False))

    for cat_name, classes, use_focal, apply_rr in categories:
        cat_key = cat_name.lower()
        ckpt_dir = os.path.join(args.checkpoint_dir, cat_key)

        stream_results = {}
        for stream_name in V44_EXPR7_CONFIG["streams"]:
            result = train_stream(
                stream_name, cat_name, classes, V44_EXPR7_CONFIG,
                train_dirs, val_dir, ckpt_dir, device,
                use_focal=use_focal, apply_rr=apply_rr,
            )
            if result:
                stream_results[stream_name] = result

        fusion_weights, fused_acc = learn_fusion_weights(
            cat_name, classes, V44_EXPR7_CONFIG, val_dir, ckpt_dir, device
        )
    ...
    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] SUMMARY v44_expr7 - JMDA (SpatialMix + TemporalMix)")
    print(f"[{ts()}] {'=' * 70}")
    ...
    results_path = os.path.join(
        args.results_dir,
        f"v44_expr7_{args.model_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    )
    results_out = {
        "version": "v44_expr7",
        "model_type": args.model_type,
        "seed": args.seed,
        "config": V44_EXPR7_CONFIG,
        "results": results,
        ...
        "timestamp": ts(),
        "changes_from_v43": [
            "1. JMDA: SpatialMix augmentation (mix body part joints between samples)",
            "2. JMDA: TemporalMix augmentation (mix temporal segments between samples)",
            "3. Mixing applied to joint coordinates, bone/velocity recomputed from mixed joints",
            "4. Soft CE loss: lam * CE(pred, label_a) + (1-lam) * CE(pred, label_b)",
            "5. SpatialMix OR TemporalMix chosen per batch (not both)",
            "6. Same base: exp5 arch (BatchNorm, SupCon), R&R for numbers only",
        ],
    }
