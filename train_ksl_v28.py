#!/usr/bin/env python3
"""
KSL Training v28 (Alpine) - Multi-Stream Late Fusion + Self-Supervised Pretrain

Based on v27 (12 signers, focal loss for words, MediaPipe 0.10.5). Three changes:

  1. MULTI-STREAM LATE FUSION: Train 3 separate 3-channel models (joint, bone,
     velocity), then fuse softmax scores with learned weights. Ensemble diversity
     typically outperforms early fusion (v27's 9-channel concatenation).
  2. SELF-SUPERVISED PRETRAIN (optional): Masked Joint Prediction on all ksl-alpha
     data (all 15 signers, no labels needed). Pretrains the ST-GCN encoder to learn
     skeleton geometry before fine-tuning with classification loss.
  3. FUSION WEIGHT LEARNING: Grid search optimal per-stream weights on val set.

Architecture per stream: Same KSLGraphNetV25 with ic=3. ~260K params per stream.
Total across 3 streams: ~780K params (similar to v27's 778K for 9-channel).

Usage:
    python train_ksl_v28.py --model-type numbers
    python train_ksl_v28.py --model-type words
    python train_ksl_v28.py --model-type both --seed 42
    python train_ksl_v28.py --model-type both --seed 42 --pretrain
"""
...
# ---------------------------------------------------------------------------
# Config (v28 - multi-stream: in_channels=3 per stream)
# ---------------------------------------------------------------------------

V28_CONFIG = {
    "max_frames": 90,
    "num_nodes": 48,
    "in_channels": 3,           # per-stream (was 9 in v27)
...
# ---------------------------------------------------------------------------
# Temporal CutMix (simplified for 3-channel streams)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Model (same KSLGraphNetV25 architecture, now with ic=3 per stream)
# ---------------------------------------------------------------------------
...
class KSLGraphNetV25(nn.Module):
    """ST-GCN with aux branch, GRL, SupCon. Same as v25-v27, ic=3 for v28."""
...
# ---------------------------------------------------------------------------
# Masked Joint Prediction Decoder (v28 NEW - self-supervised pretrain)
# ---------------------------------------------------------------------------

class MaskedJointDecoder(nn.Module):
    """Decoder for masked joint prediction pretraining.

    Takes ST-GCN encoder output (B, final_ch, T', N) and reconstructs
    original joint coordinates (B, 3, T, N).
    """

    def __init__(self, in_channels, target_frames):
        super().__init__()
    ...
    def forward(self, x):
        # x: (B, C, T', N) -- T' may be reduced by strides
        # Upsample temporal dim back to target
        if x.shape[2] != self.target_frames:
            x = F.interpolate(
                x, size=(self.target_frames, x.shape[3]),
                mode='bilinear', align_corners=False
            )
        return self.decoder(x)  # (B, 3, T, N)


class PretrainEncoder(nn.Module):
    """ST-GCN encoder for pretraining (data_bn + layers, no pooling/classifier)."""

    def __init__(self, nn_=48, ic=3, hd=64, nl=4, tk=(3, 5, 7), dr=0.3,
    ...
    def forward(self, gcn_input):
        ...
        for layer in self.layers:
            x = layer(x)
        return x  # (B, final_ch, T', N)


# ---------------------------------------------------------------------------
# Self-Supervised Pretrain: Masked Joint Prediction (v28 NEW)
# ---------------------------------------------------------------------------

def pretrain_masked_joints(classes, all_data_dirs, config, device):
    """Self-supervised pretraining: mask joints, predict masked coordinates.

    Uses ALL ksl-alpha data (all 15 signers, no labels needed).
    Returns path to saved encoder weights.
    """
    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] Self-Supervised Pretrain: Masked Joint Prediction")
    print(f"[{ts()}] {'=' * 70}")
    print(f"[{ts()}] Data dirs: {all_data_dirs}")

    adj = build_adj(config["num_nodes"]).to(device)
    mf = config["max_frames"]
    mask_ratio = config["pretrain_mask_ratio"]

    # Load dataset (all signers, no augmentation for pretraining)
    ds = KSLMultiStreamDataset(all_data_dirs, classes, config, aug=False)
    dl = DataLoader(ds, batch_size=config["batch_size"], shuffle=True,
                    num_workers=2, pin_memory=True, collate_fn=stream_collate_fn)

    print(f"[{ts()}] Pretrain dataset: {len(ds)} samples")
    ...
    decoder = MaskedJointDecoder(encoder.final_ch, mf).to(device)

    param_count = sum(p.numel() for p in encoder.parameters()) + \
                  sum(p.numel() for p in decoder.parameters())
    print(f"[{ts()}] Pretrain params: {param_count:,} "
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        opt, T_max=config["pretrain_epochs"], eta_min=1e-6,
    )
    ...
    for ep in range(config["pretrain_epochs"]):
        ...
        for streams_batch, _, _, _ in dl:
            # Use joint stream for pretraining
            joint_data = streams_batch["joint"].to(device)  # (B, 3, T, N)
            B, C, T, N = joint_data.shape

            # Create mask: randomly mask mask_ratio of nodes (same across time)
            n_mask = max(1, int(N * mask_ratio))
            mask_indices = np.random.choice(N, n_mask, replace=False)
            mask = torch.ones(B, 1, 1, N, device=device)
            mask[:, :, :, mask_indices] = 0

            # Masked input
            masked_input = joint_data * mask

            # Forward
            encoded = encoder(masked_input)  # (B, final_ch, T', N)
            reconstructed = decoder(encoded)  # (B, 3, T, N)

            # MSE loss on masked joints only
            inv_mask = 1 - mask  # (B, 1, 1, N) -- 1 where masked
            loss = ((reconstructed - joint_data) ** 2 * inv_mask).sum() / (
                inv_mask.sum() * C + 1e-8
            )
        ...
        if ep < 5 or (ep + 1) % 10 == 0 or avg_loss < best_loss:
            print(f"[{ts()}] Pretrain Ep {ep+1:3d}/{config['pretrain_epochs']} | "
                  f"MSE: {avg_loss:.6f} | Time: {ep_time:.1f}s")
    ...
    pretrain_dir = os.path.join(
        os.path.dirname(all_data_dirs[0]) if isinstance(all_data_dirs, list) else os.path.dirname(all_data_dirs),
        "checkpoints", "v28_pretrained"
    )
    os.makedirs(pretrain_dir, exist_ok=True)
    pretrain_path = os.path.join(pretrain_dir, "pretrained_encoder.pt")
    torch.save({
    ...
    print(f"[{ts()}] Pretrained encoder saved to {pretrain_path}")
    print(f"[{ts()}] Final MSE loss: {best_loss:.6f}")

    return pretrain_path


def load_pretrained_encoder(model, pretrained_path, device):
    """Load pretrained ST-GCN encoder weights into a KSLGraphNetV25 model.

    Matches data_bn and layer weights. Skips classifier, signer_head, aux branch.
    """
    pretrained = torch.load(pretrained_path, map_location=device, weights_only=False)
    encoder_state = pretrained["encoder"]
    ...
    for key in encoder_state:
        # Map PretrainEncoder keys to KSLGraphNetV25 keys (same names)
        if key in model_state and model_state[key].shape == encoder_state[key].shape:
    ...
    print(f"[{ts()}]   Loaded {loaded} pretrained layers, skipped {skipped}")


# ---------------------------------------------------------------------------
# Train One Stream (adapted from v27's train_split)
# ---------------------------------------------------------------------------

def train_stream(stream_name, name, classes, config, train_dir, val_dir,
                 ckpt_dir, device, use_focal=False, pretrained_path=None):
    ...
    Args:
        ...
        pretrained_path: optional path to pretrained encoder weights
    """
    ...
    if pretrained_path and os.path.exists(pretrained_path):
        print(f"[{ts()}] Loading pretrained encoder from {pretrained_path}")
        load_pretrained_encoder(model, pretrained_path, device)
...
# ---------------------------------------------------------------------------
# Fusion Weight Learning (v28 NEW)
# ---------------------------------------------------------------------------
...
def main():
    parser = argparse.ArgumentParser(
        description="KSL ST-GCN Training v28 - Multi-Stream Late Fusion",
    ...
    parser.add_argument("--pretrain", action="store_true",
                        help="Run self-supervised masked joint prediction pretraining")
    ...
    print("=" * 70)
    print(f"KSL Training v28 - Multi-Stream Late Fusion")
    print(f"Started: {ts()}")
    ...
    print(f"  Streams: {V28_CONFIG['streams']}")
    print(f"  In channels per stream: {V28_CONFIG['in_channels']}")

    print(f"\n[{ts()}] v28 Config:")
    print(json.dumps(V28_CONFIG, indent=2, default=str))
    ...
    for cat_name, classes, use_focal in categories:
        ...
        ckpt_dir = os.path.join(args.checkpoint_dir, f"v28_{cat_key}")

        # Optional self-supervised pretraining
        pretrained_path = None
        if args.pretrain:
            pretrained_path = pretrain_masked_joints(
                classes, all_data_dirs, V28_CONFIG, device
            )

        # Train each stream
        stream_results = {}
        for stream_name in V28_CONFIG["streams"]:
            result = train_stream(
                stream_name, cat_name, classes, V28_CONFIG,
        ...
        fusion_weights, fused_acc = learn_fusion_weights(
            cat_name, classes, V28_CONFIG, val_dir, ckpt_dir, device
        )
    ...
    # Summary
    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] SUMMARY v28 - Multi-Stream Late Fusion")
    print(f"[{ts()}] {'=' * 70}")
    ...
    results_path = os.path.join(
        args.results_dir,
        f"v28_{args.model_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    )
    results_out = {
        "version": "v28",
        ...
        "timestamp": ts(),
        "changes_from_v27": [
            "1. MULTI-STREAM LATE FUSION: 3 separate 3-channel models (joint/bone/velocity), "
               "fused with learned softmax weights.",
            "2. SELF-SUPERVISED PRETRAIN (optional): Masked Joint Prediction on all ksl-alpha data.",
            "3. FUSION WEIGHT LEARNING: Grid search optimal per-stream weights on val set.",
        ],
    }
