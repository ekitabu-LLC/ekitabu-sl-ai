#!/usr/bin/env python3
"""
V43 Real-Tester Evaluation — Category-Conditional R&R + SupCon (exp5 arch).

V43 uses BatchNorm (not GroupNorm) — same architecture as exp5 (KSLGraphNetV25/V43).
No projection head. R&R applied during training for numbers only.

Expected:
  Numbers: ~67.8% (benefits from R&R, same as v41)
  Words:   ~65.4% (no R&R, same as exp5)

Usage:
    python evaluate_v43.py --category both
    python evaluate_v43.py --category numbers
    python evaluate_v43.py --category words
"""
...
def load_v43_models(ckpt_dir, classes, device):
    """Load V43 category-conditional R&R models (exp5 arch: BatchNorm, no projection head)."""
...
def main():
    parser = argparse.ArgumentParser(
        description="V43 Real-Tester Evaluation (Category-Conditional R&R + SupCon)")
    parser.add_argument("--category", type=str, default="both",
                        choices=["numbers", "words", "both"])
    parser.add_argument("--checkpoint-base", type=str,
                        default="/scratch/alpine/hama5612/ksl-dir-2/data/checkpoints/v43")
    ...
    print("=" * 70)
    print("V43 Real-Tester Evaluation")
    print("Category-Conditional R&R + BatchNorm + SupCon (exp5 arch)")
    print("  Numbers: R&R applied during training (target ~67.8%)")
    print("  Words:   No R&R (target ~65.4%)")
    ...
    for cat_name, cat_videos, classes in categories:
        ...
        print(f"\n  Loading V43 models from {ckpt_dir}...")
        models, fusion_weights = load_v43_models(ckpt_dir, classes, device)
        ...
        result = evaluate_method(f"v43_{cat_name}", cat_videos, predict_fn, classes)
        all_results[cat_name] = result

    # --- Summary ---
    print(f"\n{'='*70}")
    print("FINAL SUMMARY - V43 Category-Conditional R&R (exp5 arch)")
    print(f"{'='*70}")
    print(f"\n  Reference baselines:")
    print(f"    exp5 SupCon:                Numbers=61.0%, Words=65.4%")
    print(f"    v41 R&R (uniform):          Numbers=67.8%, Words=55.6%")
    print(f"    v42 (GroupNorm+proj head):  Numbers=67.8%, Words=58.0%")
    ...
    os.makedirs(os.path.join(results_dir, "v43"), exist_ok=True)
    out_path = os.path.join(results_dir, "v43", "real_tester_results.json")
