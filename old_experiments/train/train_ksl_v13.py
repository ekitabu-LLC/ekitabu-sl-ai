"""
KSL Training v13 - Robust to Sparse Hand Detection

Key Changes from v12:
- Aggressive landmark dropout augmentation (30-70% dropout per hand)
- Frame-level hand dropout to simulate poor detection
- More robust normalization
- Heavier augmentation overall

Usage:
    modal run train_ksl_v13.py
"""

import modal
import os

app = modal.App("ksl-trainer-v13")
...
def train_robust_models():
    ...
    print("=" * 70)
    print("KSL Training v13 - Robust to Sparse Hand Detection")
    print("=" * 70)
    ...
    class Dataset_(Dataset):
        ...
        def __getitem__(s, idx):
            ...
            # Hand landmark dropout augmentation
            if s.aug and np.random.random() < CONFIG["hand_dropout_prob"]:
                dropout_rate = np.random.uniform(0.3, 0.7)
            ...
            # Frame dropout
            if s.aug and np.random.random() < CONFIG["frame_drop_prob"]:
                drop_ratio = np.random.uniform(0.1, 0.3)
                frame_mask = np.random.random(f) > drop_ratio
                if np.sum(frame_mask) < f * 0.3:
                    frame_mask[:int(f*0.3)] = True
                h[~frame_mask, :42, :] = 0
            ...
            if s.aug:
                ...
                if np.random.random() < 0.3:
                    ...
                    new_len = int(s.mf * speed)
                    ind = np.linspace(0, s.mf-1, new_len, dtype=int)
                    ind = np.clip(ind, 0, s.mf-1)
                    h_new = h[ind]
                    if new_len >= s.mf:
                        h = h_new[:s.mf]
                    else:
                        h = np.concatenate([h_new, np.zeros((s.mf-new_len, 48, 3), dtype=np.float32)])
    ...
    def train_one(name, classes, ckpt_dir):
        print(f"\n{'='*70}\nTraining {name} ({len(classes)} classes) - Robust v13\n{'='*70}")
        ...
        for ep in range(CONFIG["epochs"]):
            ...
            if va > best:
                best, patience = va, 0
                torch.save({"model": model.state_dict(), "val_acc": va, "classes": classes, "num_nodes": 48, "version": "v13"}, os.path.join(ckpt_dir, "best_model.pt"))
                print(f"  -> Best! {va:.1f}%")
            else:
                patience += 1
    ...
    results = {}
    results["numbers"] = train_one("Numbers", NUMBER_CLASSES, "/data/checkpoints_v13_numbers")
    results["words"] = train_one("Words", WORD_CLASSES, "/data/checkpoints_v13_words")
    ...
    with open("/data/checkpoints_v13_results.json", "w") as f:
