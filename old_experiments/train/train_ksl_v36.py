#!/usr/bin/env python3
"""
KSL Training v36 (Alpine) - Synthetic Signer Augmentation + GroupNorm

Based on v31 Exp1 (GroupNorm replacing BatchNorm) with synthetic signer
augmentation to expand effective signer diversity during training.

10 synthetic signer profiles with fixed transform parameters are generated
at startup. During training, 50% of samples get transformed by a random
synthetic signer, expanding effective signer count from 12 to 22 for the
GRL (Gradient Reversal Layer) signer-adversarial head.

Each synthetic signer profile defines a unique combination of:
  - Hand scale and per-finger bone scales
  - Shoulder width scaling
  - Signing speed (temporal resampling)
  - Spatial biases (height and lateral offsets)
  - Hand rotation bias
  - Gaussian noise level

Changes from v31 Exp1:
  1. Add 10 synthetic signer profiles with minimum pairwise distance
  2. Apply synthetic signer transforms to 50% of training samples
  3. Expand GRL signer head from 12 to 22 classes (12 real + 10 synthetic)
  4. All other architecture, losses, augmentation identical to v31 Exp1

Usage:
    python train_ksl_v36.py --model-type numbers
    python train_ksl_v36.py --model-type words
"""
    ...
    try:
        ...
        if version != "0.10.5":
            print(f"[{ts()}] WARNING: MediaPipe version is {version}, "
                  f"v31 training data was extracted with 0.10.5.")
        else:
...
# ---------------------------------------------------------------------------
# Config (v36 - same as v31 exp1 + synthetic signer augmentation)
# ---------------------------------------------------------------------------
...
V36_CONFIG = {
    ...
    "cutmix_alpha": 1.0,
    "cutmix_prob": 0.3,
    # Synthetic signer augmentation (v36)
    "num_synthetic_signers": 10,
    "synthetic_prob": 0.5,
    "synthetic_seed": 42,
}
...
# ---------------------------------------------------------------------------
# Synthetic Signer Profile Generation (v36)
# ---------------------------------------------------------------------------

def generate_synthetic_profiles(n, seed):
    """Generate n synthetic signer profiles with minimum pairwise distance.

    Each profile is a dict with fixed transform parameters that simulate
    a unique signer's physical characteristics and signing style.

    Parameters are sampled uniformly within biologically plausible ranges.
    A minimum pairwise L2 distance of 0.3 in normalized [0,1] parameter
    space is enforced to ensure sufficient diversity between profiles.

    Args:
        n: Number of synthetic profiles to generate.
    ...
    PARAM_RANGES = {
        "hand_scale": (0.8, 1.2),
        "bone_thumb": (0.85, 1.15),
        "bone_index": (0.85, 1.15),
        "bone_middle": (0.85, 1.15),
        "bone_ring": (0.85, 1.15),
        "bone_pinky": (0.85, 1.15),
        "shoulder_width": (0.85, 1.25),
        "signing_speed": (0.75, 1.25),
        "height_bias": (-0.12, 0.12),
        "lateral_bias": (-0.08, 0.08),
        "rotation_bias": (-10.0, 10.0),
        "noise_level": (0.005, 0.025),
    }

    param_names = list(PARAM_RANGES.keys())
    ranges = np.array([PARAM_RANGES[k] for k in param_names])  # (D, 2)

    def sample_profile():
        return {
            "hand_scale": rng.uniform(0.8, 1.2),
            "bone_scales": {
                "thumb": rng.uniform(0.85, 1.15),
                "index": rng.uniform(0.85, 1.15),
                "middle": rng.uniform(0.85, 1.15),
                "ring": rng.uniform(0.85, 1.15),
                "pinky": rng.uniform(0.85, 1.15),
            },
            "shoulder_width": rng.uniform(0.85, 1.25),
            "signing_speed": rng.uniform(0.75, 1.25),
            "height_bias": rng.uniform(-0.12, 0.12),
            "lateral_bias": rng.uniform(-0.08, 0.08),
            "rotation_bias": rng.uniform(-10.0, 10.0),
            "noise_level": rng.uniform(0.005, 0.025),
        }

    def profile_to_vector(p):
        """Flatten profile to a vector for distance computation."""
        return np.array([
            p["hand_scale"],
            p["bone_scales"]["thumb"],
            p["bone_scales"]["index"],
            p["bone_scales"]["middle"],
            p["bone_scales"]["ring"],
            p["bone_scales"]["pinky"],
            p["shoulder_width"],
            p["signing_speed"],
    ...
    def normalize_vector(v):
        """Normalize each dimension to [0,1] based on its range."""
        mins = ranges[:, 0]
        maxs = ranges[:, 1]
        return (v - mins) / (maxs - mins + 1e-10)
    ...
    for i in range(n):
        for attempt in range(100):
            ...
            cand_vec = normalize_vector(profile_to_vector(candidate))

            # Check minimum distance to all existing profiles
            too_close = False
            for existing_vec in norm_vectors:
                dist = np.linalg.norm(cand_vec - existing_vec)
            ...
            if not too_close:
                ...
                norm_vectors.append(cand_vec)
                break
        else:
            # After 100 retries, accept the last candidate anyway
            profiles.append(candidate)
            norm_vectors.append(cand_vec)
...
# ---------------------------------------------------------------------------
# Synthetic Signer Transform (v36)
# ---------------------------------------------------------------------------

def apply_synthetic_transform(h, profile):
    """Apply a synthetic signer's characteristic transforms to skeleton data.

    Takes h of shape (T, 48, 3) and a profile dict. Applies 7 sub-transforms
    in order to simulate a different signer's physical characteristics.

    Args:
        h: Skeleton data of shape (T, 48, 3).
        profile: Dict with transform parameters from generate_synthetic_profiles.

    Returns:
        Transformed skeleton data of shape (T', 48, 3) where T' may differ
        from T due to temporal resampling by signing_speed.
    """
    h = h.copy()
    T = h.shape[0]

    # 1. Scale hands around wrist by hand_scale
    hand_scale = profile["hand_scale"]
    # Left hand: nodes 0-20, wrist = node 0
    lh_valid = np.abs(h[:, :21, :]).sum(axis=(1, 2)) > 0.01
    if np.any(lh_valid):
        wrist_lh = h[lh_valid, 0:1, :]  # (V, 1, 3)
        displacement = h[lh_valid, :21, :] - wrist_lh
        h[lh_valid, :21, :] = wrist_lh + displacement * hand_scale

    # Right hand: nodes 21-41, wrist = node 21
    rh_valid = np.abs(h[:, 21:42, :]).sum(axis=(1, 2)) > 0.01
    if np.any(rh_valid):
        wrist_rh = h[rh_valid, 21:22, :]  # (V, 1, 3)
        displacement = h[rh_valid, 21:42, :] - wrist_rh
        h[rh_valid, 21:42, :] = wrist_rh + displacement * hand_scale

    # 2. Scale finger bones by bone_scales
    finger_names = ['thumb', 'index', 'middle', 'ring', 'pinky']
    all_chains = LH_CHAINS + RH_CHAINS
    for chain_idx, chain in enumerate(all_chains):
        finger_idx = chain_idx % 5  # 0=thumb, 1=index, etc.
        scale = profile["bone_scales"][finger_names[finger_idx]]
        for bone_pos, (parent_idx, child_idx) in enumerate(chain):
            bone_vec = h[:, child_idx, :] - h[:, parent_idx, :]
            new_pos = h[:, parent_idx, :] + bone_vec * scale
            displacement = new_pos - h[:, child_idx, :]
            h[:, child_idx, :] = new_pos
            # Propagate displacement to downstream nodes in this chain
            found = False
            for p2, c2 in chain:
                if found:
                    h[:, c2, :] += displacement
    ...
    # 3. Scale shoulder width (pose nodes 42-47, x-axis only)
    sw_scale = profile["shoulder_width"]
    mid_shoulder = (h[:, 42, :] + h[:, 43, :]) / 2.0  # (T, 3)
    for node_idx in range(42, 48):
        # Scale displacement from mid-shoulder along x-axis only
        dx = h[:, node_idx, 0] - mid_shoulder[:, 0]
        h[:, node_idx, 0] = mid_shoulder[:, 0] + dx * sw_scale

    # 4. Temporal resample by signing_speed
    speed = profile["signing_speed"]
    if speed != 1.0:
        new_len = int(round(T * (1.0 / speed)))
        new_len = max(max(1, T // 3), min(new_len, T * 3))
        if new_len != T:
            old_times = np.linspace(0, 1, T)
            new_times = np.linspace(0, 1, new_len)
            h_resampled = np.zeros((new_len, 48, 3), dtype=np.float32)
            for node in range(48):
                for coord in range(3):
                    h_resampled[:, node, coord] = np.interp(
                        new_times, old_times, h[:, node, coord]
                    )
            h = h_resampled
            T = new_len

    # 5. Add spatial biases
    h[:, :, 1] += profile["height_bias"]   # y-coordinate
    h[:, :, 0] += profile["lateral_bias"]  # x-coordinate

    # 6. Rotate both hands around wrist by rotation_bias degrees
    angle = np.radians(profile["rotation_bias"])
    ...
    # 7. Add consistent Gaussian noise
    noise = np.random.normal(0, profile["noise_level"], h.shape).astype(np.float32)
    h = h + noise
...
def temporal_cutmix(gcn_input, aux_input, labels, signer_labels, alpha=1.0):
    """CutMix at temporal dimension for single-stream (3-ch) skeleton sequences."""
...
class KSLMultiStreamDataset(Dataset):
    ...
    def __init__(self, data_dirs, classes, config, aug=False,
                 synthetic_profiles=None, synthetic_prob=0.0):
        ...
        self.num_real_signers = self.num_signers

        # Expand num_signers to include synthetic signers for GRL head
        if self.synthetic_profiles is not None:
            self.num_signers = self.num_real_signers + len(self.synthetic_profiles)

        dirs_str = ", ".join(data_dirs)
        print(f"[{ts()}]   Loaded {len(self.samples)} samples from {dirs_str}")
        print(f"[{ts()}]   Signers: {self.num_real_signers} real "
              f"({dict(Counter(extract_signer_id(os.path.basename(p)) for p in self.samples))})")
        if self.synthetic_profiles is not None:
            print(f"[{ts()}]   Synthetic signers: {len(self.synthetic_profiles)} "
                  f"(total for GRL: {self.num_signers})")
    ...
    def __getitem__(self, idx):
        ...
        # Synthetic signer transform (applied before all other augmentation)
        signer_label = self.signer_labels[idx]
        if self.aug and self.synthetic_profiles and random.random() < self.synthetic_prob:
            synth_idx = random.randint(0, len(self.synthetic_profiles) - 1)
            h = apply_synthetic_transform(h, self.synthetic_profiles[synth_idx])
            signer_label = self.num_real_signers + synth_idx  # new signer ID for GRL
        ...
        if self.aug and np.random.random() < self.config["hand_dropout_prob"]:
            ...
            if np.random.random() < 0.6:
                lh_mask = np.random.random(h.shape[0]) > dropout_rate
                h[~lh_mask, :21, :] = 0
            if np.random.random() < 0.6:
                rh_mask = np.random.random(h.shape[0]) > dropout_rate
                h[~rh_mask, 21:42, :] = 0
        ...
        return streams, aux_tensor, self.labels[idx], signer_label
...
# ---------------------------------------------------------------------------
# Multi-Scale Temporal Convolution (v31: GroupNorm instead of BatchNorm)
# ---------------------------------------------------------------------------

class MultiScaleTCN(nn.Module):
    def __init__(self, channels, kernels=(3, 5, 7), stride=1, dropout=0.3):
        super().__init__()
        # GroupNorm: use num_groups=8 for channels>=8, num_groups=1 for smaller
        num_groups = 8 if channels >= 8 else 1
        self.branches = nn.ModuleList([
            nn.Sequential(
                nn.Conv2d(channels, channels, (k, 1), padding=(k // 2, 0), stride=(stride, 1)),
                nn.GroupNorm(num_groups, channels),  # v31: GroupNorm instead of BatchNorm2d
            )
            for k in kernels
        ])
...
# ---------------------------------------------------------------------------
# Model (v31: GroupNorm instead of BatchNorm)
# ---------------------------------------------------------------------------
...
class KSLGraphNetV31Exp1(nn.Module):
    """ST-GCN with GroupNorm instead of BatchNorm. Keeps signer-adversarial head."""

    def __init__(self, nc, num_signers, aux_dim, nn_=48, ic=3, hd=64, nl=4,
                 tk=(3, 5, 7), dr=0.3, spatial_dropout=0.1, adj=None):
        super().__init__()
        self.register_buffer("adj", adj)
        # v31: GroupNorm for data_bn (was BatchNorm1d(nn_ * ic))
        # nn_ * ic = 48 * 3 = 144 channels
        ...
        # v31: GroupNorm for aux_bn (was BatchNorm1d(aux_dim))
        # aux_dim may not be divisible by 8 (e.g., 61), so find a valid divisor
        num_groups_aux = 1
        for g in [8, 4, 2]:
            if aux_dim % g == 0:
                num_groups_aux = g
        ...
        # v31: GroupNorm for aux_temporal_conv (was BatchNorm1d(64))
        ...
        # Keep signer-adversarial head (domain invariance during training)
    ...
    def forward(self, gcn_input, aux_input, grl_lambda=0.0):
        b, c, t, n = gcn_input.shape
        # v31: GroupNorm expects (B, C, ...), reshape for data_bn
        x = self.data_bn(
        ...
        # v31: GroupNorm for aux_bn (expects (B, C, T))
...
def train_stream(stream_name, name, classes, config, train_dir, val_dir,
                 ckpt_dir, device, use_focal=False, synthetic_profiles=None):
    """Train one stream model (joint, bone, or velocity)."""
    ...
    train_ds = KSLMultiStreamDataset(
        ...
        synthetic_prob=config["synthetic_prob"],
    )
    ...
    print(f"[{ts()}] Aux dim: {aux_dim}, Num signers: {num_signers} "
          f"({train_ds.num_real_signers} real + "
          f"{num_signers - train_ds.num_real_signers} synthetic)")
    ...
    print(f"[{ts()}] Normalization: GroupNorm (v36)")
    if use_focal:
            ...
            torch.save(
                {
                    ...
                    "stream": stream_name,
                    "version": "v36",
                    "norm_type": "groupnorm",  # v31: metadata for eval
                    "config": config,
                },
                best_path,
            )
...
def learn_fusion_weights(category_name, classes, config, val_dir, ckpt_dir, device):
    ...
    # Collect all predictions per stream
...
def main():
    parser = argparse.ArgumentParser(
        description="KSL ST-GCN Training v36 - Synthetic Signer Augmentation + GroupNorm",
    ...
    parser.add_argument("--checkpoint-dir", type=str, default=os.path.join(base_data, "checkpoints/v36"))
    ...
    print("=" * 70)
    print(f"KSL Training v36 - Synthetic Signer Augmentation + GroupNorm")
    print(f"Started: {ts()}")
    ...
    # Generate synthetic signer profiles
    synthetic_profiles = generate_synthetic_profiles(
        V36_CONFIG["num_synthetic_signers"], V36_CONFIG["synthetic_seed"]
    )
    print(f"\nSynthetic signer profiles ({len(synthetic_profiles)} generated):")
    for i, prof in enumerate(synthetic_profiles):
        print(f"  Signer S{i}: hand_scale={prof['hand_scale']:.3f}, "
              f"shoulder={prof['shoulder_width']:.3f}, "
              f"speed={prof['signing_speed']:.3f}, "
              f"h_bias={prof['height_bias']:.4f}, "
              f"l_bias={prof['lateral_bias']:.4f}, "
              f"rot={prof['rotation_bias']:.1f}deg, "
              f"noise={prof['noise_level']:.4f}")
        bs = prof['bone_scales']
        print(f"         bones: T={bs['thumb']:.3f} I={bs['index']:.3f} "
              f"M={bs['middle']:.3f} R={bs['ring']:.3f} P={bs['pinky']:.3f}")

    # Data split (same as v28: 12 train, 3 val)
    ...
    print(f"  Streams: {V36_CONFIG['streams']}")
    print(f"  In channels per stream: {V36_CONFIG['in_channels']}")
    print(f"  Normalization: GroupNorm (replacing BatchNorm)")
    print(f"  Synthetic signers: {V36_CONFIG['num_synthetic_signers']} "
          f"(prob={V36_CONFIG['synthetic_prob']})")

    print(f"\n[{ts()}] v36 Config:")
    print(json.dumps(V36_CONFIG, indent=2, default=str))
    ...
    for cat_name, classes, use_focal in categories:
        ...
        # Train each stream
        stream_results = {}
        for stream_name in V36_CONFIG["streams"]:
            result = train_stream(
                stream_name, cat_name, classes, V36_CONFIG,
        ...
        fusion_weights, fused_acc = learn_fusion_weights(
            cat_name, classes, V36_CONFIG, val_dir, ckpt_dir, device
        )
    ...
    # Summary
    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] SUMMARY v36 - Synthetic Signer Augmentation + GroupNorm")
    print(f"[{ts()}] {'=' * 70}")
    ...
    results_path = os.path.join(
        args.results_dir,
        f"v36_{args.model_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    )
    results_out = {
        "version": "v36",
        ...
        "changes_from_v31_exp1": [
            "1. SYNTHETIC SIGNERS: 10 synthetic signer profiles with fixed transforms.",
            "2. 50% of training samples transformed by random synthetic signer.",
            "3. GRL signer head expanded from 12 to 22 (12 real + 10 synthetic).",
            "4. Same GroupNorm architecture and all other settings from v31 exp1.",
        ],
    }
