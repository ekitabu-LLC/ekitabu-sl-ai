#!/usr/bin/env python3
"""
KSL Training v39 - SupCon on Cleaned Data

Identical architecture and hyperparameters to v31 exp5 (cross-signer contrastive).
Only change: trained on cleaned data (interpolated bad frames + DBSCAN outlier removal).

Key differences from v31_exp5:
  1. Train data: data/train_alpha_clean + data/val_alpha_clean (cleaned)
  2. Val data: data/test_alpha (unchanged - real testers)
  3. Checkpoint dir: data/checkpoints/v39
  4. Version tag: v39

Architecture, hyperparameters, augmentation all identical to v31_exp5.

Usage:
    python train_ksl_v39.py --model-type numbers
    python train_ksl_v39.py --model-type words
"""
...
# ---------------------------------------------------------------------------
# Config (v39 - same as v31_exp5, trained on cleaned data)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Model (KSLGraphNetV25 architecture - identical to exp5)
# ---------------------------------------------------------------------------
...
def main():
    parser = argparse.ArgumentParser(
        description="KSL ST-GCN Training v39 - SupCon on Cleaned Data",
    ...
    parser.add_argument("--train-dir", type=str, default=os.path.join(base_data, "train_alpha_clean"))
    parser.add_argument("--val-dir", type=str, default=os.path.join(base_data, "val_alpha_clean"))
    parser.add_argument("--test-dir", type=str, default=os.path.join(base_data, "test_alpha"))
    parser.add_argument("--checkpoint-dir", type=str, default=os.path.join(base_data, "checkpoints", "v39"))
    ...
    print("=" * 70)
    print(f"KSL Training v39 - SupCon on Cleaned Data")
    print(f"Started: {ts()}")
    ...
    # Same data split as exp5 (12 train, 3 val) but using cleaned data
    train_dirs = [args.train_dir, args.val_dir]  # signers 1-12 (cleaned)
    val_dir = args.test_dir                       # signers 13-15 (unchanged)

    print(f"\nv39 Data Split (cleaned data):")
    print(f"  Train dirs: {train_dirs}  (signers 1-12, cleaned)")
    print(f"  Val dir:    {val_dir}  (signers 13-15, unchanged)")
    print(f"  Streams: {V39_CONFIG['streams']}")
    print(f"  In channels per stream: {V39_CONFIG['in_channels']}")
    print(f"  Batch size: {V39_CONFIG['batch_size']}")
    print(f"  SupCon weight: {V39_CONFIG['supcon_weight']}")

    print(f"\n[{ts()}] v39 Config:")
    print(json.dumps(V39_CONFIG, indent=2, default=str))
    ...
    for cat_name, classes, use_focal in categories:
        ...
        # Train each stream
        stream_results = {}
        for stream_name in V39_CONFIG["streams"]:
            result = train_stream(
                stream_name, cat_name, classes, V39_CONFIG,
        ...
        fusion_weights, fused_acc = learn_fusion_weights(
            cat_name, classes, V39_CONFIG, val_dir, ckpt_dir, device
        )
    ...
    # Summary
    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] SUMMARY v39 - SupCon on Cleaned Data")
    print(f"[{ts()}] {'=' * 70}")
    ...
    results_path = os.path.join(
        args.results_dir,
        f"v39_{args.model_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    )
    results_out = {
        "version": "v39",
        ...
        "timestamp": ts(),
        "changes_from_exp5": [
            "1. Training data: data/train_alpha_clean + data/val_alpha_clean (cleaned)",
            "2. Cleaning: interpolated bad frames + DBSCAN outlier removal",
            "3. Architecture, hyperparameters, augmentation all identical to v31_exp5",
        ],
    }
