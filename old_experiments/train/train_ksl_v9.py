"""
KSL Training v9 - Temporal Segmentation for Multi-Digit Numbers

Problems from v8:
1. Class 444 still 0% - gets confused with 54 (65%) and 35 (25%)
2. New confusions: 66->17, 91->73, 89->73/Teach

Solutions in v9:
1. TEMPORAL FEATURES - 8 new features for multi-digit detection
   - Duration, segment similarity, velocity stats, gesture peaks
2. UPDATED CONFUSION PAIRS - Higher penalties for 444
3. NEW CONFUSION PAIRS - Address 66->17, 91->73, 89 issues
...
# Updated for v9 based on v8 results
CONFUSION_PAIRS = [
    # 444 attractor (CRITICAL - increased penalties)
    ("444", "54", 5.0),   # Increased from 3.5
    ("444", "35", 3.0),   # New: 5 errors in v8

    # Existing (keep from v8)
    ("22", "54", 2.5),
    ("125", "54", 2.5),
    ("100", "54", 2.0),

    # New confusions from v8 results
    ("66", "17", 2.5),    # 6 errors
    ("125", "22", 2.0),   # 5 errors
    ("91", "73", 2.0),    # 5 errors
    ("89", "73", 2.0),    # 3 errors
...
CONFIG = {
    "max_frames": 90,
    "feature_dim": 657,  # Changed from 649: 649 + 8 temporal features = 657
    "hidden_dim": 320,
    "batch_size": 24,
    "epochs": 250,
    "learning_rate": 3e-4,
    ...
    "checkpoint_dir": "/data/checkpoints_v9",
...
def train_model(model_type: str = "v9") -> dict:
    import torch
    ...
    print("=" * 70)
    print("KSL Training v9 - Temporal Segmentation for Multi-Digit Numbers")
    print("=" * 70)
    print("Key fixes:")
    print("  - 8 new temporal features for multi-digit detection")
    print("  - Increased confusion penalties for 444->54, 444->35")
    print("  - New confusion pairs: 66->17, 91->73, 89->73/Teach")
    print("=" * 70)
    ...
    def compute_temporal_features(data):
        ...
        # 2. Segment similarity (3 segments -> 3 pairwise similarities)
    ...
    def engineer_features(data):
        ...
        # NEW: Add temporal features (8 features per frame, broadcast to all frames)
        temporal = compute_temporal_features(data)
        temporal_broadcast = np.tile(temporal, (data.shape[0], 1))  # Broadcast to all frames

        return np.concatenate([data, left_features, right_features, left_vel, right_vel, temporal_broadcast], axis=1)

    # Dataset
    class KSLDatasetV9(Dataset):
    ...
    # Load data
    print("\nLoading datasets...")
    train_dataset = KSLDatasetV9(CONFIG["train_dir"], ALL_CLASSES, augment=True)
    val_dataset = KSLDatasetV9(CONFIG["val_dir"], ALL_CLASSES, augment=False)
    ...
    # Model
    model = TemporalPyramidV9(num_classes=len(ALL_CLASSES), feature_dim=CONFIG["feature_dim"], hidden_dim=CONFIG["hidden_dim"])
    model = model.cuda()
    ...
    for epoch in range(CONFIG["epochs"]):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save({
                ...
                "classes": ALL_CLASSES,
                "epoch": epoch + 1,
            }, CONFIG["checkpoint_dir"] + "/ksl_v9_best.pth")
    ...
    # Final report
    checkpoint = torch.load(CONFIG["checkpoint_dir"] + "/ksl_v9_best.pth", weights_only=False)
    ...
    with open(CONFIG["checkpoint_dir"] + "/ksl_v9_history.json", "w") as f:
        json.dump(history, f)
...
def main(model: str = "v9"):
    ...
    print("\nDownload: modal volume get ksl-dataset-vol /data/checkpoints_v9/ ./checkpoints/")
