#!/usr/bin/env python3
"""
KSL Training V45_expr2 - Class-Aware Hard Negative SupCon

Based on v43 (category-conditional R&R + exp5 architecture).

Key change: Replace standard SupCon with hard-negative SupCon that weights
negative pairs by class confusion scores. Classes that the model frequently
confuses get higher weight in the contrastive loss, forcing the embedding
space to better separate confusable classes.

Design:
  1. ClassConfusionTracker: running EMA of confusion matrix from training logits
  2. HardNegSupConLoss: SupCon weighted by confusion scores for negatives
  3. Warm-up: first 50 epochs use standard SupCon (tracker needs time to stabilize)
  4. After epoch 50: switch to hard negative SupCon

Architecture: exp5 (BatchNorm, no projection head, SupCon w=0.05)
Category-conditional R&R: R&R for numbers only (same as v43)
500 epochs, no early stopping.

Usage:
    python train_ksl_v45_expr2.py --model-type numbers
    python train_ksl_v45_expr2.py --model-type words
"""
...
# ---------------------------------------------------------------------------
# Config (v45_expr2 - hard negative SupCon)
# ---------------------------------------------------------------------------
...
V45_EXPR2_CONFIG = {
    ...
    "patience": 80,
    "warmup_epochs": 10,
    # SupCon (same weight as exp5, but hard-negative variant)
    "supcon_weight": 0.05,
    "supcon_temperature": 0.07,
    # Hard negative SupCon params
    "hard_neg_warmup_epochs": 50,  # standard SupCon for first 50 epochs
    "confusion_momentum": 0.99,    # EMA momentum for confusion tracker
    # Signer adversarial
    "grl_lambda_max": 0.3,
    "grl_start_epoch": 0,
    "grl_ramp_epochs": 100,
    # Augmentation (same as exp5/v43)
    "rotation_prob": 0.5,
...
# ---------------------------------------------------------------------------
# Recover-and-Resample Temporal Augmentation (from v41/v43)
# ---------------------------------------------------------------------------

def augment_recover_resample(data_list, config):
    """
    Recover-and-Resample temporal augmentation.
    Applied ONLY for numbers training (via apply_rr flag in dataset).
    """
    T = data_list[0].shape[0]
    if T < 4:
        return data_list

    # --- Stage 1: RECOVER (temporal extrapolation) ---
    n_ref = min(5, T // 4)
    ...
    # --- Stage 3: SEGMENT WARP (per-segment speed variation) ---
...
# ---------------------------------------------------------------------------
# Class Confusion Tracker (NEW for v45_expr2)
# ---------------------------------------------------------------------------

class ClassConfusionTracker:
    """Track running EMA of class confusion from training logits."""

    def __init__(self, num_classes, momentum=0.99):
        self.num_classes = num_classes
        self.momentum = momentum
        # confusion[i, j] = how much class i gets confused with class j
        self.confusion = torch.zeros(num_classes, num_classes)

    def update(self, logits, labels):
        """Update confusion matrix using softmax probabilities from logits."""
        with torch.no_grad():
            probs = F.softmax(logits.detach(), dim=-1).cpu()  # (B, C)
            # Accumulate per-class confusion
            for c in range(self.num_classes):
                mask = (labels.cpu() == c)
                if mask.sum() == 0:
                    continue
                class_probs = probs[mask].mean(dim=0)  # avg probs for class c
                self.confusion[c] = (
                    self.momentum * self.confusion[c]
                    + (1 - self.momentum) * class_probs
                )
            # Zero out diagonal (self-confusion not useful)
            self.confusion.fill_diagonal_(0)

    def get_negative_weights(self, labels, device):
        """
        Get per-sample negative class weights based on confusion scores.
        ...
        Returns:
            weights: (B, num_classes) normalized confusion weights
        """
        weights = self.confusion[labels.cpu()]  # (B, C)
...
# ---------------------------------------------------------------------------
# Standard Supervised Contrastive Loss (for warmup)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Hard Negative Supervised Contrastive Loss (NEW for v45_expr2)
# ---------------------------------------------------------------------------

class HardNegSupConLoss(nn.Module):
    """SupCon loss that weights negatives by class confusion scores."""

    def __init__(self, temperature=0.07):
        super().__init__()
        self.temperature = temperature

    def forward(self, features, labels, confusion_weights):
        """
        Args:
            features: (B, D) embedding vectors
            labels: (B,) class indices
            confusion_weights: (B, num_classes) per-anchor weight for each class as negative
        """
        device = features.device
        B = features.shape[0]
        if B <= 1:
        ...
        pos_count = pos_mask.float().sum(1)
        if (pos_count == 0).all():
            return torch.tensor(0.0, device=device)

        # Build negative weight matrix: for anchor i, weight of sample j as negative
        # = confusion_weights[i, label_j]
        neg_weight_matrix = confusion_weights[:, labels]  # (B, B)
        ...
        # Weighted negative sum: only count negatives, weighted by confusion
        weighted_neg_sum = (exp_logits * neg_weight_matrix * neg_mask.float()).sum(dim=1)  # (B,)

        # Add positive exp to denominator (standard SupCon denominator includes all non-self)
        # But we only weight negatives; positives in denominator are unweighted
        pos_exp_sum = (exp_logits * pos_mask.float()).sum(dim=1)  # (B,)
        denom = weighted_neg_sum + pos_exp_sum + 1e-8

        # Log prob for each positive pair
        log_prob = logits - torch.log(denom.unsqueeze(1) + 1e-8)

        # Mean over positive pairs per anchor
        mean_log_prob = (pos_mask.float() * log_prob).sum(1) / pos_count.clamp(min=1)
...
# ---------------------------------------------------------------------------
# Model (KSLGraphNetV45Expr2 — same arch as v43/exp5)
# ---------------------------------------------------------------------------
...
class KSLGraphNetV45Expr2(nn.Module):
...
def train_stream(stream_name, name, classes, config, train_dir, val_dir,
    ...
    """Train one stream model with hard negative SupCon."""
    ...
    print(f"[{ts()}] Hard Neg SupCon warmup: {config['hard_neg_warmup_epochs']} epochs")
    print(f"[{ts()}] {'=' * 70}")
    ...
    print(f"[{ts()}] SupCon weight: {config['supcon_weight']} (hard neg after epoch {config['hard_neg_warmup_epochs']})")
    if use_focal:
    ...
    # Losses
    std_supcon_loss_fn = SupConLoss(temperature=config["supcon_temperature"])
    hard_neg_supcon_loss_fn = HardNegSupConLoss(temperature=config["supcon_temperature"])

    # Confusion tracker
    confusion_tracker = ClassConfusionTracker(
        num_classes=num_classes,
        momentum=config["confusion_momentum"],
    )
    ...
    hard_neg_warmup = config["hard_neg_warmup_epochs"]
    ...
    for ep in range(config["epochs"]):
        ...
        # Determine if we use hard neg SupCon this epoch
        use_hard_neg = (ep >= hard_neg_warmup)
        ...
        for streams_batch, aux_data, targets, signer_targets in train_ld:
            ...
            if use_cutmix:
                ...
                # SupCon: use targets_a for contrastive labels
                if use_hard_neg:
                    cw = confusion_tracker.get_negative_weights(targets_a, device)
                    sc_loss = hard_neg_supcon_loss_fn(embedding, targets_a, cw)
                else:
                    sc_loss = std_supcon_loss_fn(embedding, targets_a)

                signer_loss = F.cross_entropy(signer_logits, signer_targets)
                loss = cls_loss + supcon_weight * sc_loss + grl_lambda * signer_loss

                # Update confusion tracker with logits
                confusion_tracker.update(logits, targets_a)
                ...
                if use_hard_neg:
                    cw = confusion_tracker.get_negative_weights(targets, device)
                    sc_loss = hard_neg_supcon_loss_fn(embedding, targets, cw)
                else:
                    sc_loss = std_supcon_loss_fn(embedding, targets)

                signer_loss = F.cross_entropy(signer_logits, signer_targets)
                loss = cls_loss + supcon_weight * sc_loss + grl_lambda * signer_loss

                confusion_tracker.update(logits, targets)
        ...
        hn_str = "HN" if use_hard_neg else "std"
        if ep < 10 or (ep + 1) % 10 == 0 or va > best:
            print(
                ...
                f"SignerAcc: {signer_acc:.1f}% | SupCon: {hn_str} | "
        ...
        if (ep + 1) % 50 == 0:
            ...
            # Log top confusions from tracker
            print(f"[{ts()}]   Top confused class pairs (confusion tracker):")
            conf = confusion_tracker.confusion.clone()
            for _ in range(min(5, num_classes)):
                max_idx = conf.argmax().item()
                ci, cj = max_idx // num_classes, max_idx % num_classes
                val = conf[ci, cj].item()
                if val < 1e-6:
                    break
                print(f"[{ts()}]     {i2c[ci]:>12s} -> {i2c[cj]:>12s}: {val:.4f}")
                conf[ci, cj] = 0
...
def main():
    parser = argparse.ArgumentParser(
        description="KSL Training V45_expr2 - Hard Negative SupCon",
    ...
    parser.add_argument("--checkpoint-dir", type=str, default=os.path.join(base_data, "checkpoints", "v45_expr2"))
    ...
    print("=" * 70)
    print(f"KSL Training V45_expr2 - Class-Aware Hard Negative SupCon")
    print(f"Started: {ts()}")
    ...
    print(f"\nV45_expr2 Data Split:")
    print(f"  Train dirs: {train_dirs}  (signers 1-12)")
    print(f"  Val dir:    {val_dir}  (signers 13-15)")
    print(f"  Streams: {V45_EXPR2_CONFIG['streams']}")
    print(f"  Batch size: {V45_EXPR2_CONFIG['batch_size']}")
    print(f"  SupCon weight: {V45_EXPR2_CONFIG['supcon_weight']}")
    print(f"  Hard neg warmup: {V45_EXPR2_CONFIG['hard_neg_warmup_epochs']} epochs")
    print(f"  Confusion momentum: {V45_EXPR2_CONFIG['confusion_momentum']}")
    print(f"  R&R: applied to NUMBERS only (not words)")

    print(f"\n[{ts()}] V45_expr2 Config:")
    print(json.dumps(V45_EXPR2_CONFIG, indent=2, default=str))
    ...
    for cat_name, classes, use_focal, apply_rr in categories:
        ...
        stream_results = {}
        for stream_name in V45_EXPR2_CONFIG["streams"]:
            result = train_stream(
                stream_name, cat_name, classes, V45_EXPR2_CONFIG,
        ...
        fusion_weights, fused_acc = learn_fusion_weights(
            cat_name, classes, V45_EXPR2_CONFIG, val_dir, ckpt_dir, device
        )
    ...
    # Summary
    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] SUMMARY V45_expr2 - Hard Negative SupCon")
    print(f"[{ts()}] {'=' * 70}")
    ...
    results_path = os.path.join(
        args.results_dir,
        f"v45_expr2_{args.model_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    )
    results_out = {
        "version": "v45_expr2",
        "model_type": args.model_type,
        "seed": args.seed,
        "config": V45_EXPR2_CONFIG,
        "results": results,
        ...
        "timestamp": ts(),
        "changes_from_v43": [
            "1. Hard Negative SupCon: weight negatives by class confusion score",
            "2. ClassConfusionTracker: running EMA of softmax confusion from training logits",
            "3. Warm-up: first 50 epochs use standard SupCon, then switch to hard neg",
            "4. Architecture identical to v43/exp5 (BatchNorm, no projection head)",
            "5. Category-conditional R&R (numbers only, same as v43)",
        ],
    }
