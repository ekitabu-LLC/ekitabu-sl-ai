#!/usr/bin/env python3
"""
KSL Training v44 Expr5 - HA-GCN: Hand-Body Sub-Graph Architecture

Hypothesis: The current ST-GCN treats all 48 joints equally, but 42/48 are hand joints
and only 6 are pose joints. The body sub-graph (shoulders, elbows, wrists) carries
coarse spatial context (hand position relative to body) while the hand sub-graphs
carry fine-grained articulation. Processing them with separate GCNs using
sub-graph-specific adjacency matrices may improve feature extraction.

Architecture:
  - HandGCN: processes LEFT_HAND (21) + RIGHT_HAND (21) = 42 hand joints
    with a hand-specific adjacency (finger kinematic chains + cross-hand wrist link)
  - BodyGCN: processes POSE (6) joints with body-specific adjacency
  - Fusion: concatenate hand + body embeddings -> MLP -> classifier

Key design choices:
  - GroupNorm throughout (proven +3.2pp over BatchNorm for cross-signer)
  - SupCon loss with weight 0.05 (proven +5pp)
  - Same training pipeline, augmentation, data handling as v31_exp1
  - Hand adjacency: finger kinematic chains with cross-finger palm connections
  - Body adjacency: chain shoulders -> elbows -> wrists
  - Inter-hand link: left wrist (joint 0 in hand subgraph) <-> right wrist (joint 21)

Usage:
    python train_ksl_v44_expr5.py --model-type numbers
    python train_ksl_v44_expr5.py --model-type words
"""
...
# ---------------------------------------------------------------------------
# Graph Topology (full 48-joint for data loading)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Sub-Graph Joint Index Groups
# ---------------------------------------------------------------------------

LEFT_HAND = list(range(0, 21))    # 21 joints (indices 0-20 in full 48-joint)
RIGHT_HAND = list(range(21, 42))  # 21 joints (indices 21-41 in full 48-joint)
HAND_JOINTS = LEFT_HAND + RIGHT_HAND  # 42 joints
POSE_JOINTS = list(range(42, 48))      # 6 joints

# ---------------------------------------------------------------------------
# Sub-Graph Adjacency Builders
# ---------------------------------------------------------------------------

def build_hand_adj(n=42):
    """Build adjacency for the 42-joint hand sub-graph.

    Joints 0-20 = left hand, 21-41 = right hand (in the sub-graph indexing).
    Each hand has: wrist -> MCP -> PIP -> DIP -> tip for each finger,
    plus cross-finger palm connections (MCP chain).
    Inter-hand: left wrist (0) <-> right wrist (21) with reduced weight.
    """
    adj = np.zeros((n, n))

    # Left hand edges (indices 0-20, same as original)
    for i, j in HAND_EDGES:
        adj[i, j] = adj[j, i] = 1

    # Right hand edges (indices 21-41, same offset as original)
    for i, j in HAND_EDGES:
        adj[i + 21, j + 21] = adj[j + 21, i + 21] = 1

    # Inter-hand wrist link (left wrist 0 <-> right wrist 21)
...
def build_body_adj(n=6):
    """Build adjacency for the 6-joint body sub-graph.

    Sub-graph indexing: 0=L_shoulder, 1=R_shoulder, 2=L_elbow, 3=R_elbow,
                        4=L_wrist, 5=R_wrist
    (Mapped from original: 42->0, 43->1, 44->2, 45->3, 46->4, 47->5)

    Connections: shoulder-shoulder, shoulder-elbow, elbow-wrist.
    Also: wrist-to-hand links (wrist 4 -> hand center, wrist 5 -> hand center)
    encoded as stronger self-connection (already via self-loop).
    """
    adj = np.zeros((n, n))
    ...
    # Cross: wrist to wrist (weak link, same as inter-hand)
    adj[4, 5] = adj[5, 4] = 0.3
...
def build_adj(n=48):
    """Build full 48-joint adjacency (for compatibility with data loading)."""
    adj = np.zeros((n, n))
    for i, j in HAND_EDGES:
        adj[i, j] = adj[j, i] = 1
    for i, j in HAND_EDGES:
...
V44_EXPR5_CONFIG = {
    ...
    "patience": 80,
    "warmup_epochs": 10,
    # SupCon (same as exp5 — proven effective)
    "supcon_weight": 0.05,
...
def temporal_cutmix(gcn_hand, gcn_body, aux_input, labels, signer_labels, alpha=1.0):
    """CutMix at temporal dimension for HA-GCN (hand + body sub-graphs)."""
    B, C, T, N_hand = gcn_hand.shape
    lam = np.random.beta(alpha, alpha)
    cut_len = int(T * (1 - lam))
    if cut_len == 0:
        return gcn_hand, gcn_body, aux_input, labels, labels, 1.0

    cut_start = np.random.randint(0, T - cut_len + 1)
    indices = torch.randperm(B, device=gcn_hand.device)

    hand_mixed = gcn_hand.clone()
    body_mixed = gcn_body.clone()
    aux_mixed = aux_input.clone()

    hand_mixed[:, :, cut_start:cut_start+cut_len, :] = gcn_hand[indices, :, cut_start:cut_start+cut_len, :]
    body_mixed[:, :, cut_start:cut_start+cut_len, :] = gcn_body[indices, :, cut_start:cut_start+cut_len, :]
    aux_mixed[:, cut_start:cut_start+cut_len, :] = aux_input[indices, cut_start:cut_start+cut_len, :]

    lam = 1.0 - cut_len / T
    labels_b = labels[indices]

    return hand_mixed, body_mixed, aux_mixed, labels, labels_b, lam
...
# ---------------------------------------------------------------------------
# Multi-Scale Temporal Convolution (GroupNorm)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# HA-GCN Model: Hand-Body Sub-Graph Architecture
# ---------------------------------------------------------------------------

class KSLGraphNetHA(nn.Module):
    """Hierarchical Attention GCN with separate hand and body sub-graphs.

    Instead of a single GCN processing all 48 joints, this model:
    1. Splits input into hand (42 joints) and body (6 joints) sub-graphs
    2. Processes each with dedicated ST-GCN stacks using sub-graph adjacency
    3. Fuses hand + body embeddings with a learned fusion layer
    4. Adds auxiliary features (angles, fingertip distances, hand-body features)
    """

    def __init__(self, nc, num_signers, aux_dim, nn_=48, ic=3, hd=64, nl=4,
                 tk=(3, 5, 7), dr=0.3, spatial_dropout=0.1,
                 hand_adj=None, body_adj=None):
        super().__init__()
        self.register_buffer("hand_adj", hand_adj)
        self.register_buffer("body_adj", body_adj)

        # Data normalization layers (GroupNorm)
        hand_bn_ch = 42 * ic  # 42 hand joints * 3 channels = 126
        body_bn_ch = 6 * ic   # 6 body joints * 3 channels = 18
        # Find valid num_groups (must divide channel count)
        num_groups_hand = 1
        for g in [8, 7, 6, 3, 2]:
            if hand_bn_ch % g == 0:
                num_groups_hand = g
        ...
        for g in [6, 3, 2]:
            if body_bn_ch % g == 0:
                num_groups_body = g
                break
        self.hand_data_bn = nn.GroupNorm(num_groups_hand, hand_bn_ch)
        self.body_data_bn = nn.GroupNorm(num_groups_body, body_bn_ch)

        # Hand GCN: 4 layers, same channel progression as baseline
        hand_ch = [ic] + [hd] * 2 + [hd * 2] * 2
        hand_ch = hand_ch[:nl + 1]
        self.hand_layers = nn.ModuleList(
            [STGCNBlock(hand_ch[i], hand_ch[i + 1], hand_adj, tk,
                        2 if i in [1, 3] else 1,
                        dr, spatial_dropout)
             for i in range(nl)]
        )

        # Body GCN: same depth but smaller hidden dim (fewer joints = less info)
        body_hd = hd // 2  # 32 for body (6 joints carry less info)
        body_ch = [ic] + [body_hd] * 2 + [body_hd * 2] * 2
        body_ch = body_ch[:nl + 1]
        self.body_layers = nn.ModuleList(
            [STGCNBlock(body_ch[i], body_ch[i + 1], body_adj, tk,
                        2 if i in [1, 3] else 1,
                        dr, spatial_dropout)
             for i in range(nl)]
        )

        # Attention pooling for each sub-graph
        hand_final_ch = hand_ch[-1]
        body_final_ch = body_ch[-1]
        self.hand_attn_pool = AttentionPool(hand_final_ch, num_heads=2)
        self.body_attn_pool = AttentionPool(body_final_ch, num_heads=2)
        hand_embed_dim = 2 * hand_final_ch  # 256
        body_embed_dim = 2 * body_final_ch  # 128

        # Fusion layer: combine hand + body embeddings
        fused_dim = hand_embed_dim + body_embed_dim  # 384
        num_groups_fuse = 8 if hd >= 8 else 1
        self.fusion = nn.Sequential(
            nn.Linear(fused_dim, hd * 2),
            nn.ReLU(),
            nn.Dropout(dr),
        )
        gcn_embed_dim = hd * 2  # 128

        # Auxiliary branch (same as baseline)
        num_groups_aux = 1
        for g in [8, 4, 2]:
            if aux_dim % g == 0:
                num_groups_aux = g
    ...
    def forward(self, gcn_input, aux_input, grl_lambda=0.0):
        """
        Args:
            gcn_input: (B, 3, T, 48) full skeleton
            aux_input: (B, T, D_aux) auxiliary features
            grl_lambda: gradient reversal lambda
        """
        b, c, t, n = gcn_input.shape

        # Split into hand and body sub-graphs
        x_hand = gcn_input[:, :, :, :42]   # (B, 3, T, 42)
        x_body = gcn_input[:, :, :, 42:]   # (B, 3, T, 6)

        # Hand data normalization
        x_hand = self.hand_data_bn(
            x_hand.permute(0, 1, 3, 2).reshape(b, c * 42, t)
        ).reshape(b, c, 42, t).permute(0, 1, 3, 2)

        # Body data normalization
        x_body = self.body_data_bn(
            x_body.permute(0, 1, 3, 2).reshape(b, c * 6, t)
        ).reshape(b, c, 6, t).permute(0, 1, 3, 2)

        # Hand ST-GCN
        for layer in self.hand_layers:
            x_hand = layer(x_hand)
        ...
        # Temporal pooling (average over nodes, then attention over time)
        hand_node_avg = x_hand.mean(dim=3)   # (B, C_hand, T')
        body_node_avg = x_body.mean(dim=3)   # (B, C_body, T')

        hand_embedding = self.hand_attn_pool(hand_node_avg)  # (B, 2*C_hand)
        body_embedding = self.body_attn_pool(body_node_avg)  # (B, 2*C_body)

        # Fuse hand + body
        gcn_fused = torch.cat([hand_embedding, body_embedding], dim=1)
        gcn_embedding = self.fusion(gcn_fused)  # (B, hd*2)
...
def train_stream(stream_name, name, classes, config, train_dir, val_dir,
    ...
    print(f"[{ts()}] Architecture: HA-GCN (Hand-Body Sub-Graphs)")
    print(f"[{ts()}] {'=' * 70}")

    hand_adj = build_hand_adj(42).to(device)
    body_adj = build_body_adj(6).to(device)
    ...
    print(f"[{ts()}] Normalization: GroupNorm (HA-GCN)")
    ...
    for ep in range(config["epochs"]):
        ...
        for streams_batch, aux_data, targets, signer_targets in train_ld:
            gcn_data = streams_batch[stream_name].to(device)  # (B, 3, T, 48)
            ...
            # Split for CutMix
            gcn_hand = gcn_data[:, :, :, :42]
            gcn_body = gcn_data[:, :, :, 42:]

            use_cutmix = np.random.random() < cutmix_prob
            use_mixup = (not use_cutmix) and (mixup_alpha > 0)

            if use_cutmix:
                hand_mixed, body_mixed, aux_mixed, targets_a, targets_b, lam = temporal_cutmix(
                    gcn_hand, gcn_body, aux_data, targets, signer_targets, alpha=cutmix_alpha,
                )
                # Reconstruct full input for model
                gcn_mixed = torch.cat([hand_mixed, body_mixed], dim=3)
...
def learn_fusion_weights(category_name, classes, config, val_dir, ckpt_dir, device):
    ...
    for sname in streams:
        ...
        model = KSLGraphNetHA(
            ...
            spatial_dropout=config.get("spatial_dropout", 0.1),
            hand_adj=hand_adj, body_adj=body_adj,
        ).to(device)
        model.load_state_dict(ckpt["model"])
        model.eval()
        models[sname] = model
...
def main():
    parser = argparse.ArgumentParser(
        description="KSL Training v44 Expr5 - HA-GCN Hand-Body Sub-Graphs",
    ...
    parser.add_argument("--checkpoint-dir", type=str,
                        default=os.path.join(base_data, "checkpoints/v44_expr5"))
    ...
    print("=" * 70)
    print(f"KSL Training v44 Expr5 - HA-GCN Hand-Body Sub-Graphs")
    print(f"Started: {ts()}")
    ...
    print(f"\nv44 Expr5 Data Split:")
    print(f"  Train dirs: {train_dirs}  (signers 1-12)")
    print(f"  Val dir:    {val_dir}  (signers 13-15)")
    print(f"  Streams: {V44_EXPR5_CONFIG['streams']}")
    print(f"  Architecture: HA-GCN (Hand + Body sub-graphs)")
    print(f"  Normalization: GroupNorm")
    print(f"  SupCon weight: {V44_EXPR5_CONFIG['supcon_weight']}")

    print(f"\n[{ts()}] v44 Expr5 Config:")
    print(json.dumps(V44_EXPR5_CONFIG, indent=2, default=str))
    ...
    for cat_name, classes, use_focal in categories:
        ...
        stream_results = {}
        for stream_name in V44_EXPR5_CONFIG["streams"]:
            result = train_stream(
                stream_name, cat_name, classes, V44_EXPR5_CONFIG,
        ...
        fusion_weights, fused_acc = learn_fusion_weights(
            cat_name, classes, V44_EXPR5_CONFIG, val_dir, ckpt_dir, device
        )
    ...
    # Summary
    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] SUMMARY v44 Expr5 - HA-GCN Hand-Body Sub-Graphs")
    print(f"[{ts()}] {'=' * 70}")
    ...
    results_path = os.path.join(
        args.results_dir,
        f"v44_expr5_{args.model_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    )
    results_out = {
        "version": "v44_expr5",
        "model_type": args.model_type,
        "seed": args.seed,
        "config": V44_EXPR5_CONFIG,
        "results": results,
        "total_time_seconds": total_time,
        "device": str(device),
        "gpu": torch.cuda.get_device_name(0) if torch.cuda.is_available() else "cpu",
        "timestamp": ts(),
        "architecture": "HA-GCN (Hand-Body Sub-Graph)",
        "changes_from_v31_exp1": [
            "1. HAND-BODY SUB-GRAPHS: Separate GCN stacks for hand (42j) and body (6j)",
            "2. HAND-SPECIFIC ADJACENCY: Finger kinematic chains + cross-hand wrist link",
            "3. BODY-SPECIFIC ADJACENCY: Shoulder-elbow-wrist chain + shoulder link",
            "4. FUSION: Concatenate hand+body embeddings -> MLP -> classifier",
            "5. GroupNorm throughout (same as exp1)",
            "6. SupCon weight=0.05 (same as exp5)",
            "7. Body GCN uses half hidden dim (32 vs 64) — fewer joints = less capacity",
        ],
    }
