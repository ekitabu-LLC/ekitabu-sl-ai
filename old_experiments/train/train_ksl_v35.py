#!/usr/bin/env python3
"""
KSL Training v35 (Alpine) - GroupNorm + EMA + RSC

Based on v31_exp1 (GroupNorm) with two additions:
  1. EMA (Exponential Moving Average): Maintain running average of model weights
     for more stable inference. EMA decay=0.999, applied after each training step.
  2. RSC (Representation Self-Challenging): During training, mask the top-p% of
     feature activations (by gradient magnitude) to force the model to use less
     dominant, more generalizable features. p=33%, applied at embedding level.

Both techniques are compatible with GroupNorm and address domain generalization.

Usage:
    python train_ksl_v35.py --model-type numbers
    python train_ksl_v35.py --model-type words
"""
...
V35_CONFIG = {
    ...
    "ema_decay": 0.999,
    # RSC
    "rsc_percentile": 0.667,  # Keep top 66.7% of features (mask 33.3%)
    "rsc_start_epoch": 20,    # Don't apply RSC during warmup
}
...
def train_stream(stream_name, name, classes, config, train_dir, val_dir,
    ...
    print(f"[{ts()}] Normalization: GroupNorm (v31 exp1)")
    if use_focal:
    ...
    print(f"[{ts()}] EMA model initialized (decay={ema_decay})")

    # --- RSC config ---
    rsc_percentile = config.get("rsc_percentile", 0.667)
    rsc_start_epoch = config.get("rsc_start_epoch", 20)
    print(f"[{ts()}] RSC: percentile={rsc_percentile}, start_epoch={rsc_start_epoch}")
    ...
    for ep in range(config["epochs"]):
        ...
        for streams_batch, aux_data, targets, signer_targets in train_ld:
            ...
            if use_cutmix:
                ...
                use_rsc = ep >= rsc_start_epoch and np.random.random() < 0.5
                if use_rsc:
                    # Step 1: Forward pass to get embedding with gradients
                    logits_init, _, embedding_init = model(
                        gcn_data, aux_data, grl_lambda=grl_lambda,
                    )
                    cls_loss_init = F.cross_entropy(logits_init, targets, label_smoothing=config["label_smoothing"])
                    # Get gradient of loss w.r.t. embedding
                    grad_emb = torch.autograd.grad(cls_loss_init, embedding_init,
                                                    retain_graph=False, create_graph=False)[0]
                    # Step 2: Mask top features (by gradient magnitude)
                    grad_mag = grad_emb.abs()
                    threshold = torch.quantile(grad_mag, rsc_percentile, dim=1, keepdim=True)
                    mask = (grad_mag < threshold).float()
                    # Step 3: Re-forward with masked embedding
                    logits, signer_logits, embedding = model(
                        gcn_data, aux_data, grl_lambda=grl_lambda,
                    )
                    # Apply mask to embedding before re-classifying
                    embedding_masked = embedding * mask.detach()
                    logits = model.classifier(embedding_masked)
                    if cls_loss_fn is not None:
        ...
        # Save best EMA model
        ema_best_path = os.path.join(stream_ckpt_dir, "best_ema_model.pt")
        if va_ema > best_ema:
            best_ema = va_ema
            torch.save(
                {
            ...
            print(f"[{ts()}]   -> New best EMA! {va_ema:.1f}%")
...
def main():
    parser = argparse.ArgumentParser(
        description="KSL ST-GCN Training v35 - GroupNorm + EMA + RSC",
    ...
    parser.add_argument("--checkpoint-dir", type=str, default=os.path.join(base_data, "checkpoints/v35"))
    ...
    print("=" * 70)
    print(f"KSL Training v35 - GroupNorm + EMA + RSC")
    print(f"Started: {ts()}")
    ...
    print(f"  Streams: {V35_CONFIG['streams']}")
    print(f"  In channels per stream: {V35_CONFIG['in_channels']}")
    print(f"  Normalization: GroupNorm (replacing BatchNorm)")

    print(f"\n[{ts()}] v31 Exp1 Config:")
    print(json.dumps(V35_CONFIG, indent=2, default=str))
    ...
    for cat_name, classes, use_focal in categories:
        ...
        # Train each stream
        stream_results = {}
        for stream_name in V35_CONFIG["streams"]:
            result = train_stream(
                stream_name, cat_name, classes, V35_CONFIG,
        ...
        fusion_weights, fused_acc = learn_fusion_weights(
            cat_name, classes, V35_CONFIG, val_dir, ckpt_dir, device
        )
    ...
    # Summary
    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] SUMMARY v35 - GroupNorm + EMA + RSC")
    print(f"[{ts()}] {'=' * 70}")
    ...
    results_path = os.path.join(
        args.results_dir,
        f"v31_exp1_{args.model_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    )
    results_out = {
        "version": "v31_exp1",
        ...
        "timestamp": ts(),
        "changes_from_v28": [
            "1. GROUPNORM: Replaced ALL BatchNorm1d and BatchNorm2d with GroupNorm.",
            "2. KEPT signer-adversarial head for domain invariance during training.",
            "3. Same multi-stream late fusion architecture as v28.",
        ],
    }
