#!/usr/bin/env python3
"""
KSL Training v18 (Alpine) - ST-GCN Bug Fixes + Velocity Features

Changes from v17 (identified in pipeline review 2026-02-09):
- FIXED: Separate normalization for hands vs pose (hands were squashed 5x)
- FIXED: SWA swa_start_frac 0.8->0.4 (SWA never activated for numbers!)
- FIXED: SWA validates with BASE model for early stopping (stale BN fix)
- FIXED: Anti-attractor re-added for numbers (class 444: 45%->10% regression)
- FIXED: No class weighting / WeightedRandomSampler (data balanced at 25/class)
- FIXED: Numbers augmentation moderated (was 3x too aggressive)
- FIXED: weight_decay equalized (numbers 0.003->0.001 to match words)
- FIXED: label_smoothing reduced for numbers (0.15->0.1)
- FIXED: patience increased (numbers 60->80, words 40->50)
- FIXED: Hand swap also flips pose X coordinates
- FIXED: Temporal shift uses pad+slice instead of np.roll (no wrap artifacts)
- ADDED: Velocity features (in_channels 3->6: xyz + vxyz)
- ADDED: AttentionPool replaces AdaptiveAvgPool (weighted temporal pooling)
- ADDED: Confusion matrix logging for numbers
- ADDED: --seed for reproducibility

Usage:
    python train_ksl_v18.py --model-type numbers
    python train_ksl_v18.py --model-type words
    python train_ksl_v18.py --model-type both --seed 42
"""

import argparse
import json
import math
import os
import random
import time
from collections import Counter
from datetime import datetime

import numpy as np
import torch
import torch.nn as nn
import torch.nn.functional as F
from torch.optim.swa_utils import AveragedModel, SWALR
from torch.utils.data import Dataset, DataLoader

# ---------------------------------------------------------------------------
# Helpers
# ---------------------------------------------------------------------------

def ts():
    return datetime.now().strftime("%Y-%m-%d %H:%M:%S")


def set_seed(seed):
    random.seed(seed)
    np.random.seed(seed)
    torch.manual_seed(seed)
    torch.cuda.manual_seed_all(seed)
    torch.backends.cudnn.deterministic = True
    torch.backends.cudnn.benchmark = False


# ---------------------------------------------------------------------------
# Class Definitions
# ---------------------------------------------------------------------------

NUMBER_CLASSES = sorted([
    "9", "17", "22", "35", "48", "54", "66", "73", "89", "91",
    "100", "125", "268", "388", "444",
])

WORD_CLASSES = sorted([
    "Agreement", "Apple", "Colour", "Friend", "Gift", "Market", "Monday",
    "Picture", "Proud", "Sweater", "Teach", "Tomatoes", "Tortoise", "Twin", "Ugali",
])

# Known confusion pairs for numbers (from v16, used for logging)
NUMBERS_CONFUSION_PAIRS = [
    ("444", "54", 3.5),
    ("35", "54", 3.0),
    ("89", "9", 3.0),
    ("73", "91", 2.5),
    ("35", "100", 2.0),
    ("100", "54", 2.0),
    ("125", "54", 2.0),
]

# ---------------------------------------------------------------------------
# Graph Topology
# ---------------------------------------------------------------------------

HAND_EDGES = [
    (0, 1), (0, 5), (0, 9), (0, 13), (0, 17),
    (1, 2), (2, 3), (3, 4), (5, 6), (6, 7), (7, 8),
    (9, 10), (10, 11), (11, 12), (13, 14), (14, 15), (15, 16),
    (17, 18), (18, 19), (19, 20), (5, 9), (9, 13), (13, 17),
]

POSE_EDGES = [
    (42, 43), (42, 44), (43, 45), (44, 46), (45, 47), (46, 0), (47, 21),
]

POSE_INDICES = [11, 12, 13, 14, 15, 16]

# ---------------------------------------------------------------------------
# Configs (v18 - FIXED)
# ---------------------------------------------------------------------------

NUMBERS_CONFIG = {
    "max_frames": 90,
    "num_nodes": 48,
    "in_channels": 6,         # FIXED: xyz + velocity (was 3)
    "hidden_dim": 128,
    "num_layers": 6,
    "temporal_kernel": 9,
    "batch_size": 32,
    "epochs": 300,
    "learning_rate": 5e-4,
    "min_lr": 1e-6,
    "weight_decay": 0.001,    # FIXED: equalized with words (was 0.003)
    "dropout": 0.4,
    "label_smoothing": 0.1,   # FIXED: reduced (was 0.15)
    "patience": 80,           # FIXED: increased (was 60)
    "warmup_epochs": 10,
    # Augmentation (FIXED: moderated for numbers)
    "hand_dropout_prob": 0.3,        # FIXED: was 0.5
    "hand_dropout_min": 0.1,         # FIXED: was 0.3
    "hand_dropout_max": 0.4,         # FIXED: was 0.7
    "complete_hand_drop_prob": 0.05,  # FIXED: was 0.1
    "noise_std": 0.01,               # FIXED: was 0.015
    "noise_prob": 0.3,               # FIXED: was 0.4
    # Anti-attractor (RE-ADDED from v16)
    "anti_attractor_weight": 0.15,
    "anti_attractor_class": "54",
    # SWA (FIXED)
    "swa_start_frac": 0.4,   # FIXED: was 0.8 (never activated!)
    "swa_lr": 1e-4,
    # No hard class boosting (data is balanced at 25/class)
    "use_hard_class_boosting": False,
}

WORDS_CONFIG = {
    "max_frames": 90,
    "num_nodes": 48,
    "in_channels": 6,         # FIXED: xyz + velocity (was 3)
    "hidden_dim": 128,
    "num_layers": 6,
    "temporal_kernel": 9,
    "batch_size": 32,
    "epochs": 200,
    "learning_rate": 5e-4,
    "min_lr": 1e-6,
    "weight_decay": 0.001,
    "dropout": 0.4,
    "label_smoothing": 0.15,
    "patience": 50,           # FIXED: increased (was 40)
    "warmup_epochs": 10,
    "hand_dropout_prob": 0.5,
    "hand_dropout_min": 0.3,
    "hand_dropout_max": 0.7,
    "complete_hand_drop_prob": 0.1,
    "noise_std": 0.015,
    "noise_prob": 0.4,
    # No anti-attractor for words
    "anti_attractor_weight": 0.0,
    "anti_attractor_class": None,
    # SWA (FIXED)
    "swa_start_frac": 0.4,   # FIXED: was 0.8
    "swa_lr": 1e-4,
    "use_hard_class_boosting": False,
}

# ---------------------------------------------------------------------------
# Adjacency Matrix Builder
# ---------------------------------------------------------------------------

def build_adj(n=48):
    adj = np.zeros((n, n))
    for i, j in HAND_EDGES:
        adj[i, j] = adj[j, i] = 1
    for i, j in HAND_EDGES:
        adj[i + 21, j + 21] = adj[j + 21, i + 21] = 1
    for i, j in POSE_EDGES:
        adj[i, j] = adj[j, i] = 1
    adj[0, 21] = adj[21, 0] = 0.3
    adj += np.eye(n)
    d = np.sum(adj, axis=1)
    d_inv = np.power(d, -0.5)
    d_inv[np.isinf(d_inv)] = 0
    return torch.FloatTensor(np.diag(d_inv) @ adj @ np.diag(d_inv))

# ---------------------------------------------------------------------------
# Dataset (FIXED normalization, velocity features, augmentation)
# ---------------------------------------------------------------------------

class KSLGraphDataset(Dataset):
    def __init__(self, data_dir, classes, config, aug=False):
        self.samples = []
        self.labels = []
        self.mf = config["max_frames"]
        self.aug = aug
        self.config = config
        self.c2i = {c: i for i, c in enumerate(classes)}

        for cn in classes:
            cd = os.path.join(data_dir, cn)
            if os.path.exists(cd):
                for fn in os.listdir(cd):
                    if fn.endswith(".npy"):
                        self.samples.append(os.path.join(cd, fn))
                        self.labels.append(self.c2i[cn])
        print(f"[{ts()}]   Loaded {len(self.samples)} samples from {data_dir}")

    def __len__(self):
        return len(self.samples)

    def __getitem__(self, idx):
        d = np.load(self.samples[idx])
        f = d.shape[0]

        # Extract landmarks
        if d.shape[1] >= 225:
            pose = np.zeros((f, 6, 3), dtype=np.float32)
            for pi, idx_pose in enumerate(POSE_INDICES):
                start = idx_pose * 3
                pose[:, pi, :] = d[:, start:start + 3]
            lh = d[:, 99:162].reshape(f, 21, 3)
            rh = d[:, 162:225].reshape(f, 21, 3)
        else:
            pose = np.zeros((f, 6, 3), dtype=np.float32)
            lh = np.zeros((f, 21, 3), dtype=np.float32)
            rh = np.zeros((f, 21, 3), dtype=np.float32)

        h = np.concatenate([lh, rh, pose], axis=1).astype(np.float32)
        # h shape: (f, 48, 3)

        # --- Augmentation Step 1: Hand dropout ---
        if self.aug and np.random.random() < self.config["hand_dropout_prob"]:
            dropout_rate = np.random.uniform(
                self.config["hand_dropout_min"], self.config["hand_dropout_max"]
            )
            if np.random.random() < 0.6:
                lh_mask = np.random.random(f) > dropout_rate
                h[~lh_mask, :21, :] = 0
            if np.random.random() < 0.6:
                rh_mask = np.random.random(f) > dropout_rate
                h[~rh_mask, 21:42, :] = 0

        # --- Augmentation Step 2: Complete hand drop ---
        if self.aug and np.random.random() < self.config["complete_hand_drop_prob"]:
            if np.random.random() < 0.5:
                h[:, :21, :] = 0
            else:
                h[:, 21:42, :] = 0

        # --- Augmentation Step 3: Hand swap (FIXED: also flips pose X) ---
        if self.aug and np.random.random() < 0.3:
            h_copy = h.copy()
            h[:, :21, :] = h_copy[:, 21:42, :]
            h[:, 21:42, :] = h_copy[:, :21, :]
            h[:, :42, 0] = -h[:, :42, 0]
            h[:, 42:48, 0] = -h[:, 42:48, 0]  # FIX: flip pose X too

        # --- Step 4: Centering ---
        lh_valid = np.abs(h[:, :21, :]).sum(axis=(1, 2)) > 0.01
        rh_valid = np.abs(h[:, 21:42, :]).sum(axis=(1, 2)) > 0.01

        if np.any(lh_valid):
            h[lh_valid, :21, :] -= h[lh_valid, 0:1, :]
        if np.any(rh_valid):
            h[rh_valid, 21:42, :] -= h[rh_valid, 21:22, :]

        mid_shoulder = (h[:, 42:43, :] + h[:, 43:44, :]) / 2
        h[:, 42:48, :] -= mid_shoulder

        # --- Step 5: FIXED Separate normalization (hands vs pose) ---
        hand_data = h[:, :42, :]
        pose_data = h[:, 42:48, :]
        hand_max = np.abs(hand_data).max()
        pose_max = np.abs(pose_data).max()
        if hand_max > 0.01:
            h[:, :42, :] = hand_data / hand_max
        if pose_max > 0.01:
            h[:, 42:48, :] = pose_data / pose_max
        h = np.clip(h, -1, 1).astype(np.float32)

        # --- Augmentation Step 6: Scale jitter ---
        if self.aug and np.random.random() < 0.5:
            h = h * np.random.uniform(0.8, 1.2)

        # --- Augmentation Step 7: Gaussian noise (XYZ only) ---
        if self.aug and np.random.random() < self.config["noise_prob"]:
            noise = np.random.normal(0, self.config["noise_std"], h.shape).astype(np.float32)
            h = h + noise

        # --- Step 8: Temporal sampling / padding ---
        if f >= self.mf:
            indices = np.linspace(0, f - 1, self.mf, dtype=int)
            h = h[indices]
        else:
            h = np.concatenate([h, np.zeros((self.mf - f, 48, 3), dtype=np.float32)])

        # --- Augmentation Step 9: FIXED Temporal shift (pad+slice, no roll) ---
        if self.aug and np.random.random() < 0.3:
            shift = np.random.randint(-8, 9)
            if shift > 0:
                h = np.concatenate(
                    [np.zeros((shift, 48, 3), dtype=np.float32), h[:-shift]], axis=0
                )
            elif shift < 0:
                h = np.concatenate(
                    [h[-shift:], np.zeros((-shift, 48, 3), dtype=np.float32)], axis=0
                )

        # --- Step 10: Compute velocity features ---
        velocity = np.zeros_like(h)  # (max_frames, 48, 3)
        velocity[1:] = h[1:] - h[:-1]
        h = np.concatenate([h, velocity], axis=2)  # (max_frames, 48, 6)

        # Return (6, max_frames, 48) = (C, T, N)
        return torch.FloatTensor(h).permute(2, 0, 1), self.labels[idx]

# ---------------------------------------------------------------------------
# Attention Pooling (replaces AdaptiveAvgPool)
# ---------------------------------------------------------------------------

class AttentionPool(nn.Module):
    """Multi-head attention pooling over temporal dimension."""

    def __init__(self, in_channels, num_heads=2):
        super().__init__()
        self.num_heads = num_heads
        mid = in_channels // 2

        self.heads = nn.ModuleList([
            nn.Sequential(
                nn.Linear(in_channels, mid),
                nn.Tanh(),
                nn.Linear(mid, 1),
            )
            for _ in range(num_heads)
        ])

    def forward(self, x):
        # x: (B, C, T) after averaging over node dimension
        b, c, t = x.shape
        x_t = x.permute(0, 2, 1)  # (B, T, C)

        outs = []
        for head in self.heads:
            attn_logits = head(x_t)                        # (B, T, 1)
            attn_weights = F.softmax(attn_logits, dim=1)   # (B, T, 1)
            pooled = (x_t * attn_weights).sum(dim=1)       # (B, C)
            outs.append(pooled)

        return torch.cat(outs, dim=1)  # (B, num_heads * C)

# ---------------------------------------------------------------------------
# Model Architecture (ST-GCN + AttentionPool + Anti-Attractor)
# ---------------------------------------------------------------------------

class GConv(nn.Module):
    def __init__(self, i, o):
        super().__init__()
        self.fc = nn.Linear(i, o)

    def forward(self, x, adj):
        return self.fc(torch.matmul(adj, x))


class STGCNBlock(nn.Module):
    def __init__(self, ic, oc, adj, ks=9, st=1, dr=0.3):
        super().__init__()
        self.register_buffer("adj", adj)
        self.gcn = GConv(ic, oc)
        self.bn1 = nn.BatchNorm2d(oc)
        self.tcn = nn.Sequential(
            nn.Conv2d(oc, oc, (ks, 1), padding=(ks // 2, 0), stride=(st, 1)),
            nn.BatchNorm2d(oc),
        )
        self.residual = (
            nn.Sequential(nn.Conv2d(ic, oc, 1, stride=(st, 1)), nn.BatchNorm2d(oc))
            if ic != oc or st != 1
            else nn.Identity()
        )
        self.dropout = nn.Dropout(dr)

    def forward(self, x):
        r = self.residual(x)
        b, c, t, n = x.shape
        x = self.gcn(x.permute(0, 2, 3, 1).reshape(b * t, n, c), self.adj)
        x = self.dropout(
            self.tcn(torch.relu(self.bn1(x.reshape(b, t, n, -1).permute(0, 3, 1, 2))))
        )
        return torch.relu(x + r)


class KSLGraphNet(nn.Module):
    def __init__(self, nc, nn_=48, ic=6, hd=128, nl=6, tk=9, dr=0.3, adj=None,
                 use_anti_attractor=False):
        super().__init__()
        self.register_buffer("adj", adj)
        self.data_bn = nn.BatchNorm1d(nn_ * ic)
        self.use_anti_attractor = use_anti_attractor

        ch = [ic] + [hd] * 2 + [hd * 2] * 2 + [hd * 4] * 2
        ch = ch[: nl + 1]

        self.layers = nn.ModuleList(
            [STGCNBlock(ch[i], ch[i + 1], adj, tk, 2 if i in [2, 4] else 1, dr)
             for i in range(nl)]
        )

        final_ch = ch[-1]

        # Attention pooling: average over nodes, then attend over time
        self.attn_pool = AttentionPool(final_ch, num_heads=2)

        # Classifier takes 2 * final_ch (from 2 attention heads)
        self.classifier = nn.Sequential(
            nn.Linear(2 * final_ch, hd),
            nn.ReLU(),
            nn.Dropout(dr),
            nn.Linear(hd, nc),
        )

        # Optional anti-attractor head
        if use_anti_attractor:
            self.anti_attractor = nn.Sequential(
                nn.Linear(2 * final_ch, hd // 2),
                nn.GELU(),
                nn.Linear(hd // 2, 1),
            )

    def forward(self, x, return_features=False):
        b, c, t, n = x.shape
        x = self.data_bn(
            x.permute(0, 1, 3, 2).reshape(b, c * n, t)
        ).reshape(b, c, n, t).permute(0, 1, 3, 2)

        for layer in self.layers:
            x = layer(x)

        # x: (B, C_final, T', N)
        # Average over node dimension first
        x_node_avg = x.mean(dim=3)  # (B, C_final, T')

        # Attention pooling over temporal dimension
        features = self.attn_pool(x_node_avg)  # (B, 2 * C_final)

        logits = self.classifier(features)

        if return_features:
            anti_score = None
            if self.use_anti_attractor:
                anti_score = self.anti_attractor(features)
            return logits, features, anti_score

        return logits

# ---------------------------------------------------------------------------
# Training Function
# ---------------------------------------------------------------------------

def train_split(name, classes, config, train_dir, val_dir, ckpt_dir, device,
                writer=None):
    """Train one split (numbers or words) with v18 fixes."""

    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] Training {name} ({len(classes)} classes) - v18")
    print(f"[{ts()}] {'=' * 70}")

    adj = build_adj(config["num_nodes"]).to(device)

    train_ds = KSLGraphDataset(train_dir, classes, config, aug=True)
    val_ds = KSLGraphDataset(val_dir, classes, config, aug=False)

    if len(train_ds) == 0:
        print(f"[{ts()}] ERROR: No training samples found in {train_dir}")
        return None

    c2i = train_ds.c2i
    i2c = {v: k for k, v in c2i.items()}

    # FIXED: No class weighting -- data is balanced at 25 samples/class.
    train_ld = DataLoader(
        train_ds,
        batch_size=config["batch_size"],
        shuffle=True,
        num_workers=2,
        pin_memory=True,
        drop_last=False,
    )
    val_ld = DataLoader(
        val_ds,
        batch_size=config["batch_size"],
        shuffle=False,
        num_workers=2,
        pin_memory=True,
    )

    # Model
    use_anti_attractor = config.get("anti_attractor_weight", 0) > 0
    model = KSLGraphNet(
        len(classes),
        config["num_nodes"],
        config["in_channels"],
        config["hidden_dim"],
        config["num_layers"],
        config["temporal_kernel"],
        config["dropout"],
        adj,
        use_anti_attractor=use_anti_attractor,
    ).to(device)

    param_count = sum(p.numel() for p in model.parameters())
    print(f"[{ts()}] Parameters: {param_count:,}")
    print(f"[{ts()}] Config: hidden_dim={config['hidden_dim']}, layers={config['num_layers']}, "
          f"dropout={config['dropout']}, wd={config['weight_decay']}, "
          f"ls={config['label_smoothing']}, in_ch={config['in_channels']}")
    print(f"[{ts()}] Anti-attractor: {use_anti_attractor}")
    print(f"[{ts()}] Class weighting: DISABLED (balanced data)")

    # Anti-attractor setup (numbers only)
    anti_attractor_w = config.get("anti_attractor_weight", 0)
    attractor_class = config.get("anti_attractor_class", None)
    attractor_idx = c2i.get(attractor_class, -1) if attractor_class else -1
    anti_attractor_criterion = None
    if anti_attractor_w > 0 and attractor_idx >= 0:
        anti_attractor_criterion = nn.BCEWithLogitsLoss()
        print(f"[{ts()}] Anti-attractor targeting class '{attractor_class}' "
              f"(idx={attractor_idx}, weight={anti_attractor_w})")

    # Optimizer + Cosine schedule with warmup
    opt = torch.optim.AdamW(
        model.parameters(), lr=config["learning_rate"], weight_decay=config["weight_decay"]
    )

    def sched_fn(ep):
        if ep < config["warmup_epochs"]:
            return ep / config["warmup_epochs"]
        p = (ep - config["warmup_epochs"]) / (config["epochs"] - config["warmup_epochs"])
        return config["min_lr"] / config["learning_rate"] + (
            1 - config["min_lr"] / config["learning_rate"]
        ) * 0.5 * (1 + math.cos(math.pi * p))

    scheduler = torch.optim.lr_scheduler.LambdaLR(opt, sched_fn)
    os.makedirs(ckpt_dir, exist_ok=True)
    best_path = os.path.join(ckpt_dir, "best_model.pt")

    # SWA setup (FIXED: swa_start_frac=0.4 so it actually activates)
    swa_start = int(config["epochs"] * config["swa_start_frac"])
    swa_model = AveragedModel(model)
    swa_scheduler = SWALR(opt, swa_lr=config["swa_lr"])
    swa_active = False
    print(f"[{ts()}] SWA starts at epoch {swa_start} (frac={config['swa_start_frac']})")

    best, patience_counter = 0.0, 0
    use_aux = anti_attractor_criterion is not None

    for ep in range(config["epochs"]):
        ep_start = time.time()
        model.train()
        tl, tc, tt = 0.0, 0, 0

        for d, t in train_ld:
            d, t = d.to(device), t.to(device)
            opt.zero_grad()

            if use_aux:
                logits, features, anti_score = model(d, return_features=True)
            else:
                logits = model(d)

            # FIXED: Simple CE with label smoothing, NO class weights
            loss = F.cross_entropy(logits, t, label_smoothing=config["label_smoothing"])

            # Anti-attractor loss (numbers only, re-added from v16)
            if anti_attractor_criterion is not None and use_aux:
                anti_targets = (t != attractor_idx).float().unsqueeze(1)
                anti_loss = anti_attractor_criterion(anti_score, anti_targets)
                loss = loss + anti_attractor_w * anti_loss

            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            opt.step()

            tl += loss.item()
            tt += t.size(0)
            _, p = logits.max(1)
            tc += p.eq(t).sum().item()

        # Schedule update + SWA
        if ep >= swa_start:
            if not swa_active:
                swa_active = True
                print(f"[{ts()}] SWA activated at epoch {ep + 1}")
            swa_model.update_parameters(model)
            swa_scheduler.step()
        else:
            scheduler.step()

        # CRITICAL FIX: Always validate with BASE model for early stopping.
        # SWA model has stale BN stats -> looks worse -> triggers early stop.
        model.eval()
        vc, vt = 0, 0
        with torch.no_grad():
            for d, t in val_ld:
                d, t = d.to(device), t.to(device)
                _, p = model(d).max(1)
                vt += t.size(0)
                vc += p.eq(t).sum().item()

        va = 100.0 * vc / vt
        ta = 100.0 * tc / tt
        ep_time = time.time() - ep_start
        lr_now = opt.param_groups[0]["lr"]

        swa_str = ""
        # Periodically evaluate SWA with updated BN (every 10 SWA epochs, logged only)
        if swa_active and (ep - swa_start) % 10 == 0 and ep > swa_start:
            torch.optim.swa_utils.update_bn(train_ld, swa_model, device=device)
            swa_model.eval()
            svc, svt = 0, 0
            with torch.no_grad():
                for d, t in val_ld:
                    d, t = d.to(device), t.to(device)
                    _, p = swa_model(d).max(1)
                    svt += t.size(0)
                    svc += p.eq(t).sum().item()
            swa_va = 100.0 * svc / svt
            swa_str = f" | SWA val: {swa_va:.1f}%"

        print(
            f"[{ts()}] Ep {ep + 1:3d}/{config['epochs']} | "
            f"Loss: {tl / len(train_ld):.4f} | Train: {ta:.1f}% | Val: {va:.1f}% | "
            f"LR: {lr_now:.2e} | Time: {ep_time:.1f}s"
            + (" [SWA]" if swa_active else "")
            + swa_str
        )

        # TensorBoard logging
        if writer is not None:
            writer.add_scalar(f"{name}/train_loss", tl / len(train_ld), ep)
            writer.add_scalar(f"{name}/train_acc", ta, ep)
            writer.add_scalar(f"{name}/val_acc", va, ep)
            writer.add_scalar(f"{name}/lr", lr_now, ep)

        # Early stopping based on BASE model only
        if va > best:
            best, patience_counter = va, 0
            torch.save(
                {
                    "model": model.state_dict(),
                    "val_acc": va,
                    "epoch": ep + 1,
                    "classes": classes,
                    "num_nodes": config["num_nodes"],
                    "version": "v18",
                    "config": config,
                    "swa": False,
                },
                best_path,
            )
            print(f"[{ts()}]   -> New best (base model)! {va:.1f}%")
        else:
            patience_counter += 1

        if patience_counter >= config["patience"]:
            print(f"[{ts()}] Early stopping at epoch {ep + 1}")
            break

    # After training: Final SWA evaluation with proper BN update
    if swa_active:
        print(f"\n[{ts()}] Updating SWA batch norm statistics (final)...")
        torch.optim.swa_utils.update_bn(train_ld, swa_model, device=device)
        swa_model.eval()

        swa_preds, swa_tgts = [], []
        with torch.no_grad():
            for d, t in val_ld:
                d, t = d.to(device), t.to(device)
                _, p = swa_model(d).max(1)
                swa_preds.extend(p.cpu().numpy())
                swa_tgts.extend(t.cpu().numpy())

        swa_correct = sum(1 for t, p in zip(swa_tgts, swa_preds) if t == p)
        swa_final_acc = 100.0 * swa_correct / len(swa_tgts)
        print(f"[{ts()}] SWA final accuracy (BN updated): {swa_final_acc:.1f}% (base best: {best:.1f}%)")

        # Per-class SWA results
        print(f"[{ts()}] SWA Per-Class:")
        for i in range(len(classes)):
            cn = i2c[i]
            tot_cls = sum(1 for t in swa_tgts if t == i)
            cor = sum(1 for t, p in zip(swa_tgts, swa_preds) if t == i and t == p)
            acc = 100.0 * cor / tot_cls if tot_cls > 0 else 0.0
            print(f"[{ts()}]   {cn:12s}: {acc:5.1f}% ({cor}/{tot_cls})")

        if swa_final_acc > best:
            best = swa_final_acc
            torch.save(
                {
                    "model": swa_model.module.state_dict(),
                    "val_acc": swa_final_acc,
                    "epoch": ep + 1,
                    "classes": classes,
                    "num_nodes": config["num_nodes"],
                    "version": "v18",
                    "config": config,
                    "swa": True,
                    "swa_bn_updated": True,
                },
                best_path,
            )
            print(f"[{ts()}]   -> SWA+BN model is new best! {swa_final_acc:.1f}%")
        else:
            print(f"[{ts()}]   -> Base model still better ({best:.1f}% vs SWA {swa_final_acc:.1f}%)")

    # Final evaluation with per-class breakdown
    ckpt = torch.load(best_path, map_location=device)
    model.load_state_dict(ckpt["model"])
    model.eval()

    preds, tgts = [], []
    with torch.no_grad():
        for d, t in val_ld:
            _, p = model(d.to(device)).max(1)
            preds.extend(p.cpu().numpy())
            tgts.extend(t.cpu().numpy())

    print(f"\n[{ts()}] {name} Per-Class Results (final):")
    res = {}
    for i in range(len(classes)):
        cn = i2c[i]
        tot_cls = sum(1 for t in tgts if t == i)
        cor = sum(1 for t, p in zip(tgts, preds) if t == i and t == p)
        res[cn] = 100.0 * cor / tot_cls if tot_cls > 0 else 0.0
        print(f"[{ts()}]   {cn:12s}: {res[cn]:5.1f}% ({cor}/{tot_cls})")

    ov = 100.0 * sum(1 for t, p in zip(tgts, preds) if t == p) / len(tgts)
    print(f"[{ts()}] {name} Overall: {ov:.1f}%")

    # Confusion matrix for numbers
    if name == "Numbers":
        print(f"\n[{ts()}] {name} Confusion Matrix:")
        nc = len(classes)
        cm = [[0] * nc for _ in range(nc)]
        for t, p in zip(tgts, preds):
            cm[t][p] += 1

        hdr = "True\\Pred  " + " ".join(f"{i2c[j]:>5s}" for j in range(nc))
        print(f"[{ts()}] {hdr}")
        for i in range(nc):
            row_str = f"{i2c[i]:>9s}  " + " ".join(f"{cm[i][j]:5d}" for j in range(nc))
            print(f"[{ts()}] {row_str}")

        print(f"\n[{ts()}] Known Confusion Pairs:")
        for true_name, pred_name, penalty in NUMBERS_CONFUSION_PAIRS:
            ti = c2i.get(true_name, -1)
            pi = c2i.get(pred_name, -1)
            if ti >= 0 and pi >= 0:
                tot_cls = sum(1 for t in tgts if t == ti)
                confused = cm[ti][pi]
                rate = 100.0 * confused / tot_cls if tot_cls > 0 else 0.0
                print(f"[{ts()}]   {true_name:>5s} -> {pred_name:<5s}: "
                      f"{confused}/{tot_cls} ({rate:.1f}%) [penalty={penalty}]")

    return {
        "overall": ov,
        "per_class": res,
        "best_epoch": ckpt["epoch"],
        "params": param_count,
        "swa_used": ckpt.get("swa", False),
    }

# ---------------------------------------------------------------------------
# Main
# ---------------------------------------------------------------------------

def main():
    parser = argparse.ArgumentParser(
        description="KSL ST-GCN Training v18 (Alpine HPC)",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument(
        "--model-type", type=str, default="both",
        choices=["numbers", "words", "both"],
        help="Which model(s) to train",
    )
    parser.add_argument("--seed", type=int, default=42, help="Random seed")

    base_data = "/scratch/alpine/hama5612/ksl-dir-2/data"
    parser.add_argument("--train-dir", type=str, default=os.path.join(base_data, "train_v2"))
    parser.add_argument("--val-dir", type=str, default=os.path.join(base_data, "val_v2"))
    parser.add_argument("--checkpoint-dir", type=str, default=os.path.join(base_data, "checkpoints"))
    parser.add_argument("--results-dir", type=str, default=os.path.join(base_data, "results"))

    # TensorBoard
    parser.add_argument("--tensorboard", action="store_true", help="Enable TensorBoard logging")
    parser.add_argument("--tb-logdir", type=str, default=os.path.join(base_data, "runs"))

    args = parser.parse_args()

    set_seed(args.seed)

    print("=" * 70)
    print(f"KSL Training v18 - ST-GCN Bug Fixes + Velocity Features (Alpine HPC)")
    print(f"Started: {ts()}")
    print(f"Seed: {args.seed}")
    print("=" * 70)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Device: {device}")
    if torch.cuda.is_available():
        print(f"GPU: {torch.cuda.get_device_name(0)}")
        print(f"GPU Memory: {torch.cuda.get_device_properties(0).total_memory / 1e9:.1f} GB")
    else:
        print("WARNING: No GPU detected, training will be slow")

    print(f"\nTrain dir: {args.train_dir}")
    print(f"Val dir:   {args.val_dir}")
    print(f"Ckpt dir:  {args.checkpoint_dir}")

    # TensorBoard setup
    writer = None
    if args.tensorboard:
        try:
            from torch.utils.tensorboard import SummaryWriter
            tb_dir = os.path.join(args.tb_logdir, f"v18_{args.model_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
            writer = SummaryWriter(log_dir=tb_dir)
            print(f"TensorBoard: {tb_dir}")
        except ImportError:
            print("WARNING: tensorboard not installed, skipping TensorBoard logging")

    results = {}
    start_time = time.time()

    if args.model_type in ("numbers", "both"):
        print(f"\n[{ts()}] Numbers Config:")
        print(json.dumps(NUMBERS_CONFIG, indent=2))
        ckpt_dir = os.path.join(args.checkpoint_dir, "v18_numbers")
        results["numbers"] = train_split(
            "Numbers", NUMBER_CLASSES, NUMBERS_CONFIG,
            args.train_dir, args.val_dir, ckpt_dir, device, writer=writer,
        )

    if args.model_type in ("words", "both"):
        print(f"\n[{ts()}] Words Config:")
        print(json.dumps(WORDS_CONFIG, indent=2))
        ckpt_dir = os.path.join(args.checkpoint_dir, "v18_words")
        results["words"] = train_split(
            "Words", WORD_CLASSES, WORDS_CONFIG,
            args.train_dir, args.val_dir, ckpt_dir, device, writer=writer,
        )

    total_time = time.time() - start_time

    # Summary
    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] SUMMARY v18")
    print(f"[{ts()}] {'=' * 70}")
    if results.get("numbers"):
        r = results["numbers"]
        print(f"[{ts()}] Numbers: {r['overall']:.1f}% "
              f"(best epoch {r['best_epoch']}, params {r['params']:,}, swa={r['swa_used']})")
    if results.get("words"):
        r = results["words"]
        print(f"[{ts()}] Words:   {r['overall']:.1f}% "
              f"(best epoch {r['best_epoch']}, params {r['params']:,}, swa={r['swa_used']})")
    if results.get("numbers") and results.get("words"):
        combined = (results["numbers"]["overall"] + results["words"]["overall"]) / 2
        print(f"[{ts()}] Combined: {combined:.1f}%")
    print(f"[{ts()}] Total training time: {total_time / 60:.1f} minutes")

    # Save results JSON
    os.makedirs(args.results_dir, exist_ok=True)
    results_path = os.path.join(
        args.results_dir,
        f"v18_{args.model_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    )
    results_out = {
        "version": "v18",
        "model_type": args.model_type,
        "seed": args.seed,
        "numbers_config": NUMBERS_CONFIG if args.model_type in ("numbers", "both") else None,
        "words_config": WORDS_CONFIG if args.model_type in ("words", "both") else None,
        "results": results,
        "total_time_seconds": total_time,
        "device": str(device),
        "gpu": torch.cuda.get_device_name(0) if torch.cuda.is_available() else "cpu",
        "timestamp": ts(),
        "changes_from_v17": [
            "FIX: Separate normalization for hands vs pose (hands were squashed 5x)",
            "FIX: SWA swa_start_frac 0.8->0.4 (actually activates now)",
            "FIX: SWA validates with BASE model for early stopping",
            "FIX: Anti-attractor re-added for numbers (class 54)",
            "FIX: No class weighting (data is balanced at 25/class)",
            "FIX: Numbers augmentation moderated (was 3x too aggressive)",
            "FIX: weight_decay equalized (numbers 0.003->0.001)",
            "FIX: label_smoothing reduced for numbers (0.15->0.1)",
            "FIX: patience increased (numbers 60->80, words 40->50)",
            "FIX: Hand swap also flips pose X coordinates",
            "FIX: Temporal shift uses pad+slice (no wrap artifacts)",
            "ADD: Velocity features (in_channels 3->6)",
            "ADD: AttentionPool replaces AdaptiveAvgPool",
            "ADD: Confusion matrix logging for numbers",
            "ADD: --seed for reproducibility",
        ],
    }
    with open(results_path, "w") as f:
        json.dump(results_out, f, indent=2, default=str)
    print(f"[{ts()}] Results saved to {results_path}")

    if writer is not None:
        writer.close()

    print(f"[{ts()}] Done.")
    return results


if __name__ == "__main__":
    main()
