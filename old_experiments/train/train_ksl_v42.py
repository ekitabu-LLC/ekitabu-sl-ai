#!/usr/bin/env python3
"""
KSL Training v42 - Category-Conditional R&R + SupCon

Builds on v41 (GroupNorm + SupCon + Recover-and-Resample) with a key insight:
  R&R helps numbers (position/shape-dominant) but hurts words (motion-dominant).

V42 applies R&R conditionally based on the category being trained:
  - Numbers training: GroupNorm + SupCon + R&R (same as v41) -> target ~67.8%
  - Words training:   GroupNorm + SupCon, NO R&R (same as exp5) -> target ~65.4%

Since training is already split per category (--model-type numbers vs words),
this is achieved by passing an apply_rr flag to the dataset constructor.

Expected combined single-model: ~66.5% (best of both worlds).

Changes from v41:
  1. Dataset accepts apply_rr flag (True for numbers, False for words)
  2. R&R augmentation skipped when apply_rr=False
  3. All other settings identical (GroupNorm, SupCon, proj head, spatial aug)

Usage:
    python train_ksl_v42.py --model-type numbers  # with R&R
    python train_ksl_v42.py --model-type words    # without R&R
"""
...
# ---------------------------------------------------------------------------
# Graph Topology (identical to v41)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Config (v42 - identical to v41, R&R applied conditionally)
# ---------------------------------------------------------------------------
...
V42_CONFIG = {
    ...
    "grl_start_epoch": 0,
    "grl_ramp_epochs": 100,
    # Recover-and-Resample (applied only for numbers, skipped for words)
    "rr_prob": 0.7,
    "rr_extend_min": 0.05,
    "rr_extend_max": 0.15,
    "rr_crop_min": 0.7,
...
# ---------------------------------------------------------------------------
# Adjacency Matrix (identical to v41)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Data Deduplication (identical to v41)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Normalization (identical to v41)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Augmentation Suite (identical to v41)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Recover-and-Resample Temporal Augmentation (from v41)
# ---------------------------------------------------------------------------
...
def augment_recover_resample(data_list, config):
    ...
    Three stages:
      1. RECOVER: Linearly extrapolate start/end frames
      2. CROP-RESAMPLE: Randomly crop a sub-window and resample back to original T
      3. SEGMENT WARP: Per-segment speed variation
    """
    T = data_list[0].shape[0]
    if T < 4:
        return data_list
...
# ---------------------------------------------------------------------------
# Feature Computation (identical to v41)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Signer-Balanced Batch Sampler (identical to v41)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Temporal CutMix (identical to v41)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Dataset (v42: R&R applied conditionally via apply_rr flag)
# ---------------------------------------------------------------------------
...
class KSLMultiStreamDataset(Dataset):
    def __init__(self, data_dirs, classes, config, aug=False, apply_rr=False):
        ...
        if aug:
            print(f"[{ts()}]   R&R augmentation: {'ENABLED' if apply_rr else 'DISABLED'}")
    ...
    def __getitem__(self, idx):
        ...
        # V42: R&R applied only when apply_rr=True (numbers training)
...
# ---------------------------------------------------------------------------
# Model Components (identical to v41 — GroupNorm throughout)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Main Model (v42: identical architecture to v41)
# ---------------------------------------------------------------------------

class KSLGraphNetV42(nn.Module):
    """ST-GCN v42: GroupNorm + SupCon + category-conditional R&R."""
...
# ---------------------------------------------------------------------------
# Loss Functions (identical to v41)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Train One Stream (v42: passes apply_rr to dataset)
# ---------------------------------------------------------------------------
...
def train_stream(stream_name, name, classes, config, train_dirs, val_dir,
    ...
    train_ds = KSLMultiStreamDataset(train_dirs, classes, config, aug=True, apply_rr=apply_rr)
    val_ds = KSLMultiStreamDataset(val_dir, classes, config, aug=False, apply_rr=False)
    ...
    print(f"[{ts()}] Normalization: GroupNorm (v42)")
...
# ---------------------------------------------------------------------------
# Fusion Weight Learning (identical to v41)
# ---------------------------------------------------------------------------
...
def main():
    parser = argparse.ArgumentParser(
        description="KSL ST-GCN v42 - GroupNorm + SupCon + Category-Conditional R&R",
    ...
    parser.add_argument("--checkpoint-dir", type=str,
                        default=os.path.join(base_data, "checkpoints", "v42"))
    ...
    print("=" * 70)
    print("KSL Training v42 - GroupNorm + SupCon + Category-Conditional R&R")
    print(f"Started: {ts()}")
    ...
    print(f"\nv42 Key Design:")
    print(f"  - Numbers training: GroupNorm + SupCon + R&R (like v41)")
    print(f"  - Words training:   GroupNorm + SupCon, NO R&R (like exp5)")
    print(f"  - Rationale: R&R helps numbers (position/shape) but hurts words (motion)")
    print(f"\n[{ts()}] Config:")
    print(json.dumps(V42_CONFIG, indent=2, default=str))
    ...
    for cat_name, classes, use_focal, apply_rr in categories:
        ...
        stream_results = {}
        for stream_name in V42_CONFIG["streams"]:
            result = train_stream(stream_name, cat_name, classes, V42_CONFIG,
        ...
        fusion_weights, fused_acc = learn_fusion_weights(
            cat_name, classes, V42_CONFIG, val_dir, ckpt_dir, device
        )
        results[cat_key] = {
    ...
    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] SUMMARY v42 - Category-Conditional R&R")
    print(f"[{ts()}] {'=' * 70}")
    for cat_key, cat_result in results.items():
        rr_status = "R&R ON" if cat_result.get("apply_rr") else "R&R OFF"
        print(f"\n[{ts()}] {cat_key.upper()} ({rr_status}):")
    ...
    results_path = os.path.join(
        args.results_dir,
        f"v42_{args.model_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    )
    with open(results_path, "w") as f:
        json.dump({
            ...
            "timestamp": ts(),
            "changes_from_v41": [
                "1. R&R applied conditionally: ON for numbers, OFF for words",
                "2. Numbers: GroupNorm + SupCon + R&R (same as v41, target ~67.8%)",
                "3. Words: GroupNorm + SupCon, NO R&R (same as exp5, target ~65.4%)",
                "4. Rationale: R&R helps position/shape (numbers) but hurts motion (words)",
            ],
