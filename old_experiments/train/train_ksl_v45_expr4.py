#!/usr/bin/env python3
"""
KSL Training V45_expr4 - LLM Semantic Supervision

Based on V43 (exp5 arch + category-conditional R&R):
  - BatchNorm, SupCon w=0.05, R&R for numbers only
  - NEW: semantic supervision loss using CLIP text embeddings

Key idea (CrossGLG ECCV 2024, SCoPLe CVPR 2025):
  LLM-generated text descriptions serve as signer-invariant anchors.
  A projection head maps skeleton features to text embedding space.
  The model learns to match skeleton features to class descriptions,
  providing a domain-invariant training signal.

Total loss: CE + 0.05*SupCon + 0.1*SemanticLoss + GRL*SignerLoss

Usage:
    python train_ksl_v45_expr4.py --model-type numbers
    python train_ksl_v45_expr4.py --model-type words
"""
...
# ---------------------------------------------------------------------------
# KSL Sign Descriptions for semantic supervision
# ---------------------------------------------------------------------------

KSL_SIGN_DESCRIPTIONS = {
    # Numbers
    "9": "The number nine in KSL: extend index finger and thumb forming an L-shape, curl remaining fingers into palm",
    "17": "The number seventeen in KSL: open hand with all five fingers spread wide apart",
    "22": "The number twenty-two in KSL: extend index and middle fingers in a V-shape or peace sign",
    "35": "The number thirty-five in KSL: open hand with fingers together pointing upward",
    "48": "The number forty-eight in KSL: show four fingers then close hand, rapid motion",
    "54": "The number fifty-four in KSL: extend thumb and index finger in an L-shape",
    "66": "The number sixty-six in KSL: make a circular gesture with index finger pointing",
    "73": "The number seventy-three in KSL: extend three fingers upward with wrist motion",
    "89": "The number eighty-nine in KSL: closed fist raised and lowered with emphasis",
    "91": "The number ninety-one in KSL: index finger raised and moved in arc motion",
    "100": "The number one hundred in KSL: flat hand pushed forward with palm facing outward",
    "125": "The number one twenty-five in KSL: hand configuration showing combined digit motion",
    "268": "The number two sixty-eight in KSL: sequential hand shapes representing digits",
    "388": "The number three eighty-eight in KSL: hand with thumb and fingers in specific pose",
    "444": "The number four forty-four in KSL: four fingers extended with repeated motion",
    # Words
    "Agreement": "Agreement in KSL: both hands come together symmetrically, palms touching or clasping",
    "Apple": "Apple in KSL: closed fist with knuckles pressed against cheek, wrist rotates in circular motion",
    "Colour": "Colour in KSL: fingers of one hand spread and wiggle near face or chin",
    "Friend": "Friend in KSL: index fingers of both hands hook together and shake",
    "Gift": "Gift in KSL: both hands held out palms up, moving forward as if offering something",
    "Market": "Market in KSL: hand sweeps horizontally indicating a wide space or area",
    "Monday": "Monday in KSL: index finger circles forward once representing start of the week",
    "Picture": "Picture in KSL: fingers frame face like a camera or picture frame",
    "Proud": "Proud in KSL: fist placed at chest and raised upward with dignity",
    "Sweater": "Sweater in KSL: both hands at chest, pulling motion as if putting on a sweater",
    "Teach": "Teach in KSL: both hands at temples, moving outward repeatedly indicating knowledge transfer",
    "Tomatoes": "Tomatoes in KSL: round shape traced with hand at chin, then sliced motion",
    "Tortoise": "Tortoise in KSL: one hand cups over the other mimicking shell, slow movement",
    "Twin": "Twin in KSL: two index fingers side by side moving together in parallel",
    "Ugali": "Ugali in KSL: stirring motion with one hand as if making ugali in a pot",
}
...
def compute_text_embeddings(descriptions, class_list):
    """Compute text embeddings using transformers CLIP, with random fallback."""
    try:
        from transformers import CLIPModel, CLIPTokenizer
        model_name = "openai/clip-vit-base-patch32"
        print(f"[{ts()}] Loading CLIP model: {model_name}")
        clip_model = CLIPModel.from_pretrained(model_name, use_safetensors=True)
        tokenizer = CLIPTokenizer.from_pretrained(model_name)
        texts = [descriptions[c] for c in class_list]
        inputs = tokenizer(texts, padding=True, truncation=True, max_length=77, return_tensors="pt")
        with torch.no_grad():
            text_features = clip_model.get_text_features(**inputs)
        print(f"[{ts()}] Using transformers CLIP: dim={text_features.shape[1]}")
        return text_features.float()  # (num_classes, 512)
    except Exception as e:
        print(f"[{ts()}] CLIP failed: {e}")

    # Fallback: random fixed embeddings (still provides structure via loss landscape)
    print(f"[{ts()}] WARNING: No text encoder found, using random fixed embeddings dim=256")
    torch.manual_seed(42)
    return torch.randn(len(class_list), 256)
...
class SemanticSupervisionLoss(nn.Module):
    def __init__(self, embed_dim, semantic_dim):
        super().__init__()
    ...
    def forward(self, features, labels, text_embeddings):
        """
        features: (B, embed_dim) skeleton features from model embedding
        labels: (B,) class indices
        text_embeddings: (num_classes, semantic_dim) pre-computed text embeddings
        """
        proj_feat = F.normalize(self.proj(features), dim=-1)
        text_emb = F.normalize(text_embeddings[labels], dim=-1)
        cos_sim = (proj_feat * text_emb).sum(dim=-1)
        loss = (1 - cos_sim).mean()
        return loss
...
# ---------------------------------------------------------------------------
# Config (V45_expr4 = V43 + semantic supervision)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Model (KSLGraphNetV45Expr4 — same as V43 but returns embedding for semantic loss)
# ---------------------------------------------------------------------------
...
class KSLGraphNetV45Expr4(nn.Module):
    """ST-GCN with aux branch, GRL, SupCon — identical arch to V43, named for checkpoint clarity."""
...
def train_stream(stream_name, name, classes, config, train_dir, val_dir,
    ...
    """Train one stream model with semantic supervision."""
    ...
    print(f"[{ts()}] Semantic supervision: weight={config['semantic_weight']}, "
          f"dim={text_embeddings.shape[1] if text_embeddings is not None else 'N/A'}")
    print(f"[{ts()}] {'=' * 70}")

    adj = build_adj(config["num_nodes"]).to(device)

    train_ds = KSLMultiStreamDataset(train_dir, classes, config, aug=True, apply_rr=apply_rr)
    ...
    # Semantic supervision loss
    semantic_dim = text_embeddings.shape[1] if text_embeddings is not None else 256
    sem_loss_fn = SemanticSupervisionLoss(
        embed_dim=model.embed_dim,
        semantic_dim=semantic_dim,
    ).to(device)

    # Move text embeddings to device
    text_emb_device = text_embeddings.to(device) if text_embeddings is not None else None

    param_count = sum(p.numel() for p in model.parameters())
    sem_param_count = sum(p.numel() for p in sem_loss_fn.parameters())
    print(f"[{ts()}] Model parameters: {param_count:,}")
    print(f"[{ts()}] Semantic head parameters: {sem_param_count:,}")
    ...
    print(f"[{ts()}] SupCon weight: {config['supcon_weight']}, Semantic weight: {config['semantic_weight']}")
    if use_focal:
    ...
    # Include semantic projection head params in optimizer
    all_params = list(model.parameters()) + list(sem_loss_fn.parameters())
    ...
    for ep in range(config["epochs"]):
        ...
        for streams_batch, aux_data, targets, signer_targets in train_ld:
            ...
            if use_cutmix:
                ...
                # Semantic loss on primary targets
                sem_loss = sem_loss_fn(embedding, targets_a, text_emb_device)
                loss = cls_loss + supcon_weight * sc_loss + grl_lambda * signer_loss + semantic_weight * sem_loss
                _, p = logits.max(1)
                ...
                signer_loss = F.cross_entropy(signer_logits, signer_targets)
                sem_loss = sem_loss_fn(embedding, targets, text_emb_device)
                loss = cls_loss + supcon_weight * sc_loss + grl_lambda * signer_loss + semantic_weight * sem_loss
                _, p = logits.max(1)
        ...
        avg_sem_loss = sem_loss_accum / max(len(train_ld), 1)
        ...
        if ep < 10 or (ep + 1) % 10 == 0 or va > best:
            print(
                ...
                f"Loss: {tl / max(len(train_ld), 1):.4f} | SemLoss: {avg_sem_loss:.4f} | "
...
def main():
    parser = argparse.ArgumentParser(
        description="KSL Training V45_expr4 - LLM Semantic Supervision",
    ...
    parser.add_argument("--checkpoint-dir", type=str, default=os.path.join(base_data, "checkpoints", "v45_expr4"))
    ...
    print("=" * 70)
    print(f"KSL Training V45_expr4 - LLM Semantic Supervision")
    print(f"Base: V43 (exp5 arch + category-conditional R&R)")
    print(f"NEW: CLIP text embeddings as signer-invariant anchors")
    print(f"Loss: CE + 0.05*SupCon + 0.1*SemanticLoss + GRL*SignerLoss")
    print(f"Started: {ts()}")
    ...
    print(f"\nV45_expr4 Data Split:")
    print(f"  Train dirs: {train_dirs}  (signers 1-12)")
    print(f"  Val dir:    {val_dir}  (signers 13-15)")

    # Pre-compute text embeddings for both categories
    print(f"\n[{ts()}] Pre-computing text embeddings...")
    number_text_emb = compute_text_embeddings(KSL_SIGN_DESCRIPTIONS, NUMBER_CLASSES)
    word_text_emb = compute_text_embeddings(KSL_SIGN_DESCRIPTIONS, WORD_CLASSES)
    print(f"[{ts()}] Numbers text embeddings: {number_text_emb.shape}")
    print(f"[{ts()}] Words text embeddings: {word_text_emb.shape}")

    print(f"\n[{ts()}] V45_expr4 Config:")
    print(json.dumps(V45_EXPR4_CONFIG, indent=2, default=str))
    ...
    if args.model_type in ("numbers", "both"):
        categories.append(("Numbers", NUMBER_CLASSES, False, True, number_text_emb))
    if args.model_type in ("words", "both"):
        categories.append(("Words", WORD_CLASSES, True, False, word_text_emb))

    for cat_name, classes, use_focal, apply_rr, text_emb in categories:
        cat_key = cat_name.lower()
        ckpt_dir = os.path.join(args.checkpoint_dir, cat_key)

        stream_results = {}
        for stream_name in V45_EXPR4_CONFIG["streams"]:
            result = train_stream(
                stream_name, cat_name, classes, V45_EXPR4_CONFIG,
        ...
        fusion_weights, fused_acc = learn_fusion_weights(
            cat_name, classes, V45_EXPR4_CONFIG, val_dir, ckpt_dir, device
        )
    ...
    # Summary
    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] SUMMARY V45_expr4 - LLM Semantic Supervision")
    print(f"[{ts()}] {'=' * 70}")
    ...
    results_path = os.path.join(
        args.results_dir,
        f"v45_expr4_{args.model_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    )
    results_out = {
        "version": "v45_expr4",
        "model_type": args.model_type,
        "seed": args.seed,
        "config": V45_EXPR4_CONFIG,
        "results": results,
        ...
        "timestamp": ts(),
        "changes_from_v43": [
            "1. Added SemanticSupervisionLoss with CLIP text embeddings",
            "2. Semantic projection head: embed_dim -> 256 -> semantic_dim",
            "3. Total loss: CE + 0.05*SupCon + 0.1*SemanticLoss + GRL*SignerLoss",
            "4. Text embeddings from openai/clip-vit-base-patch32 (512-dim)",
            "5. Semantic head params included in optimizer",
        ],
    }
