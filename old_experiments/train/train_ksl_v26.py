#!/usr/bin/env python3
"""
KSL Training v26 (Alpine) - v25 base, retrained on ksl-alpha dataset (10 training signers)

Based on v25 (v22 base + Hand-Body Features + Temporal CutMix + LOSO).
The ksl-alpha dataset expands from 4 unique training signers to 10, with a held-out
test set of 3 signers (signers 13-15) for signer-independent evaluation.

Changes from v25:
  1. DATA PATHS: Default train/val dirs changed to train_alpha/val_alpha for ksl-alpha dataset.
  2. TEST SET EVALUATION: New --test-dir argument (default: test_alpha) for held-out test signers.
     After training, automatically evaluates on test set if the directory exists.
  3. VERSION STRING: Updated to v26, noting ksl-alpha 10 signers.

All architecture, augmentation, losses, and hyperparameters are IDENTICAL to v25.

Usage:
    python train_ksl_v26.py --model-type numbers
    python train_ksl_v26.py --model-type words
    python train_ksl_v26.py --model-type both --seed 42
    python train_ksl_v26.py --model-type numbers --loso
    python train_ksl_v26.py --model-type both --test-dir data/test_alpha
"""
...
# ---------------------------------------------------------------------------
# Config (v26 - identical to v25, retrained on ksl-alpha 10 signers)
# ---------------------------------------------------------------------------
...
def train_split(name, classes, config, train_dir, val_dir, ckpt_dir, device):
    """Train one split (numbers or words) with v26 (v25 arch on ksl-alpha data)."""

    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] Training {name} ({len(classes)} classes) - v26")
    print(f"[{ts()}] {'=' * 70}")
...
def run_loso(name, classes, config, train_dir, val_dir, ckpt_dir, results_dir, device):
    ...
    """
    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] LOSO Cross-Validation: {name} ({len(classes)} classes) - v26")
    print(f"[{ts()}] {'=' * 70}")
...
# ---------------------------------------------------------------------------
# Test Set Evaluation (v26 NEW - held-out signer evaluation)
# ---------------------------------------------------------------------------
...
def evaluate_test_set(name, classes, config, test_dir, ckpt_path, device):
    ...
    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] Test Set Evaluation: {name} ({len(classes)} classes) - v26")
...
def main():
    parser = argparse.ArgumentParser(
        description="KSL ST-GCN Training v26 (Alpine HPC) - ksl-alpha dataset",
    ...
    print("=" * 70)
    print(f"KSL Training v26 - ksl-alpha (10 signers) + Hand-Body Features + CutMix + LOSO [{mode_str}]")
    print(f"Started: {ts()}")
    ...
    print(f"\n[{ts()}] v26 Config:")
    print(json.dumps(V26_CONFIG, indent=2, default=str))
    ...
    if args.loso:
        ...
        if args.model_type in ("numbers", "both"):
            ckpt_dir = os.path.join(args.checkpoint_dir, "v26_loso_numbers")
            results["numbers_loso"] = run_loso(
                "Numbers", NUMBER_CLASSES, V26_CONFIG,
                args.train_dir, args.val_dir, ckpt_dir, args.results_dir, device,
            )

        if args.model_type in ("words", "both"):
            ckpt_dir = os.path.join(args.checkpoint_dir, "v26_loso_words")
            results["words_loso"] = run_loso(
                "Words", WORD_CLASSES, V26_CONFIG,
                args.train_dir, args.val_dir, ckpt_dir, args.results_dir, device,
            )
    else:
        # Normal training mode
        if args.model_type in ("numbers", "both"):
            ckpt_dir = os.path.join(args.checkpoint_dir, "v26_numbers")
        ...
        if args.model_type in ("words", "both"):
            ckpt_dir = os.path.join(args.checkpoint_dir, "v26_words")
            results["words"] = train_split(
                "Words", WORD_CLASSES, V26_CONFIG,
                args.train_dir, args.val_dir, ckpt_dir, device,
            )

    # --- v26: Evaluate on held-out test set (if available) ---
    if args.test_dir and os.path.isdir(args.test_dir) and not args.loso:
        print(f"\n[{ts()}] {'=' * 70}")
        print(f"[{ts()}] HELD-OUT TEST SET EVALUATION (ksl-alpha signers 13-15)")
        print(f"[{ts()}] {'=' * 70}")

        if args.model_type in ("numbers", "both"):
            ckpt_path = os.path.join(args.checkpoint_dir, "v26_numbers", "best_model.pt")
            test_result = evaluate_test_set(
                "Numbers", NUMBER_CLASSES, V26_CONFIG,
                args.test_dir, ckpt_path, device,
            )
            if test_result:
                results["numbers_test"] = test_result

        if args.model_type in ("words", "both"):
            ckpt_path = os.path.join(args.checkpoint_dir, "v26_words", "best_model.pt")
            test_result = evaluate_test_set(
                "Words", WORD_CLASSES, V26_CONFIG,
                args.test_dir, ckpt_path, device,
            )
            if test_result:
                results["words_test"] = test_result

        # Print test summary
        if results.get("numbers_test") or results.get("words_test"):
            print(f"\n[{ts()}] --- Test Set Summary ---")
            if results.get("numbers_test"):
                print(f"[{ts()}] Numbers Test: {results['numbers_test']['overall']:.1f}%")
            if results.get("words_test"):
                print(f"[{ts()}] Words Test:   {results['words_test']['overall']:.1f}%")
    ...
    # Summary
    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] SUMMARY v26 [{mode_str}]")
    print(f"[{ts()}] {'=' * 70}")
    ...
    results_path = os.path.join(
        args.results_dir,
        f"v26{loso_tag}_{args.model_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    )
    results_out = {
        "version": "v26",
        "mode": mode_str,
        ...
        "timestamp": ts(),
        "changes_from_v25": [
            "1. KSL-ALPHA DATASET: 10 training signers (up from 4 unique in v2). "
               "Held-out test set with 3 signers (13-15) for signer-independent evaluation.",
            "2. TEST SET EVALUATION: Automatic evaluation on held-out test signers after training.",
            "3. Same v25 architecture, augmentation, and hyperparameters (hand-body features, "
               "temporal CutMix, CE+SupCon+GRL losses).",
        ],
    }
