#!/usr/bin/env python3
"""
KSL Training v31 Exp3 - Joint Mixing Augmentation

Based on v28 multi-stream late fusion. ONE CHANGE:
  - Add Joint Mixing augmentation: with prob 0.5, mix same-class skeleton sequences
    using x_mix = λ*x_i + (1-λ)*x_j where λ ~ Beta(0.2, 0.2)

Reference: "Joint Mixing Data Augmentation" (ACM Trans. Multimedia, 2024)

Everything else identical to v28.

Usage:
    python train_ksl_v31_exp3.py --model-type numbers
    python train_ksl_v31_exp3.py --model-type words
"""
...
V31_EXP3_CONFIG = {
    ...
    "cutmix_alpha": 1.0,
    "cutmix_prob": 0.3,
    # Joint Mixing (NEW for v31_exp3)
    "joint_mixing_prob": 0.5,
    "joint_mixing_beta": 0.2,
}
...
def joint_mixing(streams_batch, aux_data, labels, beta=0.2):
    """
    Mix same-class skeleton sequences with λ ~ Beta(beta, beta).

    Args:
        streams_batch: dict of stream tensors {name: (B, C, T, N)}
        aux_data: (B, T, D_aux)
        labels: (B,)
        beta: Beta distribution parameter

    Returns:
        mixed_streams, mixed_aux, labels (unchanged)
    """
    B = labels.shape[0]
    ...
    # Find mixing partners (same class)
    mixed_streams = {}
    for stream_name, stream_tensor in streams_batch.items():
        mixed_streams[stream_name] = stream_tensor.clone()
    mixed_aux = aux_data.clone()

    # For each sample, find a same-class partner
    for i in range(B):
        label_i = labels[i].item()
        # Find all indices with same label
        same_class_indices = (labels == label_i).nonzero(as_tuple=True)[0]
        # Remove self
        same_class_indices = same_class_indices[same_class_indices != i]

        if len(same_class_indices) == 0:
            # No same-class partner, skip mixing for this sample
            continue

        # Randomly select a partner
        j = same_class_indices[torch.randint(len(same_class_indices), (1,)).item()].item()

        # Mix all streams
        for stream_name in mixed_streams:
            mixed_streams[stream_name][i] = (
                lam * streams_batch[stream_name][i] +
                (1 - lam) * streams_batch[stream_name][j]
            )

        # Mix auxiliary features
        mixed_aux[i] = lam * aux_data[i] + (1 - lam) * aux_data[j]

    return mixed_streams, mixed_aux, labels
...
# ---------------------------------------------------------------------------
# Model (KSLGraphNetV25 - same as v28)
# ---------------------------------------------------------------------------
...
class KSLGraphNetV25(nn.Module):
    """ST-GCN with aux branch, GRL, SupCon. Same as v25-v28."""
...
# ---------------------------------------------------------------------------
# Train One Stream (adapted from v28, adding Joint Mixing)
# ---------------------------------------------------------------------------

def train_stream(stream_name, name, classes, config, train_dir, val_dir,
                 ckpt_dir, device, use_focal=False):
    """Train one stream model with Joint Mixing augmentation."""
    ...
    print(f"[{ts()}] Joint Mixing: prob={config['joint_mixing_prob']}, beta={config['joint_mixing_beta']}")
    ...
    joint_mixing_prob = config.get("joint_mixing_prob", 0.5)
    joint_mixing_beta = config.get("joint_mixing_beta", 0.2)
    ...
    for ep in range(config["epochs"]):
        ...
        for streams_batch, aux_data, targets, signer_targets in train_ld:
            # Move to device
            streams_batch = {k: v.to(device) for k, v in streams_batch.items()}
            ...
            # Joint Mixing (NEW for v31_exp3)
            if np.random.random() < joint_mixing_prob:
                streams_batch, aux_data, targets = joint_mixing(
                    streams_batch, aux_data, targets, beta=joint_mixing_beta
                )

            # Extract this stream's data
            gcn_data = streams_batch[stream_name]
...
def main():
    parser = argparse.ArgumentParser(
        description="KSL v31 Exp3 - Joint Mixing Augmentation",
    ...
    parser.add_argument("--checkpoint-dir", type=str,
                        default=os.path.join(base_data, "checkpoints", "v31_exp3"))
    ...
    print("=" * 70)
    print(f"KSL Training v31 Exp3 - Joint Mixing Augmentation")
    print(f"Started: {ts()}")
    ...
    print(f"  Streams: {V31_EXP3_CONFIG['streams']}")

    print(f"\n[{ts()}] v31_exp3 Config:")
    print(json.dumps(V31_EXP3_CONFIG, indent=2, default=str))
    ...
    for cat_name, classes, use_focal in categories:
        ...
        # Train each stream
        stream_results = {}
        for stream_name in V31_EXP3_CONFIG["streams"]:
            result = train_stream(
                stream_name, cat_name, classes, V31_EXP3_CONFIG,
        ...
        fusion_weights, fused_acc = learn_fusion_weights(
            cat_name, classes, V31_EXP3_CONFIG, val_dir, ckpt_dir, device
        )
    ...
    # Summary
    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] SUMMARY v31_exp3 - Joint Mixing Augmentation")
    print(f"[{ts()}] {'=' * 70}")
    ...
    results_path = os.path.join(
        args.results_dir,
        f"v31_exp3_{args.model_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    )
    results_out = {
        "version": "v31_exp3",
        "model_type": args.model_type,
        "seed": args.seed,
        "config": V31_EXP3_CONFIG,
        "results": results,
        ...
        "timestamp": ts(),
        "changes_from_v28": [
            "1. JOINT MIXING: With prob 0.5, mix same-class skeleton sequences using "
               "x_mix = λ*x_i + (1-λ)*x_j where λ ~ Beta(0.2, 0.2). Applied to all streams "
               "(joint, bone, velocity) and auxiliary features.",
        ],
    }
