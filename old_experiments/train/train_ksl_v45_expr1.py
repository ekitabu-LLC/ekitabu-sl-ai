#!/usr/bin/env python3
"""
KSL Training v45 Expr1 — Sequence-Level Signer Adversarial GRL.

Based on v44 expr2 (frame-level GRL) but applies GRL AFTER temporal pooling
on sequence-level statistics (mean+std), not on per-frame spatial features.

Key insight from ACM TOMM 2024: frame-level GRL destroys motion encoding
(v44_expr2 words dropped to 53.1%). Sequence-level GRL achieves signer
invariance without breaking temporal/motion features.

Architecture:
  - GroupNorm (no BatchNorm) — domain-invariant normalization
  - GRL on mean+std pooled GCN features (2*final_ch dim)
  - Joint and bone streams: GRL applied
  - Velocity stream: NO GRL (preserve motion encoding)
  - Signer head: Linear(2*final_ch, 128) -> ReLU -> Linear(128, 12)
  - NO SupCon loss (isolate sequence-level GRL effect)

GRL schedule:
  lambda(t) = lambda_max * (2 / (1 + exp(-10 * t/T)) - 1)
  where t = current epoch, T = total epochs

Usage:
    python train_ksl_v45_expr1.py --model-type numbers --lambda-max 0.1
    python train_ksl_v45_expr1.py --model-type words --lambda-max 0.1
"""
...
V45_EXPR1_CONFIG = {
    ...
    "patience": 80,
    "warmup_epochs": 10,
    # GRL-specific (defaults, overridden by CLI)
    "grl_lambda_max": 0.1,
    "num_signers": 12,
    # No SupCon — isolate GRL effect
    "supcon_weight": 0.0,
    # Augmentation (same as v28/exp1)
    "rotation_prob": 0.5,
...
class KSLMultiStreamDataset(Dataset):
    """Returns per-stream tensors (joint/bone/velocity) plus signer IDs."""
    ...
    def __init__(self, data_dirs, classes, config, aug=False):
        ...
        # Extract signer IDs from filenames
        self.signer_to_idx = {}
        self.signer_labels = []
...
# ---------------------------------------------------------------------------
# Model — v45_expr1: GroupNorm + Sequence-Level Signer-Adversarial GRL
# ---------------------------------------------------------------------------
...
class KSLGraphNetV45Expr1(nn.Module):
    """ST-GCN with GroupNorm + sequence-level signer-adversarial GRL head.

    Key difference from V44Expr2: GRL operates on mean+std pooled sequence
    statistics (2*final_ch dim) instead of per-frame embeddings. This preserves
    temporal/motion encoding while still encouraging signer invariance.

    The signer head takes sequence statistics, NOT the full embedding used for
    classification. This decouples signer invariance from the sign classifier.
    """
    ...
    def __init__(self, nc, num_signers, aux_dim, nn_=48, ic=3, hd=64, nl=4,
        ...
        # Sequence-level signer-adversarial head (after GRL)
        # Operates on mean+std of GCN temporal features: 2 * final_ch
        # final_ch = 128 (hd*2 for last 2 layers), so stats_dim = 256
        stats_dim = 2 * final_ch
        if use_grl:
            self.signer_classifier = nn.Sequential(
    ...
    def forward(self, gcn_input, aux_input, grl_lambda=0.0):
        ...
        # x shape: (B, final_ch, T', N) where T' is reduced by stride-2 layers
        # Average over nodes -> (B, final_ch, T')
        x_node_avg = x.mean(dim=3)

        # --- Sequence-level statistics for GRL ---
        # x_node_avg: (B, C, T') -> compute mean and std over temporal dim
        if self.use_grl and grl_lambda > 0:
            feat_mean = x_node_avg.mean(dim=2)  # (B, final_ch)
            feat_std = x_node_avg.std(dim=2)     # (B, final_ch)
            feat_stats = torch.cat([feat_mean, feat_std], dim=-1)  # (B, 2*final_ch)

            # GRL only on the statistics vector
            feat_stats_reversed = grad_reverse(feat_stats, grl_lambda)
            signer_logits = self.signer_classifier(feat_stats_reversed)
        else:
            signer_logits = None

        # --- Normal classification path (unchanged from v44_expr2) ---
        ...
        # Sign classification (normal gradient flow)
        sign_logits = self.classifier(embedding)

        # Return dummy signer_logits if GRL disabled
        if signer_logits is None:
            signer_logits = torch.zeros(b, 1, device=gcn_input.device)

        return sign_logits, signer_logits, embedding
...
def compute_grl_lambda(epoch, max_epochs, lambda_max):
    """Sigmoid ramp schedule for GRL lambda.

    lambda(t) = lambda_max * (2 / (1 + exp(-10 * t/T)) - 1)

    Ramps from ~0 to lambda_max over the training run.
    """
...
def train_stream(stream_name, name, classes, config, train_dir, val_dir,
    ...
    # Velocity stream: no GRL (preserve motion encoding)
    use_grl = (stream_name != "velocity")

    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] Training {name} - Stream: {stream_name} ({len(classes)} classes)")
    print(f"[{ts()}] GRL: {'ENABLED (sequence-level)' if use_grl else 'DISABLED (velocity stream)'}")
    print(f"[{ts()}] {'=' * 70}")
    ...
    print(f"[{ts()}] Normalization: GroupNorm")
    if use_grl:
        print(f"[{ts()}] GRL lambda_max: {config['grl_lambda_max']}")
        print(f"[{ts()}] GRL type: sequence-level (mean+std pooled, {2 * model.final_ch}-dim)")
    if use_focal:
        print(f"[{ts()}] Loss: FocalLoss(gamma=2.0) + GRL signer CE")
    else:
        print(f"[{ts()}] Loss: CE(ls={config['label_smoothing']})" +
              (" + GRL signer CE" if use_grl else ""))
    ...
    lambda_max = config["grl_lambda_max"]
    ...
    for ep in range(max_epochs):
        ...
        # Sigmoid ramp GRL schedule (0 for velocity stream)
        grl_lambda = compute_grl_lambda(ep, max_epochs, lambda_max) if use_grl else 0.0
        ...
        for streams_batch, aux_data, targets, signer_targets in train_ld:
            ...
            if use_cutmix:
                ...
                sign_logits, signer_logits, embedding = model(
                    gcn_mixed, aux_mixed, grl_lambda=grl_lambda,
                )
                if cls_loss_fn is not None:
                    cls_loss = (
                        lam * cls_loss_fn(sign_logits, targets_a)
                        + (1 - lam) * cls_loss_fn(sign_logits, targets_b)
                    )
                else:
                    cls_loss = (
                        lam * F.cross_entropy(sign_logits, targets_a, label_smoothing=config["label_smoothing"])
                        + (1 - lam) * F.cross_entropy(sign_logits, targets_b, label_smoothing=config["label_smoothing"])
                    )
                if use_grl and grl_lambda > 0:
                    signer_loss = F.cross_entropy(signer_logits, signer_targets)
                    loss = cls_loss + grl_lambda * signer_loss
                else:
                    loss = cls_loss
                _, p = sign_logits.max(1)
                ...
                if cls_loss_fn is not None:
                    cls_loss = (
                        lam * cls_loss_fn(sign_logits, targets)
                        + (1 - lam) * cls_loss_fn(sign_logits, t_perm)
                    )
                else:
                    cls_loss = (
                        lam * F.cross_entropy(sign_logits, targets, label_smoothing=config["label_smoothing"])
                        + (1 - lam) * F.cross_entropy(sign_logits, t_perm, label_smoothing=config["label_smoothing"])
                    )
                if use_grl and grl_lambda > 0:
                    signer_loss = F.cross_entropy(signer_logits, signer_targets)
                    loss = cls_loss + grl_lambda * signer_loss
                ...
                if cls_loss_fn is not None:
                    cls_loss = cls_loss_fn(sign_logits, targets)
                else:
                    cls_loss = F.cross_entropy(sign_logits, targets, label_smoothing=config["label_smoothing"])
                if use_grl and grl_lambda > 0:
                    signer_loss = F.cross_entropy(signer_logits, signer_targets)
                    loss = cls_loss + grl_lambda * signer_loss
                else:
        ...
        with torch.no_grad():
            for streams_batch, aux_data, targets, _ in val_ld:
                ...
                aux_data = aux_data.to(device)
                targets = targets.to(device)
                sign_logits, _, _ = model(gcn_data, aux_data, grl_lambda=0.0)
                _, p = sign_logits.max(1)
                vt += targets.size(0)
        ...
        if ep < 10 or (ep + 1) % 10 == 0 or va > best:
            grl_str = f" | SignerAcc: {signer_acc:.1f}% | GRL_lambda: {grl_lambda:.4f}" if use_grl else ""
            print(
                f"[{ts()}] [{stream_name}] Ep {ep + 1:3d}/{max_epochs} | "
        ...
        if (ep + 1) % 50 == 0:
            ...
            with torch.no_grad():
                for streams_batch, aux_data, targets, _ in val_ld:
                    gcn_data = streams_batch[stream_name].to(device)
                    sign_logits, _, _ = model(gcn_data.to(device), aux_data.to(device),
                                              grl_lambda=0.0)
                    _, p = sign_logits.max(1)
...
def learn_fusion_weights(category_name, classes, config, val_dir, ckpt_dir, device):
    ...
    for sname in streams:
        ...
        use_grl = ckpt.get("use_grl", sname != "velocity")
        ...
        print(f"[{ts()}] Loaded {sname} model: val_acc={ckpt['val_acc']:.1f}%, "
              f"epoch={ckpt['epoch']}, grl={'yes' if use_grl else 'no'}")
...
def main():
    parser = argparse.ArgumentParser(
        description="KSL v45 Expr1 — Sequence-Level Signer-Adversarial GRL + GroupNorm",
    ...
    parser.add_argument("--lambda-max", type=float, default=0.1,
                        help="Max GRL lambda (sigmoid ramp)")
    parser.add_argument("--num-signers", type=int, default=12,
                        help="Number of training signers")
    ...
    parser.add_argument("--checkpoint-dir", type=str, default=os.path.join(base_data, "checkpoints/v45_expr1"))
    parser.add_argument("--results-dir", type=str, default=os.path.join(base_data, "results"))

    args = parser.parse_args()

    # Apply CLI overrides to config
    config = V45_EXPR1_CONFIG.copy()
    config["grl_lambda_max"] = args.lambda_max
    config["num_signers"] = args.num_signers

    set_seed(args.seed)

    print("=" * 70)
    print("KSL Training v45 Expr1 — Sequence-Level Signer-Adversarial GRL + GroupNorm")
    print(f"Started: {ts()}")
    print(f"Seed: {args.seed}")
    print(f"GRL lambda_max: {args.lambda_max}")
    print(f"Num signers: {args.num_signers}")
    print(f"GRL strategy: sequence-level (mean+std pooled) on joint/bone only")
    print(f"Velocity stream: NO GRL (preserve motion encoding)")
    print("=" * 70)
    ...
    print(f"  GRL schedule: sigmoid ramp to {config['grl_lambda_max']}")
    print(f"  GRL on: joint, bone | GRL off: velocity")
    ...
    for cat_name, classes, use_focal in categories:
        ...
        for stream_name in config["streams"]:
            result = train_stream(
                stream_name, cat_name, classes, config,
        ...
        fusion_weights, fused_acc = learn_fusion_weights(
            cat_name, classes, config, val_dir, ckpt_dir, device
        )
    ...
    # Summary
    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] SUMMARY v45 Expr1 — Sequence-Level Signer-Adversarial GRL")
    print(f"[{ts()}] GRL lambda_max: {config['grl_lambda_max']}")
    print(f"[{ts()}] GRL on: joint, bone | GRL off: velocity")
    print(f"[{ts()}] {'=' * 70}")

    for cat_key, cat_result in results.items():
        print(f"\n[{ts()}] {cat_key.upper()}:")
        for sname, sres in cat_result.get("streams", {}).items():
            grl_tag = " [GRL]" if sres.get("use_grl", True) else " [no GRL]"
            print(f"[{ts()}]   {sname:>10s}: {sres['overall']:.1f}% "
                  f"(ep {sres['best_epoch']}, {sres['params']:,} params){grl_tag}")
    ...
    results_path = os.path.join(
        args.results_dir,
        f"v45_expr1_{args.model_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    )
    results_out = {
        "version": "v45_expr1",
        "experiment": "sequence_level_signer_adversarial_grl",
        ...
        "timestamp": ts(),
        "description": [
            "GroupNorm + sequence-level signer-adversarial GRL (no SupCon).",
            f"GRL sigmoid ramp to lambda_max={config['grl_lambda_max']}.",
            "GRL on mean+std pooled GCN features (joint/bone streams only).",
            "Velocity stream: NO GRL (preserve motion encoding).",
            "Signer head: Linear(2*final_ch, 128) -> ReLU -> Linear(128, 12).",
            "Key change from v44_expr2: GRL on sequence stats, not per-frame features.",
        ],
    }
