#!/usr/bin/env python3
"""
KSL Training v40 - Variational Contrastive Learning (VarCL)

Based on v31_exp5 (SupCon). Extends contrastive learning with a variational
projection head: instead of mapping to point embeddings, outputs (mu, logvar)
and samples z ~ N(mu, sigma^2) via reparameterization trick.

Key idea: model signer variability as distributions rather than points.
Loss = CE + supcon_weight * SupCon(z) + kl_weight * KL(q(z|x) || N(0,1))

Key changes from exp5:
  1. Variational projection head: embed_dim -> proj_dim (mu) + proj_dim (logvar)
  2. Reparameterization sampling during training
  3. KL divergence regularizer (kl_weight=0.001)
  4. At inference: use mu directly (no sampling)

Usage:
    python train_ksl_v40.py --model-type numbers
    python train_ksl_v40.py --model-type words
"""
...
# ---------------------------------------------------------------------------
# Config (v40 - VarCL: variational contrastive learning)
# ---------------------------------------------------------------------------
...
V40_CONFIG = {
    ...
    # VarCL: variational projection head
    "supcon_proj_dim": 128,
    "kl_weight": 0.001,  # KL regularizer weight (small — SupCon must dominate)
    # Signer adversarial
    "grl_lambda_max": 0.3,
    "grl_start_epoch": 0,
    "grl_ramp_epochs": 100,
    # Augmentation (same as v28/exp5)
    "rotation_prob": 0.5,
...
# ---------------------------------------------------------------------------
# Model: KSLGraphNetV40 (VarCL - Variational Contrastive Learning)
# ---------------------------------------------------------------------------
...
class KSLGraphNetV40(nn.Module):
    """ST-GCN with variational projection head for contrastive learning.

    Same backbone as V25/exp5. Key addition: variational projection head
    that outputs (mu, logvar) instead of a point embedding for SupCon.

    During training: z = mu + eps * exp(0.5 * logvar), SupCon applied to z
    During inference: use mu directly (no sampling)
    """
    ...
    def __init__(self, nc, num_signers, aux_dim, proj_dim=128,
        ...
        # Classifier: operates on the full embedding (same as exp5)
        ...
        # Signer adversarial head (same as exp5)
        ...
        # NEW: Variational projection head for SupCon
        # Shared encoder: embed_dim -> proj_dim
        self.proj_encoder = nn.Sequential(
            nn.Linear(self.embed_dim, proj_dim),
            nn.ReLU(),
        )
        # Separate heads for mu and logvar
        self.proj_mu = nn.Linear(proj_dim, proj_dim)
        self.proj_logvar = nn.Linear(proj_dim, proj_dim)
    ...
    def forward(self, gcn_input, aux_input, grl_lambda=0.0):
        ...
        # Variational projection head
        h_proj = self.proj_encoder(embedding)
        mu = self.proj_mu(h_proj)
        logvar = self.proj_logvar(h_proj)
        ...
        if self.training:
            ...
            z = mu  # At inference, use mean (no sampling)

        return logits, signer_logits, embedding, mu, logvar, z
...
def train_stream(stream_name, name, classes, config, train_dir, val_dir,
    ...
    print(f"[{ts()}] SupCon weight: {config['supcon_weight']}, "
          f"KL weight: {config['kl_weight']}, "
          f"temp={config['supcon_temperature']}, proj_dim={proj_dim}")
    if use_focal:
        print(f"[{ts()}] Loss: FocalLoss(gamma=2.0) + VarCL")
    else:
        print(f"[{ts()}] Loss: CE(ls={config['label_smoothing']}) + VarCL")
    ...
    for ep in range(config["epochs"]):
        ...
        for streams_batch, aux_data, targets, signer_targets in train_ld:
            ...
            if use_cutmix:
                ...
                logits, signer_logits, embedding, mu, logvar, z = model(
                ...
                sc_loss = supcon_loss_fn(z, targets_a)
                kl_loss = kl_divergence_loss(mu, logvar)
                signer_loss = F.cross_entropy(signer_logits, signer_targets)
                loss = cls_loss + supcon_weight * sc_loss + kl_weight * kl_loss + grl_lambda * signer_loss
                _, p = logits.max(1)
        ...
        with torch.no_grad():
            for streams_batch, aux_data, targets, _ in val_ld:
                ...
                aux_data = aux_data.to(device)
                targets = targets.to(device)
                logits, _, _, _, _, _ = model(gcn_data, aux_data, grl_lambda=0.0)
                _, p = logits.max(1)
                vt += targets.size(0)
        ...
        if ep < 10 or (ep + 1) % 10 == 0 or va > best:
            print(
                ...
                f"Loss: {tl / n_batches:.4f} | Train: {ta:.1f}% | Val: {va:.1f}% | "
                f"SC: {avg_sc:.3f} | KL: {avg_kl:.3f} | "
        ...
        if (ep + 1) % 50 == 0:
            ...
            with torch.no_grad():
                for streams_batch, aux_data, targets, _ in val_ld:
                    gcn_data = streams_batch[stream_name].to(device)
                    logits, _, _, _, _, _ = model(gcn_data.to(device), aux_data.to(device),
                                                   grl_lambda=0.0)
                    _, p = logits.max(1)
...
def learn_fusion_weights(category_name, classes, config, val_dir, ckpt_dir, device):
    ...
    for sname in streams:
        ...
        model = KSLGraphNetV40(
            ...
            proj_dim=proj_dim, nn_=config["num_nodes"], ic=3, hd=config["hidden_dim"],
    ...
    with torch.no_grad():
        for streams_batch, aux_data, targets, _ in val_ld:
            ...
            for sname in streams:
                ...
                logits, _, _, _, _, _ = models[sname](gcn_data, aux_data, grl_lambda=0.0)
...
def main():
    parser = argparse.ArgumentParser(
        description="KSL ST-GCN Training v40 - VarCL (Variational Contrastive Learning)",
    ...
    parser.add_argument("--checkpoint-dir", type=str, default=os.path.join(base_data, "checkpoints", "v40"))
    ...
    print("=" * 70)
    print(f"KSL Training v40 - VarCL (Variational Contrastive Learning)")
    print(f"Started: {ts()}")
    ...
    print(f"\nv40 VarCL Data Split:")
    print(f"  Train dirs: {train_dirs}  (signers 1-12)")
    print(f"  Val dir:    {val_dir}  (signers 13-15)")
    print(f"  Streams: {V40_CONFIG['streams']}")
    print(f"  Batch size: {V40_CONFIG['batch_size']}")
    print(f"  SupCon weight: {V40_CONFIG['supcon_weight']}")
    print(f"  KL weight: {V40_CONFIG['kl_weight']}")
    print(f"  Proj dim: {V40_CONFIG['supcon_proj_dim']}")

    print(f"\n[{ts()}] v40 Config:")
    print(json.dumps(V40_CONFIG, indent=2, default=str))
    ...
    for cat_name, classes, use_focal in categories:
        ...
        stream_results = {}
        for stream_name in V40_CONFIG["streams"]:
            result = train_stream(
                stream_name, cat_name, classes, V40_CONFIG,
        ...
        fusion_weights, fused_acc = learn_fusion_weights(
            cat_name, classes, V40_CONFIG, val_dir, ckpt_dir, device
        )
    ...
    # Summary
    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] SUMMARY v40 - VarCL (Variational Contrastive Learning)")
    print(f"[{ts()}] {'=' * 70}")
    ...
    results_path = os.path.join(
        args.results_dir,
        f"v40_{args.model_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    )
    results_out = {
        "version": "v40",
        ...
        "timestamp": ts(),
        "changes_from_exp5": [
            "1. Variational projection head: outputs (mu, logvar) instead of point z",
            "2. Reparameterization trick: z = mu + eps * exp(0.5*logvar) during training",
            "3. KL divergence regularizer: kl_weight=0.001",
            "4. SupCon applied to sampled z (not raw embedding)",
            "5. At inference: use mu directly (deterministic)",
            "6. Projection head: embed_dim(320) -> ReLU -> proj_dim(128) for mu/logvar",
        ],
    }
