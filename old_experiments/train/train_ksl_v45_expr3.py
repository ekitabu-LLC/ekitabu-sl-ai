#!/usr/bin/env python3
"""
KSL Training V45 Expr3 - Multi-Scale Dilated Temporal Convolution

Based on V43 (exp5 arch: BatchNorm in spatial GCN, SupCon w=0.05,
category-conditional R&R). Replaces the existing MultiScaleTCN
(which averages kernels 3/5/7 at dilation=1) with a dilated multi-scale
TCN using 4 parallel branches:

  Branch 1: kernel=3, dilation=1 (fast local finger motion)
  Branch 2: kernel=3, dilation=2 (medium-speed motion)
  Branch 3: kernel=3, dilation=4 (slow global arm trajectories)
  Branch 4: 1x1 skip (residual path preserving original signal)

Each branch gets out_channels//4 channels (branch4 absorbs remainder).
GroupNorm inside each branch for domain-invariant temporal features.

Key changes from V43:
  1. MultiScaleTCN replaced with DilatedMultiScaleTCN
  2. Model class: KSLGraphNetV45Expr3
  3. Checkpoint dir: v45_expr3

Usage:
    python train_ksl_v45_expr3.py --model-type numbers
    python train_ksl_v45_expr3.py --model-type words
"""
...
# ---------------------------------------------------------------------------
# Config (v45_expr3 - dilated multi-scale TCN + exp5 base + R&R for numbers)
# ---------------------------------------------------------------------------
...
V45_EXPR3_CONFIG = {
    ...
    "hidden_dim": 64,
    "num_layers": 4,
    "temporal_kernels": [3, 5, 7],  # kept for compatibility, not used by new TCN
    "batch_size": 64,
    "epochs": 500,
    "learning_rate": 1e-3,
    "min_lr": 1e-6,
    ...
    "cutmix_alpha": 1.0,
    "cutmix_prob": 0.3,
    # R&R augmentation (from v43 — numbers only)
    "rr_prob": 0.7,
    "rr_extend_min": 0.05,
    "rr_extend_max": 0.15,
    "rr_crop_min": 0.7,
...
# ---------------------------------------------------------------------------
# Recover-and-Resample Temporal Augmentation (from v43/v41)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Dilated Multi-Scale Temporal Convolution (NEW in V45 Expr3)
# ---------------------------------------------------------------------------

class DilatedMultiScaleTCN(nn.Module):
    """
    4-branch dilated multi-scale temporal convolution:
      Branch 1: kernel=3, dilation=1 (fast local motion — finger transitions)
      Branch 2: kernel=3, dilation=2 (medium motion)
      Branch 3: kernel=3, dilation=4 (slow global motion — arm trajectories)
      Branch 4: 1x1 skip (residual path)
    ...
    def __init__(self, in_channels, out_channels, stride=1, dropout=0.3):
        super().__init__()
        bc = out_channels // 4  # branch channels for branches 1-3
        bc4 = out_channels - 3 * bc  # branch4 absorbs remainder

        self.branch1 = nn.Sequential(
            nn.Conv2d(in_channels, bc, kernel_size=(3, 1),
                      padding=(1, 0), dilation=(1, 1), stride=(stride, 1)),
            nn.GroupNorm(max(1, bc // 8), bc),
            nn.ReLU(),
        )
        ...
        self.branch4 = nn.Sequential(
            nn.Conv2d(in_channels, bc4, kernel_size=(1, 1), stride=(stride, 1)),
            nn.GroupNorm(max(1, bc4 // 8), bc4),
            nn.ReLU(),
        )
...
class STGCNBlock(nn.Module):
    def __init__(self, ic, oc, adj, st=1, dr=0.3, spatial_dropout=0.1):
        super().__init__()
        self.register_buffer("adj", adj)
        self.gcn = GConv(ic, oc)
        self.bn1 = nn.BatchNorm2d(oc)
        # V45 Expr3: DilatedMultiScaleTCN replaces old MultiScaleTCN
        self.tcn = DilatedMultiScaleTCN(oc, oc, st, dr)
        self.residual = (
...
class KSLGraphNetV45Expr3(nn.Module):
    """ST-GCN with Dilated Multi-Scale TCN, aux branch, GRL, SupCon."""
...
def train_stream(stream_name, name, classes, config, train_dir, val_dir,
    ...
    print(f"[{ts()}] TCN: DilatedMultiScaleTCN (dilation 1,2,4 + 1x1 skip)")
    print(f"[{ts()}] {'=' * 70}")
...
def learn_fusion_weights(category_name, classes, config, val_dir, ckpt_dir, device):
    ...
    for sname in streams:
        ...
        model = KSLGraphNetV45Expr3(
            ...
            nl=config["num_layers"], dr=config["dropout"],
...
def main():
    parser = argparse.ArgumentParser(
        description="KSL Training V45 Expr3 - Dilated Multi-Scale TCN",
    ...
    parser.add_argument("--checkpoint-dir", type=str, default=os.path.join(base_data, "checkpoints", "v45_expr3"))
    ...
    print("=" * 70)
    print(f"KSL Training V45 Expr3 - Dilated Multi-Scale TCN + SupCon + R&R")
    print(f"Started: {ts()}")
    ...
    print(f"\nV45 Expr3 Data Split:")
    print(f"  Train dirs: {train_dirs}  (signers 1-12)")
    print(f"  Val dir:    {val_dir}  (signers 13-15)")
    print(f"  Streams: {V45_EXPR3_CONFIG['streams']}")
    print(f"  Batch size: {V45_EXPR3_CONFIG['batch_size']}")
    print(f"  SupCon weight: {V45_EXPR3_CONFIG['supcon_weight']}")
    print(f"  TCN: DilatedMultiScaleTCN (dilation 1,2,4 + 1x1 skip)")
    print(f"  R&R: applied to NUMBERS only")

    print(f"\n[{ts()}] V45 Expr3 Config:")
    print(json.dumps(V45_EXPR3_CONFIG, indent=2, default=str))
    ...
    for cat_name, classes, use_focal, apply_rr in categories:
        ...
        stream_results = {}
        for stream_name in V45_EXPR3_CONFIG["streams"]:
            result = train_stream(
                stream_name, cat_name, classes, V45_EXPR3_CONFIG,
        ...
        fusion_weights, fused_acc = learn_fusion_weights(
            cat_name, classes, V45_EXPR3_CONFIG, val_dir, ckpt_dir, device
        )
    ...
    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] SUMMARY V45 Expr3 - Dilated Multi-Scale TCN")
    print(f"[{ts()}] {'=' * 70}")
    ...
    results_path = os.path.join(
        args.results_dir,
        f"v45_expr3_{args.model_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    )
    results_out = {
        "version": "v45_expr3",
        "model_type": args.model_type,
        "seed": args.seed,
        "config": V45_EXPR3_CONFIG,
        "results": results,
        ...
        "changes_from_v43": [
            "1. MultiScaleTCN replaced with DilatedMultiScaleTCN",
            "2. 4 branches: kernel=3 dilation=1/2/4 + 1x1 skip",
            "3. GroupNorm inside TCN branches (domain-invariant temporal features)",
            "4. Each branch gets out_channels//4 channels",
            "5. Everything else identical to V43 (exp5 arch, BatchNorm spatial, SupCon, R&R numbers only)",
        ],
    }
