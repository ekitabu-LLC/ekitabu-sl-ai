#!/usr/bin/env python3
"""
KSL Training v30 (Alpine) - Phase 2 Training Improvements on v28 Multi-Stream

Based on v28 multi-stream late fusion (joint/bone/velocity). Adds ALL Phase 2
training improvements with command-line flags:

  1. MixStyle (--mixstyle): Mix BN statistics between signers after GCN blocks 1,2
  2. DropGraph (--dropgraph): Graph-aware dropout, drop k-hop neighborhoods
  3. Confused-Pair Contrastive Loss (--confused-pairs): Push confused classes apart
  4. Label Smoothing (--label-smooth): Already in v28, now configurable epsilon
  5. R-Drop (--r-drop): Forward twice, minimize KL divergence between outputs
  6. SWAD (--swad): Stochastic Weight Averaging Densely with overfit-aware schedule
  7. Aggressive Body Augmentation (--aggressive-aug): Extended bone/shoulder/torso aug
  8. Skeleton Noise (--skeleton-noise): Simulate MediaPipe estimation errors
  9. JMDA (--jmda): SpatialMix + TemporalMix augmentation
 10. Shap-Mix (--shap-mix): Class-specific joint importance mixing

Usage:
    python train_ksl_v30.py --model-type both --seed 42
    python train_ksl_v30.py --model-type numbers --no-mixstyle --no-dropgraph
"""

import argparse
import copy
import hashlib
import json
import math
import os
import random
import time
from collections import Counter, defaultdict
from datetime import datetime

import numpy as np
import torch
import torch.nn as nn
import torch.nn.functional as F
from torch.utils.data import Dataset, DataLoader, Sampler

# ---------------------------------------------------------------------------
# Helpers
# ---------------------------------------------------------------------------

def ts():
    return datetime.now().strftime("%Y-%m-%d %H:%M:%S")


def set_seed(seed):
    random.seed(seed)
    np.random.seed(seed)
    torch.manual_seed(seed)
    torch.cuda.manual_seed_all(seed)
    torch.backends.cudnn.deterministic = True
    torch.backends.cudnn.benchmark = False


def check_mediapipe_version():
    try:
        import mediapipe as mp
        version = mp.__version__
        if version != "0.10.5":
            print(f"[{ts()}] WARNING: MediaPipe version is {version}, "
                  f"v30 training data was extracted with 0.10.5.")
        else:
            print(f"[{ts()}] MediaPipe version: {version} (matches training data)")
    except ImportError:
        print(f"[{ts()}] INFO: MediaPipe not installed (not needed for training)")


# ---------------------------------------------------------------------------
# Class Definitions
# ---------------------------------------------------------------------------

NUMBER_CLASSES = sorted([
    "9", "17", "22", "35", "48", "54", "66", "73", "89", "91",
    "100", "125", "268", "388", "444",
])

WORD_CLASSES = sorted([
    "Agreement", "Apple", "Colour", "Friend", "Gift", "Market", "Monday",
    "Picture", "Proud", "Sweater", "Teach", "Tomatoes", "Tortoise", "Twin", "Ugali",
])

# Confused pairs from confusion matrix analysis (v27/v28 results)
NUMBER_CONFUSED_PAIRS = [
    ("66", "100"), ("48", "444"), ("48", "35"), ("89", "35"),
    ("388", "35"), ("9", "89"),
]

WORD_CONFUSED_PAIRS = [
    ("Apple", "Gift"), ("Apple", "Monday"), ("Tomatoes", "Friend"),
    ("Colour", "Friend"), ("Market", "Friend"),
]

# Class-specific joint importance for Shap-Mix (hard-coded from known discriminative joints)
# Higher weight = more important for that class
# Format: class_name -> list of (joint_idx, importance)
# Weak classes get special focus: Apple(1), 48(4), 66(6), Tomatoes(11)
WEAK_NUMBER_CLASSES = {"48", "66", "91", "388"}
WEAK_WORD_CLASSES = {"Apple", "Tomatoes", "Market", "Colour"}

# Important joints per hand region
FINGERTIP_JOINTS = [4, 8, 12, 16, 20, 25, 29, 33, 37, 41]  # LH + RH tips
WRIST_JOINTS = [0, 21]  # LH, RH wrists
POSE_JOINT_INDICES = [42, 43, 44, 45, 46, 47]  # shoulders, elbows, wrists

# ---------------------------------------------------------------------------
# Graph Topology
# ---------------------------------------------------------------------------

HAND_EDGES = [
    (0, 1), (0, 5), (0, 9), (0, 13), (0, 17),
    (1, 2), (2, 3), (3, 4), (5, 6), (6, 7), (7, 8),
    (9, 10), (10, 11), (11, 12), (13, 14), (14, 15), (15, 16),
    (17, 18), (18, 19), (19, 20), (5, 9), (9, 13), (13, 17),
]

POSE_EDGES = [
    (42, 43), (42, 44), (43, 45), (44, 46), (45, 47), (46, 0), (47, 21),
]

POSE_INDICES = [11, 12, 13, 14, 15, 16]

# Parent map for bone computation (48 nodes)
LH_PARENT = [-1, 0, 1, 2, 3, 0, 5, 6, 7, 0, 9, 10, 11, 0, 13, 14, 15, 0, 17, 18, 19]
RH_PARENT = [-1, 21, 22, 23, 24, 21, 26, 27, 28, 21, 30, 31, 32, 21, 34, 35, 36, 21, 38, 39, 40]
POSE_PARENT = [-1, 42, 42, 43, 44, 45]

PARENT_MAP = LH_PARENT + RH_PARENT + POSE_PARENT

# Kinematic chains for bone-length perturbation
LH_CHAINS = [
    [(0, 1), (1, 2), (2, 3), (3, 4)],
    [(0, 5), (5, 6), (6, 7), (7, 8)],
    [(0, 9), (9, 10), (10, 11), (11, 12)],
    [(0, 13), (13, 14), (14, 15), (15, 16)],
    [(0, 17), (17, 18), (18, 19), (19, 20)],
]

RH_CHAINS = [
    [(p + 21, c + 21) for p, c in chain]
    for chain in LH_CHAINS
]

# Joint angle topology
def _build_angle_joints():
    children = defaultdict(list)
    for child_idx, parent_idx in enumerate(PARENT_MAP):
        if parent_idx >= 0:
            children[parent_idx].append(child_idx)
    angle_joints = []
    for node in range(48):
        parent = PARENT_MAP[node]
        if parent < 0:
            continue
        for child in children[node]:
            angle_joints.append((node, parent, child))
    return angle_joints

ANGLE_JOINTS = _build_angle_joints()
NUM_ANGLE_FEATURES = len(ANGLE_JOINTS)

# Fingertip indices
LH_TIPS = [4, 8, 12, 16, 20]
RH_TIPS = [25, 29, 33, 37, 41]
NUM_FINGERTIP_PAIRS = 10
NUM_HAND_BODY_FEATURES = 8

# Build adjacency neighbor map for DropGraph
def _build_neighbor_map(n=48):
    """Build 1-hop neighbor map from graph edges."""
    neighbors = defaultdict(set)
    all_edges = []
    for i, j in HAND_EDGES:
        all_edges.append((i, j))
    for i, j in HAND_EDGES:
        all_edges.append((i + 21, j + 21))
    for i, j in POSE_EDGES:
        all_edges.append((i, j))
    for i, j in all_edges:
        neighbors[i].add(j)
        neighbors[j].add(i)
    # Self-loop
    for i in range(n):
        neighbors[i].add(i)
    return dict(neighbors)

NEIGHBOR_MAP = _build_neighbor_map()

# ---------------------------------------------------------------------------
# Config (v30 - extends v28)
# ---------------------------------------------------------------------------

V30_CONFIG = {
    "max_frames": 90,
    "num_nodes": 48,
    "in_channels": 3,
    "streams": ["joint", "bone", "velocity"],
    "hidden_dim": 64,
    "num_layers": 4,
    "temporal_kernels": [3, 5, 7],
    "batch_size": 32,
    "epochs": 200,
    "learning_rate": 1e-3,
    "min_lr": 1e-6,
    "weight_decay": 1e-3,
    "dropout": 0.3,
    "spatial_dropout": 0.1,
    "label_smoothing": 0.1,
    "patience": 80,
    "warmup_epochs": 10,
    # SupCon
    "supcon_weight": 0.1,
    "supcon_temperature": 0.07,
    # Signer adversarial
    "grl_lambda_max": 0.3,
    "grl_start_epoch": 0,
    "grl_ramp_epochs": 100,
    # Augmentation (base from v28)
    "rotation_prob": 0.5,
    "rotation_max_deg": 15.0,
    "shear_prob": 0.3,
    "shear_max": 0.2,
    "joint_dropout_prob": 0.2,
    "joint_dropout_rate": 0.08,
    "noise_std": 0.015,
    "noise_prob": 0.4,
    "bone_perturb_prob": 0.5,
    "bone_perturb_range": (0.8, 1.2),
    "hand_size_prob": 0.5,
    "hand_size_range": (0.8, 1.2),
    "temporal_warp_prob": 0.4,
    "temporal_warp_sigma": 0.2,
    "hand_dropout_prob": 0.2,
    "hand_dropout_min": 0.1,
    "hand_dropout_max": 0.3,
    "complete_hand_drop_prob": 0.03,
    "mixup_alpha": 0.2,
    "cutmix_alpha": 1.0,
    "cutmix_prob": 0.3,
    # v30 Phase 2 additions
    "confused_pair_weight": 0.1,
    "r_drop_alpha": 1.0,
    # Aggressive augmentation ranges
    "aggressive_bone_range": (0.7, 1.3),
    "shoulder_width_range": (0.8, 1.3),
    "arm_asymmetry_range": (0.9, 1.1),
    "torso_height_range": (0.9, 1.1),
    "camera_distance_range": (0.85, 1.15),
    # Skeleton noise
    "joint_jitter_std": 0.01,
    "joint_swap_prob": 0.02,
    "missing_keypoint_prob": 0.03,
    # JMDA
    "jmda_prob": 0.2,
    # Shap-Mix
    "shap_mix_prob": 0.2,
}

# ---------------------------------------------------------------------------
# Adjacency Matrix Builder
# ---------------------------------------------------------------------------

def build_adj(n=48):
    adj = np.zeros((n, n))
    for i, j in HAND_EDGES:
        adj[i, j] = adj[j, i] = 1
    for i, j in HAND_EDGES:
        adj[i + 21, j + 21] = adj[j + 21, i + 21] = 1
    for i, j in POSE_EDGES:
        adj[i, j] = adj[j, i] = 1
    adj[0, 21] = adj[21, 0] = 0.3
    adj += np.eye(n)
    d = np.sum(adj, axis=1)
    d_inv = np.power(d, -0.5)
    d_inv[np.isinf(d_inv)] = 0
    return torch.FloatTensor(np.diag(d_inv) @ adj @ np.diag(d_inv))

# ---------------------------------------------------------------------------
# Data Deduplication
# ---------------------------------------------------------------------------

def get_file_hash(filepath):
    h = hashlib.md5()
    with open(filepath, 'rb') as f:
        h.update(f.read())
    return h.hexdigest()


def extract_signer_id(filename):
    import re
    base = os.path.splitext(filename)[0]
    parts = base.split("-")
    if len(parts) >= 2:
        return parts[1]
    match = re.search(r'Signer_(\d+)', base)
    if match:
        return match.group(1)
    return "unknown"


def deduplicate_signer_groups(sample_paths):
    groups = defaultdict(list)
    for path in sample_paths:
        class_dir = os.path.basename(os.path.dirname(path))
        signer = extract_signer_id(os.path.basename(path))
        groups[(class_dir, signer)].append(path)

    class_groups = defaultdict(dict)
    for (class_dir, signer), paths in groups.items():
        class_groups[class_dir][signer] = sorted(paths)

    duplicated_signers = set()
    total_removed = 0

    for class_dir, signer_dict in class_groups.items():
        signers = list(signer_dict.keys())
        group_hashes = {}
        for signer in signers:
            file_hashes = tuple(sorted(get_file_hash(p) for p in signer_dict[signer]))
            group_hashes[signer] = file_hashes

        seen = {}
        for signer in signers:
            gh = group_hashes[signer]
            if gh in seen:
                duplicated_signers.add((class_dir, signer))
                total_removed += len(signer_dict[signer])
                print(f"[{ts()}]     Class {class_dir}: Signer {signer} is duplicate of "
                      f"Signer {seen[gh]} ({len(signer_dict[signer])} files)")
            else:
                seen[gh] = signer

    unique = []
    removed = []
    for path in sample_paths:
        class_dir = os.path.basename(os.path.dirname(path))
        signer = extract_signer_id(os.path.basename(path))
        if (class_dir, signer) in duplicated_signers:
            removed.append(path)
        else:
            unique.append(path)

    return unique, removed


# ---------------------------------------------------------------------------
# Wrist-Centric + Palm-Size Normalization
# ---------------------------------------------------------------------------

def normalize_wrist_palm(h):
    T = h.shape[0]

    lh = h[:, :21, :]
    lh_valid = np.abs(lh).sum(axis=(1, 2)) > 0.01
    if np.any(lh_valid):
        lh_wrist = lh[:, 0:1, :]
        lh[lh_valid] = lh[lh_valid] - lh_wrist[lh_valid]
        palm_sizes = np.linalg.norm(lh[lh_valid, 9, :], axis=-1, keepdims=True)
        palm_sizes = np.maximum(palm_sizes, 1e-6)
        lh[lh_valid] = lh[lh_valid] / palm_sizes[:, :, np.newaxis]
    h[:, :21, :] = lh

    rh = h[:, 21:42, :]
    rh_valid = np.abs(rh).sum(axis=(1, 2)) > 0.01
    if np.any(rh_valid):
        rh_wrist = rh[:, 0:1, :]
        rh[rh_valid] = rh[rh_valid] - rh_wrist[rh_valid]
        palm_sizes = np.linalg.norm(rh[rh_valid, 9, :], axis=-1, keepdims=True)
        palm_sizes = np.maximum(palm_sizes, 1e-6)
        rh[rh_valid] = rh[rh_valid] / palm_sizes[:, :, np.newaxis]
    h[:, 21:42, :] = rh

    pose = h[:, 42:48, :]
    pose_valid = np.abs(pose).sum(axis=(1, 2)) > 0.01
    if np.any(pose_valid):
        mid_shoulder = (pose[:, 0:1, :] + pose[:, 1:2, :]) / 2
        pose[pose_valid] = pose[pose_valid] - mid_shoulder[pose_valid]
        shoulder_width = np.linalg.norm(
            pose[pose_valid, 0, :] - pose[pose_valid, 1, :],
            axis=-1, keepdims=True
        )
        shoulder_width = np.maximum(shoulder_width, 1e-6)
        pose[pose_valid] = pose[pose_valid] / shoulder_width[:, :, np.newaxis]
    h[:, 42:48, :] = pose

    return h


# ---------------------------------------------------------------------------
# Augmentation Suite (v28 base + v30 additions)
# ---------------------------------------------------------------------------

def augment_bone_length_perturbation(h, chains, scale_range=(0.8, 1.2)):
    h = h.copy()
    for chain in chains:
        for parent_idx, child_idx in chain:
            scale = np.random.uniform(scale_range[0], scale_range[1])
            bone_vec = h[:, child_idx, :] - h[:, parent_idx, :]
            new_pos = h[:, parent_idx, :] + bone_vec * scale
            displacement = new_pos - h[:, child_idx, :]
            h[:, child_idx, :] = new_pos
            found = False
            for p2, c2 in chain:
                if found:
                    h[:, c2, :] += displacement
                if (p2, c2) == (parent_idx, child_idx):
                    found = True
    return h


def augment_hand_size(h, scale_range=(0.8, 1.2)):
    h = h.copy()
    scale = np.random.uniform(scale_range[0], scale_range[1])
    lh_valid = np.abs(h[:, :21, :]).sum(axis=(1, 2)) > 0.01
    if np.any(lh_valid):
        wrist = h[:, 0:1, :]
        h[np.ix_(lh_valid, range(1, 21))] = (
            wrist[lh_valid] + (h[np.ix_(lh_valid, range(1, 21))] - wrist[lh_valid]) * scale
        )
    rh_valid = np.abs(h[:, 21:42, :]).sum(axis=(1, 2)) > 0.01
    if np.any(rh_valid):
        wrist = h[:, 21:22, :]
        h[np.ix_(rh_valid, range(22, 42))] = (
            wrist[rh_valid] + (h[np.ix_(rh_valid, range(22, 42))] - wrist[rh_valid]) * scale
        )
    return h


def augment_temporal_warp(data_list, sigma=0.2):
    T = data_list[0].shape[0]
    if T < 2:
        return data_list
    warp = np.cumsum(np.abs(np.random.normal(1.0, sigma, T)))
    warp = warp / warp[-1] * (T - 1)
    indices = np.clip(np.round(warp).astype(int), 0, T - 1)
    return [arr[indices] for arr in data_list]


def augment_rotation(h, max_deg=15.0):
    h = h.copy()
    angle = np.radians(np.random.uniform(-max_deg, max_deg))
    cos_a, sin_a = np.cos(angle), np.sin(angle)
    rot = np.array([
        [cos_a, -sin_a, 0],
        [sin_a,  cos_a, 0],
        [0,      0,     1],
    ], dtype=np.float32)

    lh_valid = np.abs(h[:, :21, :]).sum(axis=(1, 2)) > 0.01
    if np.any(lh_valid):
        wrist = h[lh_valid, 0:1, :]
        centered = h[lh_valid, :21, :] - wrist
        rotated = np.einsum('ij,ntj->nti', rot, centered)
        h[lh_valid, :21, :] = rotated + wrist

    rh_valid = np.abs(h[:, 21:42, :]).sum(axis=(1, 2)) > 0.01
    if np.any(rh_valid):
        wrist = h[rh_valid, 21:22, :]
        centered = h[rh_valid, 21:42, :] - wrist
        rotated = np.einsum('ij,ntj->nti', rot, centered)
        h[rh_valid, 21:42, :] = rotated + wrist

    return h


def augment_shear(h, max_shear=0.2):
    h = h.copy()
    s = np.random.uniform(-max_shear, max_shear)
    axis_pairs = [(0, 1), (0, 2), (1, 0), (1, 2), (2, 0), (2, 1)]
    src_axis, dst_axis = axis_pairs[np.random.randint(len(axis_pairs))]
    h[:, :, dst_axis] = h[:, :, dst_axis] + s * h[:, :, src_axis]
    return h


def augment_joint_dropout(h, dropout_rate=0.08):
    h = h.copy()
    n_nodes = h.shape[1]
    n_drop = max(1, int(n_nodes * dropout_rate))
    drop_indices = np.random.choice(n_nodes, n_drop, replace=False)
    h[:, drop_indices, :] = 0
    return h


# --- v30 NEW: Aggressive Body Augmentation ---

def augment_aggressive_body(h, config):
    """Extended body proportion augmentation (v30).

    Adds shoulder width scaling, arm length asymmetry, torso height variation,
    and camera distance simulation on top of standard bone perturbation.
    """
    h = h.copy()

    # 1. Shoulder width scaling
    sw_range = config["shoulder_width_range"]
    sw_scale = np.random.uniform(sw_range[0], sw_range[1])
    pose = h[:, 42:48, :]
    pose_valid = np.abs(pose).sum(axis=(1, 2)) > 0.01
    if np.any(pose_valid):
        mid = (pose[pose_valid, 0, :] + pose[pose_valid, 1, :]) / 2
        # Scale shoulders relative to midpoint
        for si in [0, 1]:  # left/right shoulder
            pose[pose_valid, si, :] = mid + (pose[pose_valid, si, :] - mid) * sw_scale
        h[:, 42:48, :] = pose

    # 2. Arm length asymmetry (left/right independent scaling)
    arm_range = config["arm_asymmetry_range"]
    for side, elbow_idx, wrist_idx, shoulder_idx in [(0, 44, 46, 42), (1, 45, 47, 43)]:
        arm_scale = np.random.uniform(arm_range[0], arm_range[1])
        valid = np.abs(h[:, shoulder_idx, :]).sum(axis=1) > 0.01
        if np.any(valid):
            shoulder = h[valid, shoulder_idx:shoulder_idx+1, :]
            for joint_idx in [elbow_idx, wrist_idx]:
                h[valid, joint_idx, :] = shoulder[:, 0, :] + \
                    (h[valid, joint_idx, :] - shoulder[:, 0, :]) * arm_scale

    # 3. Torso height variation
    th_range = config["torso_height_range"]
    th_scale = np.random.uniform(th_range[0], th_range[1])
    pose = h[:, 42:48, :]
    pose_valid = np.abs(pose).sum(axis=(1, 2)) > 0.01
    if np.any(pose_valid):
        # Scale y-coordinates of pose joints relative to mid-shoulder
        mid_y = (pose[pose_valid, 0, 1] + pose[pose_valid, 1, 1]) / 2
        for pi in range(6):
            pose[pose_valid, pi, 1] = mid_y + (pose[pose_valid, pi, 1] - mid_y) * th_scale
        h[:, 42:48, :] = pose

    # 4. Camera distance simulation (uniform scaling)
    cd_range = config["camera_distance_range"]
    cd_scale = np.random.uniform(cd_range[0], cd_range[1])
    h = h * cd_scale

    return h


# --- v30 NEW: Skeleton Noise (simulate MediaPipe errors) ---

def augment_skeleton_noise(h, config):
    """Simulate MediaPipe estimation errors (v30).

    - Joint jitter: Gaussian noise proportional to confidence
    - Occasional joint swap: swap adjacent joint positions
    - Missing keypoint simulation: zero out keypoints
    """
    h = h.copy()
    T, N, C = h.shape

    # 1. Joint jitter: add Gaussian noise
    jitter_std = config["joint_jitter_std"]
    # More noise for extremity joints (fingers), less for wrists/pose
    noise_scale = np.ones(N, dtype=np.float32)
    # Fingertips get more noise (lower confidence in MediaPipe)
    for tip in FINGERTIP_JOINTS:
        noise_scale[tip] = 2.0
    noise = np.random.normal(0, jitter_std, h.shape).astype(np.float32)
    noise *= noise_scale[np.newaxis, :, np.newaxis]
    h = h + noise

    # 2. Occasional joint swap (swap with adjacent joint)
    swap_prob = config["joint_swap_prob"]
    for t in range(T):
        if np.random.random() < swap_prob:
            # Pick a random node that has a neighbor
            node = np.random.randint(0, N)
            neighbors = list(NEIGHBOR_MAP.get(node, set()) - {node})
            if neighbors:
                neighbor = np.random.choice(neighbors)
                h[t, node, :], h[t, neighbor, :] = h[t, neighbor, :].copy(), h[t, node, :].copy()

    # 3. Missing keypoint simulation
    missing_prob = config["missing_keypoint_prob"]
    for t in range(T):
        for n in range(N):
            if np.random.random() < missing_prob:
                h[t, n, :] = 0

    return h


# --- v30 NEW: JMDA (SpatialMix + TemporalMix) ---

def jmda_spatial_mix(h1, h2, alpha=0.5):
    """SpatialMix: Mix spatial coordinates between two samples.

    Project 3D coords to 2D mixing weights, blend joint positions.
    """
    T, N, C = h1.shape
    # Generate per-joint mixing weights
    lam = np.random.beta(alpha, alpha)
    # Random binary mask per joint (which joints come from sample 1 vs 2)
    joint_mask = np.random.random(N) < lam
    mixed = h1.copy()
    mixed[:, ~joint_mask, :] = h2[:, ~joint_mask, :]
    return mixed, lam


def jmda_temporal_mix(h1, h2, alpha=0.5):
    """TemporalMix: Temporal resize and merge two sequences.

    Take first portion from h1, second from h2, resize to original length.
    """
    T = h1.shape[0]
    lam = np.random.beta(alpha, alpha)
    cut_point = max(1, int(T * lam))

    # Concatenate portions
    mixed = np.concatenate([h1[:cut_point], h2[cut_point:]], axis=0)

    # Resize back to T if needed (should already be T)
    if mixed.shape[0] != T:
        indices = np.linspace(0, mixed.shape[0] - 1, T, dtype=int)
        mixed = mixed[indices]

    return mixed, lam


# --- v30 NEW: Shap-Mix ---

def shap_mix(h1, h2, label1, label2, classes, weak_classes):
    """Shap-Mix: Mix non-important joints between samples.

    Keeps important joints from weak-class samples, mixes the rest.
    Uses class-specific joint importance (hard-coded).
    """
    T, N, C = h1.shape
    class_name_1 = classes[label1] if isinstance(label1, int) else label1
    class_name_2 = classes[label2] if isinstance(label2, int) else label2

    # Important joints: fingertips + wrists for all classes
    important_joints = set(FINGERTIP_JOINTS + WRIST_JOINTS)

    # If either sample is a weak class, keep its important joints intact
    if class_name_1 in weak_classes:
        # Keep all important joints from sample 1, mix the rest
        lam = 0.7
        mixed = h1.copy()
        non_important = [j for j in range(N) if j not in important_joints]
        if non_important:
            mix_mask = np.random.random(len(non_important)) < (1 - lam)
            for i, j in enumerate(non_important):
                if mix_mask[i]:
                    mixed[:, j, :] = h2[:, j, :]
        return mixed, lam
    else:
        # Standard mix with random lambda
        lam = np.random.beta(0.5, 0.5)
        mixed = lam * h1 + (1 - lam) * h2
        return mixed, lam


# ---------------------------------------------------------------------------
# Feature Computation
# ---------------------------------------------------------------------------

def compute_bones(h):
    bones = np.zeros_like(h)
    for child in range(48):
        parent = PARENT_MAP[child]
        if parent >= 0:
            bones[:, child, :] = h[:, child, :] - h[:, parent, :]
    return bones


def compute_joint_angles(h):
    T = h.shape[0]
    angles = np.zeros((T, NUM_ANGLE_FEATURES), dtype=np.float32)
    for idx, (node, parent, child) in enumerate(ANGLE_JOINTS):
        bone_in = h[:, node, :] - h[:, parent, :]
        bone_out = h[:, child, :] - h[:, node, :]
        norm_in = np.linalg.norm(bone_in, axis=-1, keepdims=True)
        norm_out = np.linalg.norm(bone_out, axis=-1, keepdims=True)
        norm_in = np.maximum(norm_in, 1e-8)
        norm_out = np.maximum(norm_out, 1e-8)
        bone_in_n = bone_in / norm_in
        bone_out_n = bone_out / norm_out
        dot = np.sum(bone_in_n * bone_out_n, axis=-1)
        dot = np.clip(dot, -1.0, 1.0)
        angles[:, idx] = np.arccos(dot)
    return angles


def compute_fingertip_distances(h):
    T = h.shape[0]
    distances = np.zeros((T, 2 * NUM_FINGERTIP_PAIRS), dtype=np.float32)
    col = 0
    for i in range(len(LH_TIPS)):
        for j in range(i + 1, len(LH_TIPS)):
            diff = h[:, LH_TIPS[i], :] - h[:, LH_TIPS[j], :]
            distances[:, col] = np.linalg.norm(diff, axis=-1)
            col += 1
    for i in range(len(RH_TIPS)):
        for j in range(i + 1, len(RH_TIPS)):
            diff = h[:, RH_TIPS[i], :] - h[:, RH_TIPS[j], :]
            distances[:, col] = np.linalg.norm(diff, axis=-1)
            col += 1
    return distances


def compute_hand_body_features(h_raw):
    T = h_raw.shape[0]
    features = np.zeros((T, NUM_HAND_BODY_FEATURES), dtype=np.float32)
    mid_shoulder = (h_raw[:, 42, :] + h_raw[:, 43, :]) / 2
    shoulder_width = np.linalg.norm(
        h_raw[:, 42, :] - h_raw[:, 43, :], axis=-1, keepdims=True
    )
    shoulder_width = np.maximum(shoulder_width, 1e-6)
    lh_centroid = h_raw[:, :21, :].mean(axis=1)
    rh_centroid = h_raw[:, 21:42, :].mean(axis=1)
    features[:, 0] = (lh_centroid[:, 1] - mid_shoulder[:, 1]) / shoulder_width[:, 0]
    features[:, 1] = (rh_centroid[:, 1] - mid_shoulder[:, 1]) / shoulder_width[:, 0]
    features[:, 2] = (lh_centroid[:, 0] - mid_shoulder[:, 0]) / shoulder_width[:, 0]
    features[:, 3] = (rh_centroid[:, 0] - mid_shoulder[:, 0]) / shoulder_width[:, 0]
    features[:, 4] = np.linalg.norm(
        lh_centroid - rh_centroid, axis=-1
    ) / shoulder_width[:, 0]
    face_approx = mid_shoulder.copy()
    face_approx[:, 1] -= shoulder_width[:, 0] * 0.7
    features[:, 5] = np.linalg.norm(
        lh_centroid - face_approx, axis=-1
    ) / shoulder_width[:, 0]
    features[:, 6] = np.linalg.norm(
        rh_centroid - face_approx, axis=-1
    ) / shoulder_width[:, 0]
    features[:, 7] = np.abs(lh_centroid[:, 1] - rh_centroid[:, 1]) / shoulder_width[:, 0]
    return features


# ---------------------------------------------------------------------------
# Signer-Balanced Batch Sampler
# ---------------------------------------------------------------------------

class SignerBalancedSampler(Sampler):
    def __init__(self, labels, signer_labels, batch_size, drop_last=True):
        self.labels = np.array(labels)
        self.signer_labels = np.array(signer_labels)
        self.batch_size = batch_size
        self.drop_last = drop_last
        self.class_signer_indices = defaultdict(lambda: defaultdict(list))
        for idx in range(len(self.labels)):
            cls = self.labels[idx]
            signer = self.signer_labels[idx]
            self.class_signer_indices[cls][signer].append(idx)
        self.classes = sorted(self.class_signer_indices.keys())
        self.num_samples = len(self.labels)

    def __iter__(self):
        all_indices = []
        classes_per_batch = min(len(self.classes), max(4, self.batch_size // 4))
        samples_per_class = self.batch_size // classes_per_batch
        class_order = self.classes.copy()
        num_batches = self.num_samples // self.batch_size

        for _ in range(num_batches):
            batch = []
            random.shuffle(class_order)
            selected_classes = class_order[:classes_per_batch]
            for cls in selected_classes:
                signer_dict = self.class_signer_indices[cls]
                signers = list(signer_dict.keys())
                picked = []
                signer_cycle = signers.copy()
                random.shuffle(signer_cycle)
                si = 0
                while len(picked) < samples_per_class:
                    signer = signer_cycle[si % len(signer_cycle)]
                    indices = signer_dict[signer]
                    if indices:
                        chosen = random.choice(indices)
                        picked.append(chosen)
                    si += 1
                    if si >= samples_per_class * 3:
                        break
                batch.extend(picked)
            if len(batch) >= self.batch_size:
                batch = batch[:self.batch_size]
            else:
                remaining = self.batch_size - len(batch)
                extra = random.choices(range(self.num_samples), k=remaining)
                batch.extend(extra)
            random.shuffle(batch)
            all_indices.extend(batch)
        return iter(all_indices)

    def __len__(self):
        return (self.num_samples // self.batch_size) * self.batch_size


# ---------------------------------------------------------------------------
# Temporal CutMix
# ---------------------------------------------------------------------------

def temporal_cutmix(gcn_input, aux_input, labels, signer_labels, alpha=1.0):
    B, C, T, N = gcn_input.shape
    lam = np.random.beta(alpha, alpha)
    cut_len = int(T * (1 - lam))
    if cut_len == 0:
        return gcn_input, aux_input, labels, labels, 1.0

    cut_start = np.random.randint(0, T - cut_len + 1)
    indices = torch.randperm(B, device=gcn_input.device)

    gcn_mixed = gcn_input.clone()
    aux_mixed = aux_input.clone()

    gcn_mixed[:, :, cut_start:cut_start+cut_len, :] = gcn_input[indices, :, cut_start:cut_start+cut_len, :]
    aux_mixed[:, cut_start:cut_start+cut_len, :] = aux_input[indices, cut_start:cut_start+cut_len, :]

    lam = 1.0 - cut_len / T
    labels_b = labels[indices]

    return gcn_mixed, aux_mixed, labels, labels_b, lam


# ---------------------------------------------------------------------------
# Multi-Stream Dataset (v30 - adds skeleton noise, aggressive aug, JMDA, Shap-Mix)
# ---------------------------------------------------------------------------

class KSLMultiStreamDataset(Dataset):
    def __init__(self, data_dirs, classes, config, aug=False,
                 use_aggressive_aug=False, use_skeleton_noise=False,
                 use_jmda=False, use_shap_mix=False):
        self.mf = config["max_frames"]
        self.aug = aug
        self.config = config
        self.c2i = {c: i for i, c in enumerate(classes)}
        self.classes = classes
        self.use_aggressive_aug = use_aggressive_aug and aug
        self.use_skeleton_noise = use_skeleton_noise and aug
        self.use_jmda = use_jmda and aug
        self.use_shap_mix = use_shap_mix and aug
        self.weak_classes = WEAK_NUMBER_CLASSES if classes == NUMBER_CLASSES else WEAK_WORD_CLASSES

        if isinstance(data_dirs, str):
            data_dirs = [data_dirs]

        all_paths = []
        all_labels = []
        for data_dir in data_dirs:
            for cn in classes:
                cd = os.path.join(data_dir, cn)
                if os.path.exists(cd):
                    for fn in sorted(os.listdir(cd)):
                        if fn.endswith(".npy"):
                            all_paths.append(os.path.join(cd, fn))
                            all_labels.append(self.c2i[cn])

        total_before = len(all_paths)

        if aug:
            unique_paths, removed = deduplicate_signer_groups(all_paths)
            if removed:
                print(f"[{ts()}]   Deduplication: {total_before} -> "
                      f"{len(unique_paths)} samples ({len(removed)} removed)")
            unique_set = set(unique_paths)
            self.samples = []
            self.labels = []
            for path, label in zip(all_paths, all_labels):
                if path in unique_set:
                    self.samples.append(path)
                    self.labels.append(label)
                    unique_set.discard(path)
        else:
            self.samples = all_paths
            self.labels = all_labels

        self.signer_to_idx = {}
        self.signer_labels = []
        for path in self.samples:
            signer = extract_signer_id(os.path.basename(path))
            if signer not in self.signer_to_idx:
                self.signer_to_idx[signer] = len(self.signer_to_idx)
            self.signer_labels.append(self.signer_to_idx[signer])

        self.num_signers = len(self.signer_to_idx)

        dirs_str = ", ".join(data_dirs)
        print(f"[{ts()}]   Loaded {len(self.samples)} samples from {dirs_str}")
        print(f"[{ts()}]   Signers: {self.num_signers} unique "
              f"({dict(Counter(extract_signer_id(os.path.basename(p)) for p in self.samples))})")

    def _load_skeleton(self, idx):
        """Load and preprocess a single skeleton sequence (shared logic)."""
        d = np.load(self.samples[idx])
        f = d.shape[0]

        if d.shape[1] >= 225:
            pose = np.zeros((f, 6, 3), dtype=np.float32)
            for pi, idx_pose in enumerate(POSE_INDICES):
                start = idx_pose * 3
                pose[:, pi, :] = d[:, start:start + 3]
            lh = d[:, 99:162].reshape(f, 21, 3)
            rh = d[:, 162:225].reshape(f, 21, 3)
        else:
            pose = np.zeros((f, 6, 3), dtype=np.float32)
            lh = np.zeros((f, 21, 3), dtype=np.float32)
            rh = np.zeros((f, 21, 3), dtype=np.float32)

        h = np.concatenate([lh, rh, pose], axis=1).astype(np.float32)
        return h

    def __len__(self):
        return len(self.samples)

    def __getitem__(self, idx):
        h = self._load_skeleton(idx)
        f = h.shape[0]

        # Augmentation: hand dropout
        if self.aug and np.random.random() < self.config["hand_dropout_prob"]:
            dropout_rate = np.random.uniform(
                self.config["hand_dropout_min"], self.config["hand_dropout_max"]
            )
            if np.random.random() < 0.6:
                lh_mask = np.random.random(f) > dropout_rate
                h[~lh_mask, :21, :] = 0
            if np.random.random() < 0.6:
                rh_mask = np.random.random(f) > dropout_rate
                h[~rh_mask, 21:42, :] = 0

        # Augmentation: complete hand drop
        if self.aug and np.random.random() < self.config["complete_hand_drop_prob"]:
            if np.random.random() < 0.5:
                h[:, :21, :] = 0
            else:
                h[:, 21:42, :] = 0

        # Augmentation: hand swap
        if self.aug and np.random.random() < 0.3:
            h_copy = h.copy()
            h[:, :21, :] = h_copy[:, 21:42, :]
            h[:, 21:42, :] = h_copy[:, :21, :]
            h[:, :42, 0] = -h[:, :42, 0]
            h[:, 42:48, 0] = -h[:, 42:48, 0]

        # v30 NEW: Skeleton noise (before normalization)
        if self.use_skeleton_noise and np.random.random() < 0.5:
            h = augment_skeleton_noise(h, self.config)

        # Hand-body features BEFORE normalization
        hand_body_feats = compute_hand_body_features(h)

        # Normalization
        h = normalize_wrist_palm(h)

        # v30 NEW: Aggressive body augmentation (after normalization, affects proportions)
        if self.use_aggressive_aug and np.random.random() < 0.5:
            h = augment_aggressive_body(h, self.config)

        # Augmentation: bone-length perturbation (use aggressive range if enabled)
        if self.aug and np.random.random() < self.config["bone_perturb_prob"]:
            bone_range = self.config["aggressive_bone_range"] if self.use_aggressive_aug \
                else self.config["bone_perturb_range"]
            h = augment_bone_length_perturbation(
                h, LH_CHAINS + RH_CHAINS, scale_range=bone_range
            )

        if self.aug and np.random.random() < self.config["hand_size_prob"]:
            h = augment_hand_size(h, scale_range=self.config["hand_size_range"])

        if self.aug and np.random.random() < self.config["rotation_prob"]:
            h = augment_rotation(h, max_deg=self.config["rotation_max_deg"])

        if self.aug and np.random.random() < self.config["shear_prob"]:
            h = augment_shear(h, max_shear=self.config["shear_max"])

        if self.aug and np.random.random() < self.config["joint_dropout_prob"]:
            h = augment_joint_dropout(h, dropout_rate=self.config["joint_dropout_rate"])

        if self.aug and np.random.random() < 0.5:
            h = h * np.random.uniform(0.8, 1.2)

        if self.aug and np.random.random() < self.config["noise_prob"]:
            noise = np.random.normal(0, self.config["noise_std"], h.shape).astype(np.float32)
            h = h + noise

        # v30 NEW: JMDA (SpatialMix + TemporalMix)
        jmda_applied = False
        jmda_lam = 1.0
        jmda_label_b = self.labels[idx]
        if self.use_jmda and np.random.random() < self.config["jmda_prob"]:
            other_idx = np.random.randint(0, len(self.samples))
            h2 = self._load_skeleton(other_idx)
            h2 = normalize_wrist_palm(h2)
            # Resize h2 to match h's current frame count
            if h2.shape[0] != h.shape[0]:
                indices_r = np.linspace(0, h2.shape[0] - 1, h.shape[0], dtype=int)
                h2 = h2[indices_r]
            if np.random.random() < 0.5:
                h, jmda_lam = jmda_spatial_mix(h, h2)
            else:
                h, jmda_lam = jmda_temporal_mix(h, h2)
            jmda_applied = True
            jmda_label_b = self.labels[other_idx]

        # v30 NEW: Shap-Mix
        if not jmda_applied and self.use_shap_mix and np.random.random() < self.config["shap_mix_prob"]:
            other_idx = np.random.randint(0, len(self.samples))
            h2 = self._load_skeleton(other_idx)
            h2 = normalize_wrist_palm(h2)
            if h2.shape[0] != h.shape[0]:
                indices_r = np.linspace(0, h2.shape[0] - 1, h.shape[0], dtype=int)
                h2 = h2[indices_r]
            h, jmda_lam = shap_mix(h, h2, self.labels[idx], self.labels[other_idx],
                                    self.classes, self.weak_classes)
            jmda_applied = True
            jmda_label_b = self.labels[other_idx]

        # Compute velocity BEFORE resampling
        velocity = np.zeros_like(h)
        velocity[1:] = h[1:] - h[:-1]

        # Compute bone features
        bones = compute_bones(h)

        # Compute auxiliary features
        joint_angles = compute_joint_angles(h)
        fingertip_dists = compute_fingertip_distances(h)

        # Augmentation: temporal warping
        if self.aug and np.random.random() < self.config["temporal_warp_prob"]:
            h, velocity, bones, joint_angles, fingertip_dists, hand_body_feats = augment_temporal_warp(
                [h, velocity, bones, joint_angles, fingertip_dists, hand_body_feats],
                sigma=self.config["temporal_warp_sigma"]
            )

        # Temporal sampling / padding
        f = h.shape[0]
        if f >= self.mf:
            indices = np.linspace(0, f - 1, self.mf, dtype=int)
            h = h[indices]
            velocity = velocity[indices]
            bones = bones[indices]
            joint_angles = joint_angles[indices]
            fingertip_dists = fingertip_dists[indices]
            hand_body_feats = hand_body_feats[indices]
        else:
            pad_len = self.mf - f
            h = np.concatenate([h, np.zeros((pad_len, 48, 3), dtype=np.float32)])
            velocity = np.concatenate([velocity, np.zeros((pad_len, 48, 3), dtype=np.float32)])
            bones = np.concatenate([bones, np.zeros((pad_len, 48, 3), dtype=np.float32)])
            joint_angles = np.concatenate([joint_angles, np.zeros((pad_len, NUM_ANGLE_FEATURES), dtype=np.float32)])
            fingertip_dists = np.concatenate([fingertip_dists, np.zeros((pad_len, 2 * NUM_FINGERTIP_PAIRS), dtype=np.float32)])
            hand_body_feats = np.concatenate([hand_body_feats, np.zeros((pad_len, NUM_HAND_BODY_FEATURES), dtype=np.float32)])

        # Augmentation: temporal shift
        if self.aug and np.random.random() < 0.3:
            shift = np.random.randint(-8, 9)
            if shift > 0:
                z3 = np.zeros((shift, 48, 3), dtype=np.float32)
                h = np.concatenate([z3, h[:-shift]], axis=0)
                velocity = np.concatenate([z3, velocity[:-shift]], axis=0)
                bones = np.concatenate([z3, bones[:-shift]], axis=0)
                za = np.zeros((shift, NUM_ANGLE_FEATURES), dtype=np.float32)
                zd = np.zeros((shift, 2 * NUM_FINGERTIP_PAIRS), dtype=np.float32)
                zh = np.zeros((shift, NUM_HAND_BODY_FEATURES), dtype=np.float32)
                joint_angles = np.concatenate([za, joint_angles[:-shift]], axis=0)
                fingertip_dists = np.concatenate([zd, fingertip_dists[:-shift]], axis=0)
                hand_body_feats = np.concatenate([zh, hand_body_feats[:-shift]], axis=0)
            elif shift < 0:
                z3 = np.zeros((-shift, 48, 3), dtype=np.float32)
                h = np.concatenate([h[-shift:], z3], axis=0)
                velocity = np.concatenate([velocity[-shift:], z3], axis=0)
                bones = np.concatenate([bones[-shift:], z3], axis=0)
                za = np.zeros((-shift, NUM_ANGLE_FEATURES), dtype=np.float32)
                zd = np.zeros((-shift, 2 * NUM_FINGERTIP_PAIRS), dtype=np.float32)
                zh = np.zeros((-shift, NUM_HAND_BODY_FEATURES), dtype=np.float32)
                joint_angles = np.concatenate([joint_angles[-shift:], za], axis=0)
                fingertip_dists = np.concatenate([fingertip_dists[-shift:], zd], axis=0)
                hand_body_feats = np.concatenate([hand_body_feats[-shift:], zh], axis=0)

        # --- Build per-stream GCN tensors (3, T, N) ---
        h_clipped = np.clip(h, -10, 10).astype(np.float32)
        bones_clipped = np.clip(bones, -10, 10).astype(np.float32)
        velocity_clipped = np.clip(velocity, -10, 10).astype(np.float32)

        streams = {
            "joint": torch.FloatTensor(h_clipped).permute(2, 0, 1),
            "bone": torch.FloatTensor(bones_clipped).permute(2, 0, 1),
            "velocity": torch.FloatTensor(velocity_clipped).permute(2, 0, 1),
        }

        # Auxiliary features: (T, D_aux)
        aux_features = np.concatenate([joint_angles, fingertip_dists, hand_body_feats], axis=1)
        aux_features = np.clip(aux_features, -10, 10).astype(np.float32)
        aux_tensor = torch.FloatTensor(aux_features)

        # Return extra info for JMDA/Shap-Mix mixing
        return streams, aux_tensor, self.labels[idx], self.signer_labels[idx], jmda_label_b, jmda_lam


def stream_collate_fn(batch):
    streams_list, aux_list, labels_list, signer_list, jmda_labels_list, jmda_lam_list = zip(*batch)
    stream_names = list(streams_list[0].keys())
    streams_batch = {
        name: torch.stack([s[name] for s in streams_list])
        for name in stream_names
    }
    aux_batch = torch.stack(aux_list)
    labels = torch.tensor(labels_list, dtype=torch.long)
    signers = torch.tensor(signer_list, dtype=torch.long)
    jmda_labels = torch.tensor(jmda_labels_list, dtype=torch.long)
    jmda_lams = torch.tensor(jmda_lam_list, dtype=torch.float32)
    return streams_batch, aux_batch, labels, signers, jmda_labels, jmda_lams


# ---------------------------------------------------------------------------
# MixStyle Module (v30 NEW)
# ---------------------------------------------------------------------------

class MixStyle(nn.Module):
    """MixStyle: mix instance-level BN statistics between samples (Zhou et al., ICLR 2021)."""
    def __init__(self, p=0.5, alpha=0.1):
        super().__init__()
        self.p = p
        self.beta = torch.distributions.Beta(alpha, alpha)

    def forward(self, x):
        if not self.training or random.random() > self.p:
            return x
        B = x.size(0)
        mu = x.mean(dim=[2, 3], keepdim=True)
        var = x.var(dim=[2, 3], keepdim=True)
        sig = (var + 1e-6).sqrt()
        x_normed = (x - mu) / sig
        perm = torch.randperm(B)
        lmbd = self.beta.sample([B, 1, 1, 1]).to(x.device)
        mu_mix = lmbd * mu + (1 - lmbd) * mu[perm]
        sig_mix = lmbd * sig + (1 - lmbd) * sig[perm]
        return x_normed * sig_mix + mu_mix


# ---------------------------------------------------------------------------
# DropGraph Module (v30 NEW)
# ---------------------------------------------------------------------------

class DropGraph(nn.Module):
    """Graph-aware dropout: drop a node and its 1-hop neighbors (Cheng et al., ECCV 2020)."""
    def __init__(self, p=0.1, num_nodes=48):
        super().__init__()
        self.p = p
        self.num_nodes = num_nodes

    def forward(self, x):
        if not self.training or self.p == 0:
            return x
        B, C, T, N = x.shape
        # For each sample, decide whether to drop a graph neighborhood
        mask = torch.ones(B, 1, 1, N, device=x.device)
        for b in range(B):
            if random.random() < self.p:
                # Select a random seed node
                seed = random.randint(0, N - 1)
                # Get 1-hop neighbors
                neighbors = NEIGHBOR_MAP.get(seed, {seed})
                for n in neighbors:
                    if n < N:
                        mask[b, 0, 0, n] = 0
        # Scale by keep ratio to maintain expected values
        keep_ratio = mask.mean()
        if keep_ratio > 0:
            return x * mask / keep_ratio
        return x * mask


# ---------------------------------------------------------------------------
# Multi-Scale Temporal Convolution
# ---------------------------------------------------------------------------

class MultiScaleTCN(nn.Module):
    def __init__(self, channels, kernels=(3, 5, 7), stride=1, dropout=0.3):
        super().__init__()
        self.branches = nn.ModuleList([
            nn.Sequential(
                nn.Conv2d(channels, channels, (k, 1), padding=(k // 2, 0), stride=(stride, 1)),
                nn.BatchNorm2d(channels),
            )
            for k in kernels
        ])
        self.dropout = nn.Dropout(dropout)

    def forward(self, x):
        out = sum(branch(x) for branch in self.branches) / len(self.branches)
        return self.dropout(out)


# ---------------------------------------------------------------------------
# Attention Pooling
# ---------------------------------------------------------------------------

class AttentionPool(nn.Module):
    def __init__(self, in_channels, num_heads=2):
        super().__init__()
        self.num_heads = num_heads
        mid = in_channels // 2
        self.heads = nn.ModuleList([
            nn.Sequential(
                nn.Linear(in_channels, mid),
                nn.Tanh(),
                nn.Linear(mid, 1),
            )
            for _ in range(num_heads)
        ])

    def forward(self, x):
        b, c, t = x.shape
        x_t = x.permute(0, 2, 1)
        outs = []
        for head in self.heads:
            attn_logits = head(x_t)
            attn_weights = F.softmax(attn_logits, dim=1)
            pooled = (x_t * attn_weights).sum(dim=1)
            outs.append(pooled)
        return torch.cat(outs, dim=1)


# ---------------------------------------------------------------------------
# Supervised Contrastive Loss
# ---------------------------------------------------------------------------

class SupConLoss(nn.Module):
    def __init__(self, temperature=0.07):
        super().__init__()
        self.temperature = temperature

    def forward(self, features, labels):
        device = features.device
        B = features.shape[0]
        if B <= 1:
            return torch.tensor(0.0, device=device)
        features = F.normalize(features, dim=1)
        similarity = torch.matmul(features, features.T) / self.temperature
        mask = (labels.unsqueeze(0) == labels.unsqueeze(1)).float()
        self_mask = torch.eye(B, device=device)
        mask = mask * (1 - self_mask)
        pos_count = mask.sum(1)
        if (pos_count == 0).all():
            return torch.tensor(0.0, device=device)
        logits_max, _ = similarity.max(dim=1, keepdim=True)
        logits = similarity - logits_max.detach()
        exp_logits = torch.exp(logits) * (1 - self_mask)
        log_prob = logits - torch.log(exp_logits.sum(1, keepdim=True) + 1e-8)
        mean_log_prob = (mask * log_prob).sum(1) / pos_count.clamp(min=1)
        return -mean_log_prob.mean()


# ---------------------------------------------------------------------------
# Confused-Pair Contrastive Loss (v30 NEW)
# ---------------------------------------------------------------------------

class ConfusedPairLoss(nn.Module):
    """Push embeddings of confused class pairs apart in embedding space."""
    def __init__(self, confused_pairs, c2i, temperature=0.1, margin=1.0):
        super().__init__()
        self.temperature = temperature
        self.margin = margin
        # Convert class name pairs to index pairs
        self.pair_indices = []
        for c1, c2 in confused_pairs:
            if c1 in c2i and c2 in c2i:
                self.pair_indices.append((c2i[c1], c2i[c2]))

    def forward(self, embeddings, labels):
        if not self.pair_indices:
            return torch.tensor(0.0, device=embeddings.device)

        embeddings = F.normalize(embeddings, dim=1)
        total_loss = torch.tensor(0.0, device=embeddings.device)
        count = 0

        for idx_a, idx_b in self.pair_indices:
            mask_a = (labels == idx_a)
            mask_b = (labels == idx_b)
            if mask_a.sum() == 0 or mask_b.sum() == 0:
                continue

            emb_a = embeddings[mask_a]  # (Na, D)
            emb_b = embeddings[mask_b]  # (Nb, D)

            # Compute pairwise similarities
            sim = torch.matmul(emb_a, emb_b.T) / self.temperature  # (Na, Nb)

            # Hinge loss: push apart (want similarity < -margin)
            # loss = max(0, sim + margin)
            pair_loss = F.relu(sim + self.margin).mean()
            total_loss = total_loss + pair_loss
            count += 1

        if count > 0:
            total_loss = total_loss / count
        return total_loss


# ---------------------------------------------------------------------------
# R-Drop KL Loss (v30 NEW)
# ---------------------------------------------------------------------------

def compute_rdrop_kl_loss(logits1, logits2):
    """Compute bidirectional KL divergence between two forward passes (R-Drop)."""
    p_loss = F.kl_div(
        F.log_softmax(logits1, dim=-1),
        F.softmax(logits2, dim=-1),
        reduction='batchmean'
    )
    q_loss = F.kl_div(
        F.log_softmax(logits2, dim=-1),
        F.softmax(logits1, dim=-1),
        reduction='batchmean'
    )
    return (p_loss + q_loss) / 2


# ---------------------------------------------------------------------------
# SWAD (Stochastic Weight Averaging Densely) (v30 NEW)
# ---------------------------------------------------------------------------

class SWADCollector:
    """Track model weights for SWAD averaging with overfit-aware schedule.

    Starts collecting when val loss begins increasing. Averages weights
    during the overfit window. Maintains a dense collection of weight snapshots.
    """
    def __init__(self, model, start_step=20, end_patience=10):
        self.start_step = start_step
        self.end_patience = end_patience
        self.weight_snapshots = []
        self.val_losses = []
        self.best_val_loss = float('inf')
        self.collecting = False
        self.start_collecting_step = None
        self.overfit_count = 0
        self.stopped = False

    def update(self, model, val_loss, step):
        """Called after each validation. Records val loss and possibly collects weights."""
        self.val_losses.append(val_loss)

        if self.stopped:
            return

        if val_loss < self.best_val_loss:
            self.best_val_loss = val_loss
            self.overfit_count = 0
        else:
            self.overfit_count += 1

        # Start collecting after start_step epochs and when we first see overfit
        if step >= self.start_step and not self.collecting and self.overfit_count >= 1:
            self.collecting = True
            self.start_collecting_step = step
            print(f"[{ts()}]   SWAD: Start collecting at step {step} "
                  f"(val_loss={val_loss:.4f}, best={self.best_val_loss:.4f})")

        if self.collecting:
            # Save a snapshot of model weights
            snapshot = {k: v.cpu().clone() for k, v in model.state_dict().items()}
            self.weight_snapshots.append(snapshot)

            # Stop if too many overfit steps
            if self.overfit_count >= self.end_patience:
                self.stopped = True
                print(f"[{ts()}]   SWAD: Stop collecting at step {step} "
                      f"({len(self.weight_snapshots)} snapshots)")

    def get_averaged_model(self, model):
        """Return model with SWAD-averaged weights."""
        if not self.weight_snapshots:
            print(f"[{ts()}]   SWAD: No snapshots collected, returning original model")
            return model

        print(f"[{ts()}]   SWAD: Averaging {len(self.weight_snapshots)} weight snapshots")
        avg_state = {}
        for key in self.weight_snapshots[0]:
            avg_state[key] = torch.stack(
                [snap[key].float() for snap in self.weight_snapshots]
            ).mean(dim=0)

        # Load averaged weights
        model_copy = copy.deepcopy(model)
        current_state = model_copy.state_dict()
        for key in avg_state:
            if key in current_state:
                current_state[key] = avg_state[key].to(current_state[key].dtype)
        model_copy.load_state_dict(current_state)
        return model_copy


# ---------------------------------------------------------------------------
# Focal Loss
# ---------------------------------------------------------------------------

class FocalLoss(nn.Module):
    def __init__(self, gamma=2.0, alpha=None, label_smoothing=0.0):
        super().__init__()
        self.gamma = gamma
        self.label_smoothing = label_smoothing
        if alpha is not None:
            self.register_buffer("alpha", alpha)
        else:
            self.alpha = None

    def forward(self, logits, targets):
        ce_loss = F.cross_entropy(
            logits, targets, reduction='none',
            label_smoothing=self.label_smoothing,
        )
        pt = torch.exp(-ce_loss)
        focal_loss = ((1 - pt) ** self.gamma) * ce_loss
        if self.alpha is not None:
            alpha_t = self.alpha[targets]
            focal_loss = alpha_t * focal_loss
        return focal_loss.mean()


# ---------------------------------------------------------------------------
# Spatial Dropout (replaced by DropGraph when enabled)
# ---------------------------------------------------------------------------

class SpatialNodeDropout(nn.Module):
    def __init__(self, p=0.1):
        super().__init__()
        self.p = p

    def forward(self, x):
        if not self.training or self.p == 0:
            return x
        B, C, T, N = x.shape
        mask = torch.ones(B, 1, 1, N, device=x.device)
        mask = F.dropout(mask, p=self.p, training=True)
        return x * mask


# ---------------------------------------------------------------------------
# Gradient Reversal Layer
# ---------------------------------------------------------------------------

class GradientReversalFunction(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x, lambda_):
        ctx.lambda_ = lambda_
        return x.clone()

    @staticmethod
    def backward(ctx, grad_output):
        return -ctx.lambda_ * grad_output, None


# ---------------------------------------------------------------------------
# Model (v30 - KSLGraphNetV25 with MixStyle and DropGraph)
# ---------------------------------------------------------------------------

class GConv(nn.Module):
    def __init__(self, i, o):
        super().__init__()
        self.fc = nn.Linear(i, o)

    def forward(self, x, adj):
        return self.fc(torch.matmul(adj, x))


class STGCNBlock(nn.Module):
    def __init__(self, ic, oc, adj, temporal_kernels=(3, 5, 7), st=1, dr=0.3,
                 spatial_dropout=0.1, use_dropgraph=False):
        super().__init__()
        self.register_buffer("adj", adj)
        self.gcn = GConv(ic, oc)
        self.bn1 = nn.BatchNorm2d(oc)
        self.tcn = MultiScaleTCN(oc, temporal_kernels, st, dr)
        self.residual = (
            nn.Sequential(nn.Conv2d(ic, oc, 1, stride=(st, 1)), nn.BatchNorm2d(oc))
            if ic != oc or st != 1
            else nn.Identity()
        )
        self.dropout = nn.Dropout(dr)
        # v30: Use DropGraph or standard SpatialNodeDropout
        if use_dropgraph:
            self.spatial_drop = DropGraph(p=spatial_dropout, num_nodes=48)
        else:
            self.spatial_drop = SpatialNodeDropout(spatial_dropout)

    def forward(self, x):
        r = self.residual(x)
        b, c, t, n = x.shape
        x = self.gcn(x.permute(0, 2, 3, 1).reshape(b * t, n, c), self.adj)
        x = x.reshape(b, t, n, -1).permute(0, 3, 1, 2)
        x = torch.relu(self.bn1(x))
        x = self.spatial_drop(x)
        x = self.tcn(x)
        return torch.relu(x + r)


class KSLGraphNetV30(nn.Module):
    """ST-GCN with MixStyle, DropGraph, aux branch, GRL, SupCon (v30)."""

    def __init__(self, nc, num_signers, aux_dim, nn_=48, ic=3, hd=64, nl=4,
                 tk=(3, 5, 7), dr=0.3, spatial_dropout=0.1, adj=None,
                 use_mixstyle=True, use_dropgraph=True):
        super().__init__()
        self.register_buffer("adj", adj)
        self.data_bn = nn.BatchNorm1d(nn_ * ic)
        self.use_mixstyle = use_mixstyle

        ch = [ic] + [hd] * 2 + [hd * 2] * 2
        ch = ch[:nl + 1]

        self.layers = nn.ModuleList(
            [STGCNBlock(ch[i], ch[i + 1], adj, tk,
                        2 if i in [1, 3] else 1,
                        dr, spatial_dropout, use_dropgraph=use_dropgraph)
             for i in range(nl)]
        )

        # MixStyle after blocks 0 and 1 (first two GCN blocks)
        if use_mixstyle:
            self.mixstyle = MixStyle(p=0.5, alpha=0.1)

        final_ch = ch[-1]
        self.attn_pool = AttentionPool(final_ch, num_heads=2)
        gcn_embed_dim = 2 * final_ch

        self.aux_bn = nn.BatchNorm1d(aux_dim)
        self.aux_mlp = nn.Sequential(
            nn.Linear(aux_dim, 128),
            nn.ReLU(),
            nn.Dropout(dr),
            nn.Linear(128, 64),
        )
        self.aux_temporal_conv = nn.Sequential(
            nn.Conv1d(64, 64, kernel_size=5, padding=2),
            nn.BatchNorm1d(64),
            nn.ReLU(),
        )
        self.aux_attn = nn.Sequential(
            nn.Linear(64, 32),
            nn.Tanh(),
            nn.Linear(32, 1),
        )

        self.embed_dim = gcn_embed_dim + 64

        self.classifier = nn.Sequential(
            nn.Linear(self.embed_dim, hd),
            nn.ReLU(),
            nn.Dropout(dr),
            nn.Linear(hd, nc),
        )

        self.signer_head = nn.Sequential(
            nn.Linear(self.embed_dim, 64),
            nn.ReLU(),
            nn.Linear(64, num_signers),
        )

    def forward(self, gcn_input, aux_input, grl_lambda=0.0):
        b, c, t, n = gcn_input.shape
        x = self.data_bn(
            gcn_input.permute(0, 1, 3, 2).reshape(b, c * n, t)
        ).reshape(b, c, n, t).permute(0, 1, 3, 2)

        for i, layer in enumerate(self.layers):
            x = layer(x)
            # Apply MixStyle after blocks 0 and 1
            if self.use_mixstyle and i in (0, 1):
                x = self.mixstyle(x)

        x_node_avg = x.mean(dim=3)
        gcn_embedding = self.attn_pool(x_node_avg)

        aux = self.aux_bn(aux_input.permute(0, 2, 1)).permute(0, 2, 1)
        aux = self.aux_mlp(aux)
        aux_t = aux.permute(0, 2, 1)
        aux_t = self.aux_temporal_conv(aux_t)
        aux = aux_t.permute(0, 2, 1)
        attn_logits = self.aux_attn(aux)
        attn_weights = F.softmax(attn_logits, dim=1)
        aux_embedding = (aux * attn_weights).sum(dim=1)

        embedding = torch.cat([gcn_embedding, aux_embedding], dim=1)
        logits = self.classifier(embedding)
        reversed_embedding = GradientReversalFunction.apply(embedding, grl_lambda)
        signer_logits = self.signer_head(reversed_embedding)

        return logits, signer_logits, embedding


# ---------------------------------------------------------------------------
# Train One Stream (v30 - with all Phase 2 improvements)
# ---------------------------------------------------------------------------

def train_stream(stream_name, name, classes, config, train_dir, val_dir,
                 ckpt_dir, device, use_focal=False, flags=None):
    """Train one stream model with v30 Phase 2 improvements.

    Args:
        stream_name: "joint", "bone", or "velocity"
        name: category name ("Numbers" or "Words")
        classes: list of class names
        config: V30_CONFIG
        train_dir: list of training data dirs
        val_dir: validation data dir
        ckpt_dir: base checkpoint dir
        device: torch device
        use_focal: use focal loss instead of CE
        flags: dict of Phase 2 feature flags
    """
    if flags is None:
        flags = {}

    stream_ckpt_dir = os.path.join(ckpt_dir, stream_name)

    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] Training {name} - Stream: {stream_name} ({len(classes)} classes)")
    print(f"[{ts()}] {'=' * 70}")
    print(f"[{ts()}] v30 Flags: {flags}")

    adj = build_adj(config["num_nodes"]).to(device)

    # Dataset with v30 augmentation flags
    train_ds = KSLMultiStreamDataset(
        train_dir, classes, config, aug=True,
        use_aggressive_aug=flags.get("aggressive_aug", True),
        use_skeleton_noise=flags.get("skeleton_noise", True),
        use_jmda=flags.get("jmda", False),
        use_shap_mix=flags.get("shap_mix", False),
    )
    val_ds = KSLMultiStreamDataset(val_dir, classes, config, aug=False)

    if len(train_ds) == 0:
        print(f"[{ts()}] ERROR: No training samples found")
        return None

    c2i = train_ds.c2i
    i2c = {v: k for k, v in c2i.items()}
    num_signers = train_ds.num_signers

    aux_dim = NUM_ANGLE_FEATURES + 2 * NUM_FINGERTIP_PAIRS + NUM_HAND_BODY_FEATURES
    print(f"[{ts()}] Aux dim: {aux_dim}, Num signers: {num_signers}")

    train_sampler = SignerBalancedSampler(
        train_ds.labels, train_ds.signer_labels,
        batch_size=config["batch_size"], drop_last=True,
    )

    train_ld = DataLoader(
        train_ds, batch_size=config["batch_size"],
        sampler=train_sampler, num_workers=2, pin_memory=True,
        collate_fn=stream_collate_fn,
    )
    val_ld = DataLoader(
        val_ds, batch_size=config["batch_size"],
        shuffle=False, num_workers=2, pin_memory=True,
        collate_fn=stream_collate_fn,
    )

    tk = tuple(config.get("temporal_kernels", [3, 5, 7]))
    model = KSLGraphNetV30(
        nc=len(classes),
        num_signers=num_signers,
        aux_dim=aux_dim,
        nn_=config["num_nodes"],
        ic=config["in_channels"],
        hd=config["hidden_dim"],
        nl=config["num_layers"],
        tk=tk,
        dr=config["dropout"],
        spatial_dropout=config.get("spatial_dropout", 0.1),
        adj=adj,
        use_mixstyle=flags.get("mixstyle", True),
        use_dropgraph=flags.get("dropgraph", True),
    ).to(device)

    param_count = sum(p.numel() for p in model.parameters())
    print(f"[{ts()}] Parameters: {param_count:,}")

    # --- Losses ---
    supcon_loss_fn = SupConLoss(temperature=config["supcon_temperature"])

    label_smooth = config["label_smoothing"] if flags.get("label_smooth", True) else 0.0

    if use_focal:
        class_counts = Counter(train_ds.labels)
        num_classes = len(classes)
        total_samples = len(train_ds)
        alpha_weights = torch.zeros(num_classes)
        for cls_idx in range(num_classes):
            count = class_counts.get(cls_idx, 1)
            alpha_weights[cls_idx] = total_samples / (num_classes * count)
        alpha_weights = alpha_weights / alpha_weights.mean()
        alpha_weights = alpha_weights.to(device)
        cls_loss_fn = FocalLoss(
            gamma=2.0, alpha=alpha_weights, label_smoothing=label_smooth,
        )
    else:
        cls_loss_fn = None

    # v30: Confused-pair contrastive loss
    confused_pair_loss_fn = None
    if flags.get("confused_pairs", True):
        confused_pairs = NUMBER_CONFUSED_PAIRS if classes == NUMBER_CLASSES else WORD_CONFUSED_PAIRS
        confused_pair_loss_fn = ConfusedPairLoss(confused_pairs, c2i)
        print(f"[{ts()}] Confused-pair loss enabled: {len(confused_pairs)} pairs")

    use_rdrop = flags.get("r_drop", True)
    rdrop_alpha = config.get("r_drop_alpha", 1.0)
    if use_rdrop:
        print(f"[{ts()}] R-Drop enabled: alpha={rdrop_alpha}")

    # v30: SWAD
    swad_collector = None
    if flags.get("swad", True):
        swad_collector = SWADCollector(model, start_step=20, end_patience=15)
        print(f"[{ts()}] SWAD enabled")

    # Optimizer
    opt = torch.optim.AdamW(
        model.parameters(), lr=config["learning_rate"], weight_decay=config["weight_decay"]
    )
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        opt, T_max=config["epochs"], eta_min=config["min_lr"]
    )
    warmup_epochs = config["warmup_epochs"]

    os.makedirs(stream_ckpt_dir, exist_ok=True)
    best_path = os.path.join(stream_ckpt_dir, "best_model.pt")

    best, patience_counter = 0.0, 0
    mixup_alpha = config.get("mixup_alpha", 0)
    cutmix_alpha = config.get("cutmix_alpha", 1.0)
    cutmix_prob = config.get("cutmix_prob", 0.3)
    supcon_weight = config["supcon_weight"]
    confused_pair_weight = config.get("confused_pair_weight", 0.1)

    for ep in range(config["epochs"]):
        ep_start = time.time()
        model.train()
        tl, tc, tt = 0.0, 0, 0
        signer_correct, signer_total = 0, 0

        if ep < config["grl_start_epoch"]:
            grl_lambda = 0.0
        else:
            progress = min(1.0, (ep - config["grl_start_epoch"]) / config["grl_ramp_epochs"])
            grl_lambda = config["grl_lambda_max"] * progress

        if ep < warmup_epochs:
            warmup_factor = (ep + 1) / warmup_epochs
            for pg in opt.param_groups:
                pg['lr'] = config["learning_rate"] * warmup_factor

        for streams_batch, aux_data, targets, signer_targets, jmda_targets, jmda_lams in train_ld:
            gcn_data = streams_batch[stream_name].to(device)
            aux_data = aux_data.to(device)
            targets = targets.to(device)
            signer_targets = signer_targets.to(device)
            jmda_targets = jmda_targets.to(device)
            jmda_lams = jmda_lams.to(device)
            opt.zero_grad()

            use_cutmix = np.random.random() < cutmix_prob
            use_mixup = (not use_cutmix) and (mixup_alpha > 0)

            # --- Forward pass (with optional R-Drop double forward) ---
            if use_cutmix:
                gcn_mixed, aux_mixed, targets_a, targets_b, lam = temporal_cutmix(
                    gcn_data, aux_data, targets, signer_targets, alpha=cutmix_alpha,
                )
                logits, signer_logits, embedding = model(
                    gcn_mixed, aux_mixed, grl_lambda=grl_lambda,
                )
                if cls_loss_fn is not None:
                    cls_loss = (
                        lam * cls_loss_fn(logits, targets_a)
                        + (1 - lam) * cls_loss_fn(logits, targets_b)
                    )
                else:
                    cls_loss = (
                        lam * F.cross_entropy(logits, targets_a, label_smoothing=label_smooth)
                        + (1 - lam) * F.cross_entropy(logits, targets_b, label_smoothing=label_smooth)
                    )
                sc_loss = supcon_loss_fn(embedding, targets_a)
                signer_loss = F.cross_entropy(signer_logits, signer_targets)
                loss = cls_loss + supcon_weight * sc_loss + grl_lambda * signer_loss

                # R-Drop for CutMix: second forward with different dropout
                if use_rdrop:
                    logits2, _, _ = model(gcn_mixed, aux_mixed, grl_lambda=grl_lambda)
                    kl_loss = compute_rdrop_kl_loss(logits, logits2)
                    loss = loss + rdrop_alpha * kl_loss

                # Confused-pair loss
                if confused_pair_loss_fn is not None:
                    cp_loss = confused_pair_loss_fn(embedding, targets_a)
                    loss = loss + confused_pair_weight * cp_loss

                _, p = logits.max(1)
                tc += (lam * p.eq(targets_a).float() + (1 - lam) * p.eq(targets_b).float()).sum().item()

            elif use_mixup:
                lam = np.random.beta(mixup_alpha, mixup_alpha)
                perm = torch.randperm(gcn_data.size(0), device=device)
                gcn_mixed = lam * gcn_data + (1 - lam) * gcn_data[perm]
                aux_mixed = lam * aux_data + (1 - lam) * aux_data[perm]
                t_perm = targets[perm]
                logits, signer_logits, embedding = model(
                    gcn_mixed, aux_mixed, grl_lambda=grl_lambda,
                )
                if cls_loss_fn is not None:
                    cls_loss = (
                        lam * cls_loss_fn(logits, targets)
                        + (1 - lam) * cls_loss_fn(logits, t_perm)
                    )
                else:
                    cls_loss = (
                        lam * F.cross_entropy(logits, targets, label_smoothing=label_smooth)
                        + (1 - lam) * F.cross_entropy(logits, t_perm, label_smoothing=label_smooth)
                    )
                sc_loss = supcon_loss_fn(embedding, targets)
                signer_loss = F.cross_entropy(signer_logits, signer_targets)
                loss = cls_loss + supcon_weight * sc_loss + grl_lambda * signer_loss

                if use_rdrop:
                    logits2, _, _ = model(gcn_mixed, aux_mixed, grl_lambda=grl_lambda)
                    kl_loss = compute_rdrop_kl_loss(logits, logits2)
                    loss = loss + rdrop_alpha * kl_loss

                if confused_pair_loss_fn is not None:
                    cp_loss = confused_pair_loss_fn(embedding, targets)
                    loss = loss + confused_pair_weight * cp_loss

                _, p = logits.max(1)
                tc += (lam * p.eq(targets).float() + (1 - lam) * p.eq(t_perm).float()).sum().item()

            else:
                logits, signer_logits, embedding = model(
                    gcn_data, aux_data, grl_lambda=grl_lambda,
                )

                # Handle JMDA/Shap-Mix labels (mixed samples have lam < 1)
                has_jmda = (jmda_lams < 0.999).any()
                if has_jmda:
                    if cls_loss_fn is not None:
                        cls_loss = (
                            jmda_lams.mean() * cls_loss_fn(logits, targets)
                            + (1 - jmda_lams.mean()) * cls_loss_fn(logits, jmda_targets)
                        )
                    else:
                        cls_loss = (
                            jmda_lams.mean() * F.cross_entropy(logits, targets, label_smoothing=label_smooth)
                            + (1 - jmda_lams.mean()) * F.cross_entropy(logits, jmda_targets, label_smoothing=label_smooth)
                        )
                else:
                    if cls_loss_fn is not None:
                        cls_loss = cls_loss_fn(logits, targets)
                    else:
                        cls_loss = F.cross_entropy(logits, targets, label_smoothing=label_smooth)

                sc_loss = supcon_loss_fn(embedding, targets)
                signer_loss = F.cross_entropy(signer_logits, signer_targets)
                loss = cls_loss + supcon_weight * sc_loss + grl_lambda * signer_loss

                # R-Drop: second forward with different dropout masks
                if use_rdrop:
                    logits2, _, _ = model(gcn_data, aux_data, grl_lambda=grl_lambda)
                    kl_loss = compute_rdrop_kl_loss(logits, logits2)
                    loss = loss + rdrop_alpha * kl_loss

                # Confused-pair contrastive loss
                if confused_pair_loss_fn is not None:
                    cp_loss = confused_pair_loss_fn(embedding, targets)
                    loss = loss + confused_pair_weight * cp_loss

                _, p = logits.max(1)
                tc += p.eq(targets).sum().item()

            _, s_pred = signer_logits.max(1)
            signer_correct += s_pred.eq(signer_targets).sum().item()
            signer_total += signer_targets.size(0)

            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            opt.step()
            tl += loss.item()
            tt += targets.size(0)

        if ep >= warmup_epochs:
            scheduler.step()

        # Validation
        model.eval()
        vc, vt = 0, 0
        val_loss_total = 0.0
        val_batches = 0
        with torch.no_grad():
            for streams_batch, aux_data, targets, _, _, _ in val_ld:
                gcn_data = streams_batch[stream_name].to(device)
                aux_data = aux_data.to(device)
                targets = targets.to(device)
                logits, _, _ = model(gcn_data, aux_data, grl_lambda=0.0)
                _, p = logits.max(1)
                vt += targets.size(0)
                vc += p.eq(targets).sum().item()
                val_loss_total += F.cross_entropy(logits, targets).item()
                val_batches += 1

        va = 100.0 * vc / vt if vt > 0 else 0.0
        ta = 100.0 * tc / tt if tt > 0 else 0.0
        signer_acc = 100.0 * signer_correct / signer_total if signer_total > 0 else 0.0
        ep_time = time.time() - ep_start
        lr_now = opt.param_groups[0]["lr"]
        val_loss_avg = val_loss_total / max(val_batches, 1)

        # SWAD: update collector with val loss
        if swad_collector is not None:
            swad_collector.update(model, val_loss_avg, ep)

        if ep < 10 or (ep + 1) % 10 == 0 or va > best:
            print(
                f"[{ts()}] [{stream_name}] Ep {ep + 1:3d}/{config['epochs']} | "
                f"Loss: {tl / max(len(train_ld), 1):.4f} | Train: {ta:.1f}% | Val: {va:.1f}% | "
                f"SignerAcc: {signer_acc:.1f}% | "
                f"LR: {lr_now:.2e} | GRL: {grl_lambda:.3f} | Time: {ep_time:.1f}s"
            )

        if va > best:
            best, patience_counter = va, 0
            torch.save(
                {
                    "model": model.state_dict(),
                    "val_acc": va,
                    "epoch": ep + 1,
                    "classes": classes,
                    "num_nodes": config["num_nodes"],
                    "num_signers": num_signers,
                    "aux_dim": aux_dim,
                    "stream": stream_name,
                    "version": "v30",
                    "config": config,
                    "flags": flags,
                },
                best_path,
            )
            print(f"[{ts()}]   -> New best! {va:.1f}%")
        else:
            patience_counter += 1

        if patience_counter >= config["patience"]:
            print(f"[{ts()}] [{stream_name}] Early stopping at epoch {ep + 1}")
            break

        # Per-class accuracy every 50 epochs
        if (ep + 1) % 50 == 0:
            model.eval()
            preds_ep, tgts_ep = [], []
            with torch.no_grad():
                for streams_batch, aux_data, targets, _, _, _ in val_ld:
                    gcn_data = streams_batch[stream_name].to(device)
                    logits, _, _ = model(gcn_data.to(device), aux_data.to(device),
                                         grl_lambda=0.0)
                    _, p = logits.max(1)
                    preds_ep.extend(p.cpu().numpy())
                    tgts_ep.extend(targets.cpu().numpy())
            print(f"[{ts()}]   Per-class at epoch {ep + 1}:")
            for i in range(len(classes)):
                cn = i2c[i]
                tot_cls = sum(1 for t in tgts_ep if t == i)
                cor = sum(1 for t, p in zip(tgts_ep, preds_ep) if t == i and t == p)
                acc = 100.0 * cor / tot_cls if tot_cls > 0 else 0.0
                print(f"[{ts()}]     {cn:12s}: {acc:5.1f}% ({cor}/{tot_cls})")

    # --- SWAD: Save SWAD-averaged model alongside best model ---
    swad_path = None
    if swad_collector is not None and swad_collector.weight_snapshots:
        swad_model = swad_collector.get_averaged_model(model)
        swad_path = os.path.join(stream_ckpt_dir, "swad_model.pt")
        # Evaluate SWAD model
        swad_model.to(device)
        swad_model.eval()
        swad_vc, swad_vt = 0, 0
        with torch.no_grad():
            for streams_batch, aux_data, targets, _, _, _ in val_ld:
                gcn_data = streams_batch[stream_name].to(device)
                aux_data = aux_data.to(device)
                targets = targets.to(device)
                logits, _, _ = swad_model(gcn_data, aux_data, grl_lambda=0.0)
                _, p = logits.max(1)
                swad_vt += targets.size(0)
                swad_vc += p.eq(targets).sum().item()
        swad_va = 100.0 * swad_vc / swad_vt if swad_vt > 0 else 0.0
        print(f"[{ts()}] [{stream_name}] SWAD val accuracy: {swad_va:.1f}% "
              f"(best early-stop: {best:.1f}%)")

        torch.save(
            {
                "model": swad_model.state_dict(),
                "val_acc": swad_va,
                "classes": classes,
                "num_nodes": config["num_nodes"],
                "num_signers": num_signers,
                "aux_dim": aux_dim,
                "stream": stream_name,
                "version": "v30-swad",
                "config": config,
                "flags": flags,
                "num_snapshots": len(swad_collector.weight_snapshots),
            },
            swad_path,
        )
        print(f"[{ts()}] SWAD model saved to {swad_path}")

        # Use SWAD model if it's better
        if swad_va > best:
            print(f"[{ts()}]   SWAD model is better! Using SWAD weights.")
            best_path_to_load = swad_path
            best = swad_va
        else:
            best_path_to_load = best_path
    else:
        best_path_to_load = best_path

    # Final evaluation with best model
    ckpt = torch.load(best_path_to_load, map_location=device, weights_only=False)
    model.load_state_dict(ckpt["model"])
    model.eval()

    preds, tgts = [], []
    with torch.no_grad():
        for streams_batch, aux_data, targets, _, _, _ in val_ld:
            gcn_data = streams_batch[stream_name].to(device)
            logits, _, _ = model(gcn_data.to(device), aux_data.to(device),
                                 grl_lambda=0.0)
            _, p = logits.max(1)
            preds.extend(p.cpu().numpy())
            tgts.extend(targets.cpu().numpy())

    print(f"\n[{ts()}] {name} [{stream_name}] Per-Class Results (best val {best:.1f}%):")
    res = {}
    for i in range(len(classes)):
        cn = i2c[i]
        tot_cls = sum(1 for t in tgts if t == i)
        cor = sum(1 for t, p in zip(tgts, preds) if t == i and t == p)
        res[cn] = 100.0 * cor / tot_cls if tot_cls > 0 else 0.0
        print(f"[{ts()}]   {cn:12s}: {res[cn]:5.1f}% ({cor}/{tot_cls})")

    ov = 100.0 * sum(1 for t, p in zip(tgts, preds) if t == p) / len(tgts) if len(tgts) > 0 else 0.0
    print(f"[{ts()}] {name} [{stream_name}] Overall: {ov:.1f}%")

    return {
        "stream": stream_name,
        "overall": ov,
        "per_class": res,
        "best_epoch": ckpt.get("epoch", -1),
        "params": param_count,
        "swad_acc": swad_va if swad_collector and swad_collector.weight_snapshots else None,
    }


# ---------------------------------------------------------------------------
# Fusion Weight Learning (same as v28)
# ---------------------------------------------------------------------------

def learn_fusion_weights(category_name, classes, config, val_dir, ckpt_dir, device, flags=None):
    if flags is None:
        flags = {}

    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] Learning fusion weights for {category_name}")
    print(f"[{ts()}] {'=' * 70}")

    streams = config["streams"]
    adj = build_adj(config["num_nodes"]).to(device)
    aux_dim = NUM_ANGLE_FEATURES + 2 * NUM_FINGERTIP_PAIRS + NUM_HAND_BODY_FEATURES
    tk = tuple(config.get("temporal_kernels", [3, 5, 7]))

    models = {}
    for sname in streams:
        # Try SWAD model first, fall back to best model
        swad_path = os.path.join(ckpt_dir, sname, "swad_model.pt")
        best_model_path = os.path.join(ckpt_dir, sname, "best_model.pt")

        if flags.get("swad", True) and os.path.exists(swad_path):
            ckpt_path = swad_path
        elif os.path.exists(best_model_path):
            ckpt_path = best_model_path
        else:
            print(f"[{ts()}] WARNING: No checkpoint found for stream '{sname}'")
            return None, 0.0

        ckpt = torch.load(ckpt_path, map_location=device, weights_only=False)
        num_signers = ckpt.get("num_signers", 12)

        model = KSLGraphNetV30(
            nc=len(classes), num_signers=num_signers, aux_dim=aux_dim,
            nn_=config["num_nodes"], ic=3, hd=config["hidden_dim"],
            nl=config["num_layers"], tk=tk, dr=config["dropout"],
            spatial_dropout=config.get("spatial_dropout", 0.1), adj=adj,
            use_mixstyle=flags.get("mixstyle", True),
            use_dropgraph=flags.get("dropgraph", True),
        ).to(device)
        model.load_state_dict(ckpt["model"])
        model.eval()
        models[sname] = model
        print(f"[{ts()}] Loaded {sname} model from {os.path.basename(ckpt_path)}: "
              f"val_acc={ckpt.get('val_acc', 0):.1f}%")

    val_ds = KSLMultiStreamDataset(val_dir, classes, config, aug=False)
    val_ld = DataLoader(val_ds, batch_size=config["batch_size"], shuffle=False,
                        num_workers=2, pin_memory=True, collate_fn=stream_collate_fn)

    all_probs = {sname: [] for sname in streams}
    all_labels = []

    with torch.no_grad():
        for streams_batch, aux_data, targets, _, _, _ in val_ld:
            aux_data = aux_data.to(device)
            all_labels.extend(targets.numpy())
            for sname in streams:
                gcn_data = streams_batch[sname].to(device)
                logits, _, _ = models[sname](gcn_data, aux_data, grl_lambda=0.0)
                probs = F.softmax(logits, dim=1).cpu()
                all_probs[sname].append(probs)

    for sname in streams:
        all_probs[sname] = torch.cat(all_probs[sname], dim=0)
    all_labels = np.array(all_labels)

    print(f"\n[{ts()}] Per-stream val accuracy:")
    for sname in streams:
        preds = all_probs[sname].argmax(dim=1).numpy()
        acc = 100.0 * (preds == all_labels).mean()
        print(f"[{ts()}]   {sname:>10s}: {acc:.1f}%")

    best_acc = 0.0
    best_weights = {"joint": 0.4, "bone": 0.35, "velocity": 0.25}

    for w_joint_100 in range(10, 81, 5):
        for w_bone_100 in range(10, 81, 5):
            w_vel_100 = 100 - w_joint_100 - w_bone_100
            if w_vel_100 < 5:
                continue
            w_joint = w_joint_100 / 100.0
            w_bone = w_bone_100 / 100.0
            w_vel = w_vel_100 / 100.0

            fused = (w_joint * all_probs["joint"] +
                     w_bone * all_probs["bone"] +
                     w_vel * all_probs["velocity"])
            preds = fused.argmax(dim=1).numpy()
            acc = 100.0 * (preds == all_labels).mean()

            if acc > best_acc:
                best_acc = acc
                best_weights = {
                    "joint": float(w_joint),
                    "bone": float(w_bone),
                    "velocity": float(w_vel),
                }

    print(f"\n[{ts()}] Best fusion weights: {best_weights}")
    print(f"[{ts()}] Fused val accuracy: {best_acc:.1f}%")

    equal_fused = sum(all_probs[s] for s in streams) / len(streams)
    equal_preds = equal_fused.argmax(dim=1).numpy()
    equal_acc = 100.0 * (equal_preds == all_labels).mean()
    print(f"[{ts()}] Equal weights val accuracy: {equal_acc:.1f}%")

    weights_path = os.path.join(ckpt_dir, "fusion_weights.json")
    with open(weights_path, "w") as f:
        json.dump({
            "weights": best_weights,
            "val_accuracy": best_acc,
            "equal_weights_accuracy": equal_acc,
            "category": category_name,
        }, f, indent=2)

    print(f"[{ts()}] Fusion weights saved to {weights_path}")
    return best_weights, best_acc


# ---------------------------------------------------------------------------
# Main
# ---------------------------------------------------------------------------

def main():
    parser = argparse.ArgumentParser(
        description="KSL ST-GCN Training v30 - Phase 2 Training Improvements",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--model-type", type=str, default="both",
                        choices=["numbers", "words", "both"])
    parser.add_argument("--seed", type=int, default=42)

    base_data = "/scratch/alpine/hama5612/ksl-dir-2/data"
    parser.add_argument("--train-dir", type=str, default=os.path.join(base_data, "train_alpha"))
    parser.add_argument("--val-dir", type=str, default=os.path.join(base_data, "val_alpha"))
    parser.add_argument("--test-dir", type=str, default=os.path.join(base_data, "test_alpha"))
    parser.add_argument("--checkpoint-dir", type=str, default=os.path.join(base_data, "checkpoints"))
    parser.add_argument("--results-dir", type=str, default=os.path.join(base_data, "results"))

    # v30 Phase 2 feature flags (default ON unless noted)
    parser.add_argument("--mixstyle", action="store_true", default=True)
    parser.add_argument("--no-mixstyle", dest="mixstyle", action="store_false")
    parser.add_argument("--dropgraph", action="store_true", default=True)
    parser.add_argument("--no-dropgraph", dest="dropgraph", action="store_false")
    parser.add_argument("--confused-pairs", action="store_true", default=True)
    parser.add_argument("--no-confused-pairs", dest="confused_pairs", action="store_false")
    parser.add_argument("--cp-weight", type=float, default=0.1,
                        help="Confused-pair contrastive loss weight")
    parser.add_argument("--label-smooth", type=float, default=0.1,
                        help="Label smoothing epsilon (0 to disable)")
    parser.add_argument("--r-drop", type=float, default=1.0,
                        help="R-Drop KL divergence weight (0 to disable)")
    parser.add_argument("--swad", action="store_true", default=True)
    parser.add_argument("--no-swad", dest="swad", action="store_false")
    parser.add_argument("--aggressive-aug", action="store_true", default=True)
    parser.add_argument("--no-aggressive-aug", dest="aggressive_aug", action="store_false")
    parser.add_argument("--skeleton-noise", action="store_true", default=True)
    parser.add_argument("--no-skeleton-noise", dest="skeleton_noise", action="store_false")
    parser.add_argument("--jmda", action="store_true", default=False)
    parser.add_argument("--no-jmda", dest="jmda", action="store_false")
    parser.add_argument("--shap-mix", action="store_true", default=False)
    parser.add_argument("--no-shap-mix", dest="shap_mix", action="store_false")

    args = parser.parse_args()

    set_seed(args.seed)

    # Build flags dict
    flags = {
        "mixstyle": args.mixstyle,
        "dropgraph": args.dropgraph,
        "confused_pairs": args.confused_pairs,
        "label_smooth": args.label_smooth > 0,
        "r_drop": args.r_drop > 0,
        "swad": args.swad,
        "aggressive_aug": args.aggressive_aug,
        "skeleton_noise": args.skeleton_noise,
        "jmda": args.jmda,
        "shap_mix": args.shap_mix,
    }

    # Update config with CLI values
    V30_CONFIG["label_smoothing"] = args.label_smooth
    V30_CONFIG["r_drop_alpha"] = args.r_drop
    V30_CONFIG["confused_pair_weight"] = args.cp_weight

    print("=" * 70)
    print(f"KSL Training v30 - Phase 2 Training Improvements")
    print(f"Started: {ts()}")
    print(f"Seed: {args.seed}")
    print(f"Flags: {json.dumps(flags, indent=2)}")
    print("=" * 70)

    check_mediapipe_version()

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Device: {device}")
    if torch.cuda.is_available():
        print(f"GPU: {torch.cuda.get_device_name(0)}")
        print(f"GPU Memory: {torch.cuda.get_device_properties(0).total_memory / 1e9:.1f} GB")

    train_dirs = [args.train_dir, args.val_dir]  # signers 1-12
    val_dir = args.test_dir                       # signers 13-15

    print(f"\nv30 Data Split:")
    print(f"  Train dirs: {train_dirs}  (signers 1-12)")
    print(f"  Val dir:    {val_dir}  (signers 13-15)")
    print(f"  Streams: {V30_CONFIG['streams']}")

    print(f"\n[{ts()}] v30 Config:")
    print(json.dumps(V30_CONFIG, indent=2, default=str))

    results = {}
    start_time = time.time()

    categories = []
    if args.model_type in ("numbers", "both"):
        categories.append(("Numbers", NUMBER_CLASSES, False))
    if args.model_type in ("words", "both"):
        categories.append(("Words", WORD_CLASSES, True))

    for cat_name, classes, use_focal in categories:
        cat_key = cat_name.lower()
        ckpt_dir = os.path.join(args.checkpoint_dir, f"v30_{cat_key}")

        stream_results = {}
        for stream_name in V30_CONFIG["streams"]:
            result = train_stream(
                stream_name, cat_name, classes, V30_CONFIG,
                train_dirs, val_dir, ckpt_dir, device,
                use_focal=use_focal,
                flags=flags,
            )
            if result:
                stream_results[stream_name] = result

        fusion_weights, fused_acc = learn_fusion_weights(
            cat_name, classes, V30_CONFIG, val_dir, ckpt_dir, device, flags=flags
        )

        results[cat_key] = {
            "streams": stream_results,
            "fusion_weights": fusion_weights,
            "fused_val_accuracy": fused_acc,
        }

    total_time = time.time() - start_time

    # Summary
    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] SUMMARY v30 - Phase 2 Training Improvements")
    print(f"[{ts()}] {'=' * 70}")

    for cat_key, cat_result in results.items():
        print(f"\n[{ts()}] {cat_key.upper()}:")
        for sname, sres in cat_result.get("streams", {}).items():
            swad_str = f", SWAD={sres['swad_acc']:.1f}%" if sres.get('swad_acc') is not None else ""
            print(f"[{ts()}]   {sname:>10s}: {sres['overall']:.1f}% "
                  f"(ep {sres['best_epoch']}, {sres['params']:,} params{swad_str})")
        fw = cat_result.get("fusion_weights", {})
        fused = cat_result.get("fused_val_accuracy", 0)
        print(f"[{ts()}]   {'fused':>10s}: {fused:.1f}% "
              f"(weights: j={fw.get('joint', 0):.2f}, b={fw.get('bone', 0):.2f}, "
              f"v={fw.get('velocity', 0):.2f})")

    if "numbers" in results and "words" in results:
        combined = (results["numbers"]["fused_val_accuracy"] +
                    results["words"]["fused_val_accuracy"]) / 2
        print(f"\n[{ts()}] Combined fused val: {combined:.1f}%")

    print(f"[{ts()}] Total training time: {total_time / 60:.1f} minutes")

    # Save results
    os.makedirs(args.results_dir, exist_ok=True)
    results_path = os.path.join(
        args.results_dir,
        f"v30_{args.model_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    )
    results_out = {
        "version": "v30",
        "model_type": args.model_type,
        "seed": args.seed,
        "flags": flags,
        "config": V30_CONFIG,
        "results": results,
        "total_time_seconds": total_time,
        "device": str(device),
        "gpu": torch.cuda.get_device_name(0) if torch.cuda.is_available() else "cpu",
        "timestamp": ts(),
        "changes_from_v28": [
            "1. MixStyle: Mix BN statistics between signers after GCN blocks 1,2",
            "2. DropGraph: Graph-aware dropout, drop 1-hop neighborhoods",
            "3. Confused-Pair Contrastive Loss: Push confused classes apart in embedding space",
            "4. Label Smoothing: Configurable epsilon (default 0.1)",
            "5. R-Drop: Forward twice, minimize KL divergence between outputs",
            "6. SWAD: Stochastic Weight Averaging Densely with overfit-aware schedule",
            "7. Aggressive Body Augmentation: Extended bone/shoulder/torso/camera aug",
            "8. Skeleton Noise: Joint jitter, swaps, missing keypoints (MediaPipe sim)",
            "9. JMDA: SpatialMix + TemporalMix augmentation (off by default)",
            "10. Shap-Mix: Class-specific joint importance mixing (off by default)",
        ],
    }
    with open(results_path, "w") as f:
        json.dump(results_out, f, indent=2, default=str)
    print(f"[{ts()}] Results saved to {results_path}")
    print(f"[{ts()}] Done.")

    return results


if __name__ == "__main__":
    main()
