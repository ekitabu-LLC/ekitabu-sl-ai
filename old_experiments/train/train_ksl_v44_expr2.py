#!/usr/bin/env python3
"""
KSL Training v44 Expr2 — Signer-Adversarial Gradient Reversal Layer (GRL).

Based on v31 exp1 (GroupNorm + multi-stream late fusion) with a focused
signer-adversarial training signal:

  - GroupNorm (no BatchNorm) — domain-invariant normalization
  - Gradient Reversal Layer with sigmoid ramp schedule
  - Signer classification head after backbone pooling
  - NO SupCon loss (isolate GRL effect)

GRL schedule:
  lambda(t) = lambda_max * (2 / (1 + exp(-10 * t/T)) - 1)
  where t = current epoch, T = total epochs

The GRL encourages the backbone to learn signer-invariant features by
reversing gradients from a signer classifier during backpropagation.

Usage:
    python train_ksl_v44_expr2.py --model-type numbers --lambda-max 0.1
    python train_ksl_v44_expr2.py --model-type words --lambda-max 0.1
"""
...
# ---------------------------------------------------------------------------
# Model — v44_expr2: GroupNorm + Signer-Adversarial GRL
# ---------------------------------------------------------------------------
...
class KSLGraphNetV44Expr2(nn.Module):
    """ST-GCN with GroupNorm + signer-adversarial GRL head."""
    ...
    def __init__(self, nc, num_signers, aux_dim, nn_=48, ic=3, hd=64, nl=4,
        ...
        # Signer-adversarial head (after GRL)
    ...
    def forward(self, gcn_input, aux_input, grl_lambda=0.0):
        ...
        # Signer classification (reversed gradient flow)
        reversed_embedding = grad_reverse(embedding, grl_lambda)
        signer_logits = self.signer_classifier(reversed_embedding)
...
def train_stream(stream_name, name, classes, config, train_dir, val_dir,
    ...
    if use_focal:
        ...
        print(f"[{ts()}] Loss: CE(ls={config['label_smoothing']}) + GRL signer CE")
    ...
    for ep in range(max_epochs):
        ...
        # Sigmoid ramp GRL schedule
        grl_lambda = compute_grl_lambda(ep, max_epochs, lambda_max)
        ...
        if ep < 10 or (ep + 1) % 10 == 0 or va > best:
            print(
                ...
                f"LR: {lr_now:.2e} | GRL_lambda: {grl_lambda:.4f} | Time: {ep_time:.1f}s"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="KSL v44 Expr2 — Signer-Adversarial GRL + GroupNorm",
    ...
    parser.add_argument("--checkpoint-dir", type=str, default=os.path.join(base_data, "checkpoints/v44_expr2"))
    parser.add_argument("--results-dir", type=str, default=os.path.join(base_data, "results"))

    args = parser.parse_args()

    # Apply CLI overrides to config
    config = V44_EXPR2_CONFIG.copy()
    ...
    print("=" * 70)
    print("KSL Training v44 Expr2 — Signer-Adversarial GRL + GroupNorm")
    print(f"Started: {ts()}")
    ...
    # Summary
    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] SUMMARY v44 Expr2 — Signer-Adversarial GRL")
    print(f"[{ts()}] GRL lambda_max: {config['grl_lambda_max']}")
    print(f"[{ts()}] {'=' * 70}")
    ...
    results_path = os.path.join(
        args.results_dir,
        f"v44_expr2_{args.model_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    )
    results_out = {
        "version": "v44_expr2",
        "experiment": "signer_adversarial_grl",
        ...
        "timestamp": ts(),
        "description": [
            "GroupNorm + signer-adversarial GRL (no SupCon).",
            f"GRL sigmoid ramp to lambda_max={config['grl_lambda_max']}.",
            "Signer classifier head after backbone pooling.",
            "Gradient reversal encourages signer-invariant features.",
        ],
    }
