#!/usr/bin/env python3
"""
KSL VideoMAE v2 Training — Fine-tune VideoMAE-Base on ALL 10 signers.

Same as train_ksl_videomae.py but combines dataset/ (signers 1-5) and
validation-data/ (signers 6-10) into one pool. Holds out signer 5 for
cross-signer validation, trains on signers 1-4 + 6-10 (9 signers).

This matches the skeleton pipeline which trains on 10+ signers.

Data sources:
  dataset/{ClassName}/{ClassName}-{SignerID}-{RepID}.mov           (signers 1-5)
  validation-data/{X}. Signer/{N}. {ClassName}/{letter}. {ClassName}.mov  (signers 6-10)

Usage:
    python train_ksl_videomae_v2.py --model-type numbers
    python train_ksl_videomae_v2.py --model-type words
    python train_ksl_videomae_v2.py --model-type numbers --val-signer 5
"""
...
CKPT_DIR = os.path.join(BASE_DIR, "data", "checkpoints", "videomae_v2")
...
# ---------------------------------------------------------------------------
# Data discovery — ALL 10 signers
# ---------------------------------------------------------------------------
def discover_all_data(classes):
    """Discover videos from both dataset/ (signers 1-5) and validation-data/ (signers 6-10).

    Returns list of (path, label_idx, signer_id) tuples.
    """
    ...
    for cls_name in classes:
        ...
        for fname in sorted(os.listdir(cls_dir)):
            if not (fname.lower().endswith(".mov") or fname.lower().endswith(".mp4")):
                continue
            # Parse signer ID from filename: {ClassName}-{SignerID}-{RepID}.mov
            parts = fname.rsplit(".", 1)[0].split("-")
            if len(parts) >= 2:
                try:
                    signer_id = int(parts[-2])
                except ValueError:
                    signer_id = -1
            else:
                signer_id = -1
    ...
    for signer_dir_path in sorted(glob_module.glob(os.path.join(VAL_DIR, "*/"))):
        ...
        # Parse signer ID: "6. Signer" -> 6, "10. Signer" -> 10
        ...
        for class_dir_path in sorted(glob_module.glob(os.path.join(signer_dir_path, "*/"))):
            ...
            for fname in sorted(os.listdir(class_dir_path)):
                if fname.lower().endswith(".mov") or fname.lower().endswith(".mp4"):
                    all_videos.append((
                        os.path.join(class_dir_path, fname),
                        c2i[cls_name],
                        signer_id,
                    ))
...
def split_by_signer(all_videos, val_signer_id):
    ...
    print(f"[{ts()}] Val signer={val_signer_id}: "
...
def train(args):
    ...
    # Split: hold out val_signer for validation
...
# ---------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="Train VideoMAE v2 (all 10 signers)")
    ...
    parser.add_argument("--val-signer", type=int, default=5,
                        help="Signer ID to hold out for validation (default: 5)")
    args = parser.parse_args()

    print(f"[{ts()}] === KSL VideoMAE v2 Training (10 signers) ===")
