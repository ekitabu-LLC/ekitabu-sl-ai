#!/usr/bin/env python3
"""
KSL Training v31 Experiment 4 - Hard-Example Mining

Based on v28 multi-stream architecture with three key changes:

  1. WEIGHTED SAMPLING: Oversample hard classes by 5x using WeightedRandomSampler
     Hard number classes: 48, 91, 388, 66, 89
     Hard word classes: Tomatoes, Apple, Tortoise, Colour, Proud

  2. FOCAL LOSS: Replace cross-entropy with focal loss (gamma=2.0, per-class alpha)
     Higher alpha weights for hard classes to focus training on difficult examples.

  3. SPEED AUGMENTATION: Random temporal stretching [0.7, 1.3] to handle signer
     speed variation (e.g., signer 3 signs faster/slower than training signers).

Architecture: Same as v28 (multi-stream late fusion with 3-channel streams).
"""
    ...
    try:
        ...
        if version != "0.10.5":
            print(f"[{ts()}] WARNING: MediaPipe version is {version}, "
                  f"v31 exp4 data was extracted with 0.10.5.")
        else:
...
# Hard classes (low accuracy on real testers)
HARD_NUMBER_CLASSES = ["48", "91", "388", "66", "89"]
HARD_WORD_CLASSES = ["Tomatoes", "Apple", "Tortoise", "Colour", "Proud"]
...
# ---------------------------------------------------------------------------
# Config (v31 exp4: same as v28 + hard mining changes)
# ---------------------------------------------------------------------------
...
V31_EXP4_CONFIG = {
    ...
    "cutmix_alpha": 1.0,
    "cutmix_prob": 0.3,
    # NEW: Speed augmentation
    "speed_aug_prob": 0.5,
    "speed_aug_range": (0.7, 1.3),
    # NEW: Weighted sampling for hard classes
    "hard_class_weight": 5.0,
    # NEW: Focal loss
    "focal_gamma": 2.0,
}
...
def augment_speed(data_list, speed_range=(0.7, 1.3)):
    """NEW: Speed augmentation - random temporal stretching.

    Resamples temporal axis with linear interpolation to simulate
    different signing speeds (e.g., signer 3 faster/slower).
    """
    T = data_list[0].shape[0]
    if T < 2:
        return data_list
    ...
    # Create new time indices via linear interpolation
    old_indices = np.arange(T)
    new_indices = np.linspace(0, T - 1, new_T)

    result = []
    for arr in data_list:
        if arr.ndim == 3:  # (T, N, C) or (T, C, N)
            # Interpolate along time axis (axis=0)
            new_arr = np.zeros((new_T,) + arr.shape[1:], dtype=arr.dtype)
            for i in range(arr.shape[1]):
                for j in range(arr.shape[2]):
                    new_arr[:, i, j] = np.interp(new_indices, old_indices, arr[:, i, j])
            result.append(new_arr)
        elif arr.ndim == 2:  # (T, D)
            # Interpolate along time axis
            new_arr = np.zeros((new_T, arr.shape[1]), dtype=arr.dtype)
            for i in range(arr.shape[1]):
                new_arr[:, i] = np.interp(new_indices, old_indices, arr[:, i])
            result.append(new_arr)
        else:
            result.append(arr)
...
# ---------------------------------------------------------------------------
# Multi-Stream Dataset (v31 exp4: add speed augmentation)
# ---------------------------------------------------------------------------

class KSLMultiStreamDataset(Dataset):
    """Returns per-stream tensors (joint/bone/velocity) with speed augmentation."""
    ...
    def __getitem__(self, idx):
        ...
        # Compute velocity BEFORE speed aug and temporal warp
        ...
        # NEW: Speed augmentation (before temporal warp)
        if self.aug and np.random.random() < self.config["speed_aug_prob"]:
            h, velocity, bones, joint_angles, fingertip_dists, hand_body_feats = augment_speed(
                [h, velocity, bones, joint_angles, fingertip_dists, hand_body_feats],
                speed_range=self.config["speed_aug_range"]
            )
...
def stream_collate_fn(batch):
    """Custom collate for multi-stream dataset."""
...
# ---------------------------------------------------------------------------
# Train One Stream (v31 exp4: weighted sampling + focal loss)
# ---------------------------------------------------------------------------

def train_stream(stream_name, name, classes, config, train_dir, val_dir,
                 ckpt_dir, device, hard_classes=None):
    """Train one stream model with hard-example mining.

    Args:
        stream_name: "joint", "bone", or "velocity"
        name: category name ("Numbers" or "Words")
        classes: list of class names
        config: V31_EXP4_CONFIG
        ...
        device: torch device
        hard_classes: list of hard class names (for weighted sampling + focal loss alpha)
    """
    stream_ckpt_dir = os.path.join(ckpt_dir, stream_name)

    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] Training {name} - Stream: {stream_name} ({len(classes)} classes)")
    print(f"[{ts()}] Hard-example mining: {len(hard_classes) if hard_classes else 0} hard classes")
    print(f"[{ts()}] {'=' * 70}")
    ...
    if hard_classes:
        hard_class_indices = {c2i[cn] for cn in hard_classes if cn in c2i}
        print(f"[{ts()}] Hard class indices: {sorted(hard_class_indices)}")

    sample_weights = []
    hard_weight = config["hard_class_weight"]
    for label in train_ds.labels:
        if label in hard_class_indices:
    ...
    print(f"[{ts()}] Weighted sampling: hard classes get {hard_weight}x weight")
    ...
    print(f"[{ts()}] Loss: FocalLoss(gamma={config['focal_gamma']})")

    # Losses
    supcon_loss_fn = SupConLoss(temperature=config["supcon_temperature"])

    # NEW: Focal loss with per-class alpha (higher for hard classes)
    ...
    # Boost alpha for hard classes (additional 2x multiplier)
    if hard_classes:
        for cls_idx in hard_class_indices:
            alpha_weights[cls_idx] *= 2.0

    alpha_weights = alpha_weights / alpha_weights.mean()
    alpha_weights = alpha_weights.to(device)

    print(f"[{ts()}] Focal loss alpha weights (top 5):")
    top_alpha_indices = torch.argsort(alpha_weights, descending=True)[:5]
    for idx in top_alpha_indices:
        print(f"[{ts()}]   {i2c[idx.item()]:12s}: {alpha_weights[idx]:.3f}")

    cls_loss_fn = FocalLoss(
        gamma=config["focal_gamma"], alpha=alpha_weights,
    ...
    for ep in range(config["epochs"]):
        ...
        if (ep + 1) % 50 == 0:
            ...
            for i in range(len(classes)):
                ...
                marker = " [HARD]" if cn in hard_classes else ""
                print(f"[{ts()}]     {cn:12s}: {acc:5.1f}% ({cor}/{tot_cls}){marker}")
    ...
    for i in range(len(classes)):
        ...
        marker = " [HARD]" if cn in hard_classes else ""
        print(f"[{ts()}]   {cn:12s}: {res[cn]:5.1f}% ({cor}/{tot_cls}){marker}")
...
def main():
    parser = argparse.ArgumentParser(
        description="KSL v31 Experiment 4 - Hard-Example Mining",
    ...
    parser.add_argument("--checkpoint-dir", type=str, default=os.path.join(base_data, "checkpoints/v31_exp4"))
    ...
    print("=" * 70)
    print(f"KSL Training v31 Experiment 4 - Hard-Example Mining")
    print(f"Started: {ts()}")
    ...
    print(f"  Streams: {V31_EXP4_CONFIG['streams']}")

    print(f"\n[{ts()}] v31 exp4 Config:")
    print(json.dumps(V31_EXP4_CONFIG, indent=2, default=str))
    ...
    if args.model_type in ("numbers", "both"):
        categories.append(("Numbers", NUMBER_CLASSES, HARD_NUMBER_CLASSES))
    if args.model_type in ("words", "both"):
        categories.append(("Words", WORD_CLASSES, HARD_WORD_CLASSES))

    for cat_name, classes, hard_classes in categories:
        cat_key = cat_name.lower()
        ckpt_dir = os.path.join(args.checkpoint_dir, cat_key)

        print(f"\n[{ts()}] {cat_name}: Hard classes = {hard_classes}")

        # Train each stream
        stream_results = {}
        for stream_name in V31_EXP4_CONFIG["streams"]:
            result = train_stream(
                stream_name, cat_name, classes, V31_EXP4_CONFIG,
        ...
        fusion_weights, fused_acc = learn_fusion_weights(
            cat_name, classes, V31_EXP4_CONFIG, val_dir, ckpt_dir, device
        )
    ...
    # Summary
    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] SUMMARY v31 Experiment 4 - Hard-Example Mining")
    print(f"[{ts()}] {'=' * 70}")
    ...
    results_path = os.path.join(
        args.results_dir,
        f"v31_exp4_{args.model_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    )
    results_out = {
        "version": "v31_exp4",
        ...
        "timestamp": ts(),
        "changes_from_v28": [
            "1. WEIGHTED SAMPLING: Oversample hard classes (48, 91, 388, 66, 89 for numbers) by 5x.",
            "2. FOCAL LOSS: Replace CE with focal loss (gamma=2.0, per-class alpha, 2x boost for hard classes).",
            "3. SPEED AUGMENTATION: Random temporal stretching [0.7, 1.3] to handle signer speed variation.",
        ],
    }
