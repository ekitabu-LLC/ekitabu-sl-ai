#!/usr/bin/env python3
"""
KSL Training V45 Expr6 - Self-Knowledge Distillation

Based on V43 (exp5 arch + category-conditional R&R). Adds self-knowledge
distillation where deeper ST-GCN blocks supervise shallower ones via soft
targets, avoiding the external-teacher failure seen in V33 (overconfident
teachers gave near-one-hot labels = no dark knowledge).

Architecture (4 ST-GCN blocks):
  Block 0: 3  -> 64  (stride 1)
  Block 1: 64 -> 64  (stride 2)
  Block 2: 64 -> 128 (stride 1)  <-- aux_head_mid (after block 2)
  Block 3: 128-> 128 (stride 2)  <-- main head (after block 3, final)

Self-KD loss:
  - Main head (block 3) provides soft targets to teach block 2's aux head
  - Total = CE(main) + alpha * KD(main->mid) + beta * CE(mid)

Key changes from V43:
  1. KSLGraphNetV45Expr6: adds aux head after block 2 with GAP + Linear
  2. skd_loss: self-KD from deeper to shallower blocks
  3. Training loop handles 2 classification outputs during training
  4. Inference: model returns only main logits (no overhead)

Usage:
    python train_ksl_v45_expr6.py --model-type numbers
    python train_ksl_v45_expr6.py --model-type words
"""
...
# ---------------------------------------------------------------------------
# Config (v45_expr6 = v43 base + self-KD)
# ---------------------------------------------------------------------------
...
V45_EXPR6_CONFIG = {
    ...
    "skd_temperature": 4.0,
    "skd_alpha": 0.3,       # weight for KD loss component
    "skd_aux_ce_weight": 0.1,  # weight for auxiliary CE losses
}
...
# ---------------------------------------------------------------------------
# Model: KSLGraphNetV45Expr6 — Self-Knowledge Distillation
# ---------------------------------------------------------------------------

class KSLGraphNetV45Expr6(nn.Module):
    """ST-GCN with self-KD auxiliary heads, GRL, SupCon.

    Architecture: 4 ST-GCN blocks.
      ch = [3, 64, 64, 128, 128]
      Strides at blocks 1,3.

    Auxiliary head after block 2 (64 channels, before expansion to 128).
    Main head after block 3 (128 channels, final).

    During training: returns (main_logits, mid_logits, signer_logits, embedding)
    During eval: returns (main_logits, signer_logits, embedding) — same as V43
    """
    ...
    def __init__(self, nc, num_signers, aux_dim, nn_=48, ic=3, hd=64, nl=4,
        ...
        # --- Self-KD: auxiliary classification head after block 2 ---
        # Block 2 output has ch[2+1] = ch[3] = 128 channels (after 64->128 expansion)
        # Wait -- let me recheck: ch = [3, 64, 64, 128, 128]
        # Block 0: 3->64, Block 1: 64->64 (stride 2), Block 2: 64->128, Block 3: 128->128 (stride 2)
        # After block 1: 64 channels. That's where we should tap for the "shallow" head.
        mid_ch = ch[2]  # 64 (output of block 1)
        self.aux_head_mid = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),  # (B, mid_ch, T, N) -> (B, mid_ch, 1, 1)
            nn.Flatten(),             # (B, mid_ch)
            nn.Linear(mid_ch, nc),
        )
    ...
    def forward(self, gcn_input, aux_input, grl_lambda=0.0):
        ...
        # Run ST-GCN blocks with intermediate feature capture
        for i, layer in enumerate(self.layers):
            x = layer(x)
            if i == 1:  # After block 1 (64-ch, before 128 expansion)
                feat_mid = x  # (B, 64, T', N)

        # Main path: final GCN features -> attention pooling
        x_node_avg = x.mean(dim=3)  # (B, 128, T'')
        gcn_embedding = self.attn_pool(x_node_avg)  # (B, 256)
        ...
        if self.training:
            # Self-KD: classify from intermediate features
            mid_logits = self.aux_head_mid(feat_mid)  # (B, nc)
            return logits, mid_logits, signer_logits, embedding
...
def skd_loss(out_main, out_mid, labels, config, cls_loss_fn=None):
    """
    Self-Knowledge Distillation loss.

    out_main: (B, C) logits from final (deepest) block — teacher
    out_mid:  (B, C) logits from block 1 — student
    labels:   (B,) ground truth

    Returns: total_loss, ce_main, kd_mid, ce_mid (all scalars)
    """
    temperature = config["skd_temperature"]
    alpha = config["skd_alpha"]
    aux_ce_weight = config["skd_aux_ce_weight"]
    ls = config["label_smoothing"]

    # Main CE loss
    if cls_loss_fn is not None:
        ce_main = cls_loss_fn(out_main, labels)
    else:
        ce_main = F.cross_entropy(out_main, labels, label_smoothing=ls)

    # Self-KD: main (deeper) teaches mid (shallower) via soft targets
    with torch.no_grad():
        soft_main = F.softmax(out_main / temperature, dim=-1)

    kd_mid = F.kl_div(
        F.log_softmax(out_mid / temperature, dim=-1),
        soft_main, reduction='batchmean'
    ) * (temperature ** 2)

    # Auxiliary CE for mid head (helps convergence)
    if cls_loss_fn is not None:
        ce_mid = cls_loss_fn(out_mid, labels)
    else:
        ce_mid = F.cross_entropy(out_mid, labels, label_smoothing=ls)

    total = ce_main + alpha * kd_mid + aux_ce_weight * ce_mid

    return total, ce_main.item(), kd_mid.item(), ce_mid.item()
...
def train_stream(stream_name, name, classes, config, train_dir, val_dir,
    ...
    print(f"[{ts()}] Self-KD: T={config['skd_temperature']}, alpha={config['skd_alpha']}, "
          f"aux_ce={config['skd_aux_ce_weight']}")
    print(f"[{ts()}] {'=' * 70}")
    ...
    for ep in range(config["epochs"]):
        ...
        for streams_batch, aux_data, targets, signer_targets in train_ld:
            ...
            if use_cutmix:
                ...
                logits, mid_logits, signer_logits, embedding = model(
                    gcn_mixed, aux_mixed, grl_lambda=grl_lambda,
                )
                # SKD loss with mixed targets
                skd_total_a, ce_a, kd_a, _ = skd_loss(
                    logits, mid_logits, targets_a, config, cls_loss_fn)
                skd_total_b, ce_b, kd_b, _ = skd_loss(
                    logits, mid_logits, targets_b, config, cls_loss_fn)
                cls_skd_loss = lam * skd_total_a + (1 - lam) * skd_total_b

                sc_loss = supcon_loss_fn(embedding, targets_a)
                signer_loss = F.cross_entropy(signer_logits, signer_targets)
                loss = cls_skd_loss + supcon_weight * sc_loss + grl_lambda * signer_loss
                _, p = logits.max(1)
                tc += (lam * p.eq(targets_a).float() + (1 - lam) * p.eq(targets_b).float()).sum().item()
                skd_kd_sum += lam * kd_a + (1 - lam) * kd_b
                ...
                skd_total_a, _, kd_a, _ = skd_loss(
                    logits, mid_logits, targets, config, cls_loss_fn)
                skd_total_b, _, kd_b, _ = skd_loss(
                    logits, mid_logits, t_perm, config, cls_loss_fn)
                cls_skd_loss = lam * skd_total_a + (1 - lam) * skd_total_b
                ...
                cls_skd_loss, ce_val, kd_val, ce_mid_val = skd_loss(
                    logits, mid_logits, targets, config, cls_loss_fn)
        ...
        # Validation (eval mode: returns main logits only)
        model.eval()
        vc, vt = 0, 0
        with torch.no_grad():
        ...
        avg_kd = skd_kd_sum / max(skd_count, 1)
        ...
        if ep < 10 or (ep + 1) % 10 == 0 or va > best:
            print(
                ...
                f"SignerAcc: {signer_acc:.1f}% | KD: {avg_kd:.4f} | "
...
def main():
    parser = argparse.ArgumentParser(
        description="KSL Training V45 Expr6 - Self-Knowledge Distillation",
    ...
    parser.add_argument("--checkpoint-dir", type=str, default=os.path.join(base_data, "checkpoints", "v45_expr6"))
    ...
    print("=" * 70)
    print(f"KSL Training V45 Expr6 - Self-Knowledge Distillation")
    print(f"Started: {ts()}")
    print(f"Seed: {args.seed}")
    print(f"Self-KD: T={V45_EXPR6_CONFIG['skd_temperature']}, "
          f"alpha={V45_EXPR6_CONFIG['skd_alpha']}, "
          f"aux_ce={V45_EXPR6_CONFIG['skd_aux_ce_weight']}")
    print("=" * 70)
    ...
    print(f"\nV45 Expr6 Data Split:")
    print(f"  Train dirs: {train_dirs}  (signers 1-12)")
    print(f"  Val dir:    {val_dir}  (signers 13-15)")
    print(f"  Streams: {V45_EXPR6_CONFIG['streams']}")
    print(f"  In channels per stream: {V45_EXPR6_CONFIG['in_channels']}")
    print(f"  Batch size: {V45_EXPR6_CONFIG['batch_size']}")
    print(f"  SupCon weight: {V45_EXPR6_CONFIG['supcon_weight']}")
    print(f"  R&R: applied to NUMBERS only (not words)")

    print(f"\n[{ts()}] V45 Expr6 Config:")
    print(json.dumps(V45_EXPR6_CONFIG, indent=2, default=str))
    ...
    for cat_name, classes, use_focal, apply_rr in categories:
        ...
        stream_results = {}
        for stream_name in V45_EXPR6_CONFIG["streams"]:
            result = train_stream(
                stream_name, cat_name, classes, V45_EXPR6_CONFIG,
        ...
        fusion_weights, fused_acc = learn_fusion_weights(
            cat_name, classes, V45_EXPR6_CONFIG, val_dir, ckpt_dir, device
        )
    ...
    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] SUMMARY V45 Expr6 - Self-Knowledge Distillation")
    print(f"[{ts()}] {'=' * 70}")
    ...
    results_path = os.path.join(
        args.results_dir,
        f"v45_expr6_{args.model_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    )
    results_out = {
        "version": "v45_expr6",
        "model_type": args.model_type,
        "seed": args.seed,
        "config": V45_EXPR6_CONFIG,
        "results": results,
        ...
        "timestamp": ts(),
        "changes_from_v43": [
            "1. Self-Knowledge Distillation: deeper blocks teach shallower via soft targets",
            "2. Auxiliary classification head after ST-GCN block 1 (64-ch)",
            "3. SKD loss: CE(main) + alpha*KD(main->mid) + beta*CE(mid)",
            "4. T=4.0, alpha=0.3, aux_ce=0.1",
            "5. At inference: only main head used (no overhead)",
        ],
    }
