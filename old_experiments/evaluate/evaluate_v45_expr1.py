#!/usr/bin/env python3
"""
V45 Expr1 Real-Tester Evaluation — Sequence-Level Signer-Adversarial GRL + GroupNorm.

GroupNorm models do NOT need AdaBN (no signer-specific batch statistics).
Evaluate directly on real testers.

Usage:
    python evaluate_v45_expr1.py --category both
    python evaluate_v45_expr1.py --category numbers
    python evaluate_v45_expr1.py --category words
"""
...
from train_ksl_v45_expr1 import KSLGraphNetV45Expr1, build_adj
...
def load_v45_expr1_models(ckpt_dir, classes, device):
    """Load V45 Expr1 models (GroupNorm, no AdaBN needed)."""
    ...
    for sname in ["joint", "bone", "velocity"]:
        ...
        grl_tag = "GRL" if use_grl else "no GRL"
        print(f"  Loaded {sname}: val_acc={ckpt.get('val_acc', 0):.1f}%, "
              f"epoch={ckpt.get('epoch', '?')}, params={param_count:,}, {grl_tag}")
...
def main():
    parser = argparse.ArgumentParser(
        description="V45 Expr1 Real-Tester Evaluation (Sequence-Level GRL)")
    parser.add_argument("--category", type=str, default="both",
                        choices=["numbers", "words", "both"])
    parser.add_argument("--checkpoint-base", type=str,
                        default="/scratch/alpine/hama5612/ksl-dir-2/data/checkpoints/v45_expr1")
    ...
    print("=" * 70)
    print("V45 Expr1 Real-Tester Evaluation")
    print("Sequence-Level Signer-Adversarial GRL + GroupNorm")
    print("GRL on: joint, bone | GRL off: velocity")
    ...
    for cat_name, cat_videos, classes in categories:
        ...
        print(f"\n  Loading V45 Expr1 models from {ckpt_dir}...")
        models, fusion_weights = load_v45_expr1_models(ckpt_dir, classes, device)
        ...
        result = evaluate_method(f"v45_expr1_{cat_name}", cat_videos, predict_fn, classes)
        all_results[cat_name] = result

    # --- Summary ---
    print(f"\n{'='*70}")
    print("FINAL SUMMARY - V45 Expr1 Sequence-Level Signer-Adversarial GRL")
    print(f"{'='*70}")
    print(f"\n  Reference baselines:")
    print(f"    exp1 GroupNorm:              Numbers=61.0%, Words=61.7%, Combined=61.4%")
    print(f"    v44_expr2 frame-level GRL:   Numbers=67.8%, Words=53.1%, Combined=59.3%")
    ...
    os.makedirs(os.path.join(results_dir, "v45_expr1"), exist_ok=True)
    out_path = os.path.join(results_dir, "v45_expr1", "real_tester_results.json")
