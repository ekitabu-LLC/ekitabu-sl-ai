#!/usr/bin/env python3
"""
V41 Real-Tester Evaluation — Recover-and-Resample Temporal Augmentation.

V41 uses GroupNorm throughout — no AdaBN needed at inference.
Builds on v37 (GroupNorm + SupCon), replacing speed_aug with R&R temporal aug.

Usage:
    python evaluate_v41.py --category both
    python evaluate_v41.py --category numbers
    python evaluate_v41.py --category words
"""
...
def main():
    parser = argparse.ArgumentParser(description="V41 Real-Tester Evaluation (R&R Temporal Aug)")
    parser.add_argument("--category", type=str, default="both",
                        choices=["numbers", "words", "both"])
    parser.add_argument("--checkpoint-base", type=str,
                        default="/scratch/alpine/hama5612/ksl-dir-2/data/checkpoints/v41")
    ...
    print("=" * 70)
    print("V41 Real-Tester Evaluation")
    print("R&R Temporal Augmentation + GroupNorm + SupCon (no AdaBN needed)")
    ...
    for cat_name, cat_videos, classes in categories:
        ...
        print(f"\n  Loading V41 R&R models from {ckpt_dir}...")
        models, fusion_weights = load_v41_models(ckpt_dir, classes, device)
        ...
        result = evaluate_method(f"v41_{cat_name}", cat_videos, predict_fn, classes)
        all_results[cat_name] = result

    # --- Summary ---
    print(f"\n{'='*70}")
    print("FINAL SUMMARY - V41 R&R Temporal Augmentation")
    print(f"{'='*70}")
    print(f"\n  Reference baselines:")
    ...
    os.makedirs(os.path.join(results_dir, "v41"), exist_ok=True)
    out_path = os.path.join(results_dir, "v41", "real_tester_results.json")
