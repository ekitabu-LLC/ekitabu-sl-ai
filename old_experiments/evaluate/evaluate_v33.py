#!/usr/bin/env python3
"""
V33 Evaluation - KD Student (GroupNorm, Signer-Agnostic)
...
Usage:
    ...
    python evaluate_v33.py --category numbers
"""
...
from train_ksl_v33 import KSLGraphNetV33


def load_v33_models(ckpt_dir, classes, device):
    """Load v33 GroupNorm KD models — no BN adaptation needed."""
    ...
    for sname in ["joint", "bone", "velocity"]:
        ...
        print(f"  Loaded v33/{sname}: val_acc={ckpt.get('val_acc', 'N/A'):.1f}%, "
...
def main():
    parser = argparse.ArgumentParser(description="V33 KD Evaluation (Signer-Agnostic)")
    parser.add_argument("--checkpoint-dir", type=str,
                        default="/scratch/alpine/hama5612/ksl-dir-2/data/checkpoints/v33")
    ...
    print("=" * 70)
    print(f"V33 Evaluation: KD Student (Signer-Agnostic)")
    print(f"Teacher: v27+v28+v29+exp1+exp5 ensemble (70.7%)")
    ...
    for cat_name, cat_videos, classes in categories:
        ...
        print(f"\n{'='*70}")
        print(f"{cat_name.upper()} — V33 KD Student ({len(cat_videos)} videos)")
        print(f"{'='*70}")

        print(f"\n  Loading models from {cat_ckpt_dir}...")
        models, fusion_weights = load_v33_models(cat_ckpt_dir, classes, device)
        ...
        result = evaluate_method(f"v33_{cat_name}", cat_videos, predict, classes)
        all_results[cat_name] = result

    # Summary
    print(f"\n{'='*70}")
    print(f"SUMMARY — V33 KD Student (NO BN adaptation)")
    print(f"{'='*70}")
    ...
    if "numbers" in all_results and "words" in all_results:
        ...
        print(f"\n  Reference: v31 exp1 (GroupNorm, no KD) = 61.4%")
        print(f"  Reference: v31 ensemble (5 models + AdaBN) = 70.7%")
        print(f"  Target: > 61.4% (beating baseline GroupNorm)")

    # Save
    out_dir = "/scratch/alpine/hama5612/ksl-dir-2/data/results"
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, f"v33_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
    with open(out_path, "w") as f:
        json.dump({
            "version": "v33_kd",
            "description": "KD Student: GroupNorm (signer-agnostic, no BN adaptation)",
            "teacher": "v27+v28+v29+exp1+exp5 (70.7%)",
