#!/usr/bin/env python3
"""
Evaluate v25 model on real-world test signers.

V25 changes from v22:
...
- MediaPipe version check (warn if != 0.10.14)
- Temporal CutMix used during training (no effect on eval)

Usage:
    python evaluate_real_testers_v25.py
"""
...
def check_mediapipe_version():
    """Warn if MediaPipe version != 0.10.14 (version used for training data)."""
    version = mp.__version__
    if version != "0.10.14":
        print(f"  WARNING: MediaPipe version is {version}, "
              f"training data was processed with 0.10.14. "
              f"Landmark coordinates may differ between versions.")
    else:
        print(f"  MediaPipe version: {version} (matches training data)")


# ---------------------------------------------------------------------------
# Constants (matching train_ksl_v25.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Inference (v25: same as v22, no ArcFace, no labels in forward)
# ---------------------------------------------------------------------------
...
def main():
    ...
    print("=" * 70)
    print("V25 Real-World Evaluation (with TTA + Confidence Analysis)")
    ...
    # Import v25 model class (same architecture as v22, different aux_dim)
    from train_ksl_v25 import KSLGraphNetV25, build_adj

    results = {}

    # ===== NUMBERS =====
    numbers_ckpt = os.path.join(ckpt_dir, "v25_numbers", "best_model.pt")
    ...
    # ===== WORDS =====
    words_ckpt = os.path.join(ckpt_dir, "v25_words", "best_model.pt")
    ...
    # ===== FINAL SUMMARY =====
    print(f"\n{'='*70}")
    print("FINAL SUMMARY - V25 Real-World Evaluation")
    print(f"{'='*70}")
    ...
    results_path = os.path.join(results_dir, f"v25_real_testers_{ts_str}.json")
    ...
    results_out = {
        ...
        "v25_changes": [
            "Hand-to-body spatial features (8 features: hand height/lateral, inter-hand distance, hand-to-face, symmetry)",
            "Features computed BEFORE wrist-centric normalization on raw landmarks",
            "Temporal CutMix augmentation during training (alpha=1.0, p=0.3)",
            "MediaPipe version check added to evaluation script",
        ],
