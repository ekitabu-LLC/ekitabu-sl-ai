"""
KSL v8 Model Assessment
Comprehensive assessment with confusion matrix and per-class analysis.

Usage:
    modal run evaluate_v8.py
"""

import modal
import os

app = modal.App("ksl-assess-v8")
...
CONFIG = {"max_frames": 90, "feature_dim": 649, "hidden_dim": 320, "dropout": 0.35}
...
def assess_v8():
    ...
    print(f"\n{'='*70}\nV5 vs V7 vs V8 COMPARISON (key classes)\n{'='*70}")
    ...
    print(f"{'Class':>12} | {'v5':>6} | {'v7':>6} | {'v8':>6} | Change from v7")
    print("-" * 55)
    for c in ["444", "35", "91", "100", "125", "54", "Tortoise", "388", "9", "268"]:
        v8_acc = per_class_correct[c]/per_class_total[c]*100 if per_class_total[c] else 0
        change = v8_acc - v7_ref.get(c, 0)
        arrow = "+" if change > 0 else ""
        print(f"{c:>12} | {v5_ref.get(c, 0):5.1f}% | {v7_ref.get(c, 0):5.1f}% | {v8_acc:5.1f}% | {arrow}{change:.1f}%")
