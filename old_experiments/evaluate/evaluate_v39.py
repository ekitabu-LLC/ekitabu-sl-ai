#!/usr/bin/env python3
"""
V39 Real-Tester Evaluation.

Same architecture as v31_exp5 (KSLGraphNetV25 with BatchNorm).
Uses AdaBN Global for domain adaptation at inference (same as exp5).

Usage:
    python evaluate_v39.py --category both
    python evaluate_v39.py --category numbers
    python evaluate_v39.py --category words
"""
...
# Import from v39 training script (same architecture as exp5)
from train_ksl_v39 import KSLGraphNetV25


def load_v39_models(ckpt_dir, classes, device, train_dirs=None):
    """Load v39 models with AdaBN Global adaptation."""
...
def adapt_bn_global(models, test_videos, device):
    """Run AdaBN Global: update BN stats using all test data in a single pass."""
    from evaluate_real_testers_v30 import preprocess_multistream

    print(f"  Running AdaBN Global on {len(test_videos)} test videos...")

    # Set models to train mode to update BN stats
    for sname, model in models.items():
        model.train()

    # Forward pass through all test data
    with torch.no_grad():
        for video_path, class_name, signer_id in test_videos:
            raw = np.load(video_path) if video_path.endswith('.npy') else None
            if raw is None:
                continue
            ...
            for sname, model in models.items():
                ...
                _ = model(gcn, aux.unsqueeze(0).to(device), grl_lambda=0.0)

    # Set back to eval mode
    for sname, model in models.items():
        model.eval()

    print(f"  AdaBN Global complete.")
...
def main():
    parser = argparse.ArgumentParser(description="V39 Real-Tester Evaluation")
    parser.add_argument("--category", type=str, default="both",
                        choices=["numbers", "words", "both"])
    parser.add_argument("--checkpoint-base", type=str,
                        default="/scratch/alpine/hama5612/ksl-dir-2/data/checkpoints/v39")
    parser.add_argument("--no-adabn", action="store_true",
                        help="Skip AdaBN Global (evaluate with training BN stats)")
    ...
    print("=" * 70)
    print("V39 Real-Tester Evaluation")
    print("SupCon on Cleaned Data (AdaBN Global at inference)")
    print(f"Checkpoint base: {args.checkpoint_base}")
    print(f"Category:        {args.category}")
    print(f"AdaBN Global:    {'disabled' if args.no_adabn else 'enabled'}")
    ...
    for cat_name, cat_videos, classes in categories:
        ...
        models, fusion_weights = load_v39_models(ckpt_dir, classes, device)
        ...
        # AdaBN Global adaptation
        if not args.no_adabn:
            adapt_bn_global(models, cat_videos, device)

        predict_fn = make_predict_fn(models, fusion_weights, classes, device)
        result = evaluate_method(f"v39_{cat_name}", cat_videos, predict_fn, classes)
        all_results[cat_name] = result

    # --- Summary ---
    print(f"\n{'='*70}")
    print("FINAL SUMMARY - V39")
    print(f"{'='*70}")
    print(f"\n  Reference baselines:")
    print(f"    exp5 SupCon (original):     Numbers=61.0%, Words=65.4%")
    ...
    adabn_tag = "noadabn" if args.no_adabn else "adabn"
    out_path = os.path.join(results_dir, f"v39_eval_{adabn_tag}_{ts_str}.json")
    save_data = {
        "version": "v39",
        "checkpoint_base": args.checkpoint_base,
        "adabn_global": not args.no_adabn,
        "timestamp": ts_str,
        "results": {
            cat: {
                "accuracy": r["overall"],
