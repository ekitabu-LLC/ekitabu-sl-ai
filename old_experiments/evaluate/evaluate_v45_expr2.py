#!/usr/bin/env python3
"""
V45_expr2 Real-Tester Evaluation — Hard Negative SupCon.

Same architecture as v43/exp5 (BatchNorm, no projection head).
Hard-negative SupCon during training; inference is identical to v43.

Usage:
    python evaluate_v45_expr2.py --category both
    python evaluate_v45_expr2.py --category numbers
    python evaluate_v45_expr2.py --category words
"""
...
from train_ksl_v45_expr2 import KSLGraphNetV45Expr2, build_adj
...
def load_v45_expr2_models(ckpt_dir, classes, device):
    """Load V45_expr2 models (same arch as v43/exp5)."""
...
def main():
    parser = argparse.ArgumentParser(
        description="V45_expr2 Real-Tester Evaluation (Hard Negative SupCon)")
    parser.add_argument("--category", type=str, default="both",
                        choices=["numbers", "words", "both"])
    parser.add_argument("--checkpoint-base", type=str,
                        default="/scratch/alpine/hama5612/ksl-dir-2/data/checkpoints/v45_expr2")
    ...
    print("=" * 70)
    print("V45_expr2 Real-Tester Evaluation")
    print("Hard Negative SupCon + BatchNorm + Category-Conditional R&R")
    ...
    for cat_name, cat_videos, classes in categories:
        ...
        print(f"\n  Loading V45_expr2 models from {ckpt_dir}...")
        models, fusion_weights = load_v45_expr2_models(ckpt_dir, classes, device)
        ...
        result = evaluate_method(f"v45_expr2_{cat_name}", cat_videos, predict_fn, classes)
        all_results[cat_name] = result

    # Summary
    print(f"\n{'='*70}")
    print("FINAL SUMMARY - V45_expr2 Hard Negative SupCon")
    print(f"{'='*70}")
    print(f"\n  Reference baselines:")
    ...
    os.makedirs(os.path.join(results_dir, "v45_expr2"), exist_ok=True)
    out_path = os.path.join(results_dir, "v45_expr2", "real_tester_results.json")
