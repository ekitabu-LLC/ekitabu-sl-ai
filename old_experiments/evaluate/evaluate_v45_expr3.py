#!/usr/bin/env python3
"""
V45 Expr3 Real-Tester Evaluation -- Dilated Multi-Scale TCN + SupCon (exp5 arch).

V45 Expr3 uses BatchNorm in spatial GCN (same as exp5/v43) but replaces the
temporal conv with DilatedMultiScaleTCN (4 branches: dilation 1,2,4 + 1x1 skip,
GroupNorm inside branches). Category-conditional R&R for numbers only.

Usage:
    python evaluate_v45_expr3.py --category both
    python evaluate_v45_expr3.py --category numbers
    python evaluate_v45_expr3.py --category words
"""
...
from train_ksl_v45_expr3 import KSLGraphNetV45Expr3, build_adj
...
def load_v45_expr3_models(ckpt_dir, classes, device):
    """Load V45 Expr3 models (Dilated MS-TCN, exp5 arch: BatchNorm spatial, no projection head)."""
...
def main():
    parser = argparse.ArgumentParser(
        description="V45 Expr3 Real-Tester Evaluation (Dilated Multi-Scale TCN)")
    parser.add_argument("--category", type=str, default="both",
                        choices=["numbers", "words", "both"])
    parser.add_argument("--checkpoint-base", type=str,
                        default="/scratch/alpine/hama5612/ksl-dir-2/data/checkpoints/v45_expr3")
    ...
    print("=" * 70)
    print("V45 Expr3 Real-Tester Evaluation")
    print("Dilated Multi-Scale TCN + BatchNorm spatial + SupCon (exp5 arch)")
    print("  TCN branches: dilation 1,2,4 + 1x1 skip (GroupNorm inside)")
    print("  Numbers: R&R applied during training")
    print("  Words:   No R&R")
    ...
    for cat_name, cat_videos, classes in categories:
        ...
        print(f"\n  Loading V45 Expr3 models from {ckpt_dir}...")
        models, fusion_weights = load_v45_expr3_models(ckpt_dir, classes, device)
        ...
        result = evaluate_method(f"v45_expr3_{cat_name}", cat_videos, predict_fn, classes)
        all_results[cat_name] = result

    print(f"\n{'='*70}")
    print("FINAL SUMMARY - V45 Expr3 Dilated Multi-Scale TCN")
    print(f"{'='*70}")
    print(f"\n  Reference baselines:")
    print(f"    v43 (exp5+R&R):             Numbers=66.1%, Words=65.4%, Combined=65.7%")
    ...
    os.makedirs(os.path.join(results_dir, "v45_expr3"), exist_ok=True)
    out_path = os.path.join(results_dir, "v45_expr3", "real_tester_results.json")
