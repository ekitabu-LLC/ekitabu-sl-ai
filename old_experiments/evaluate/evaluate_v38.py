#!/usr/bin/env python3
"""
V38 Real-Tester Evaluation.

DSTA-SLR backbone + GroupNorm throughout — no AdaBN needed at inference.
Forward: (logits, signer_logits, embedding, proj_features)

Usage:
    python evaluate_v38.py --category both
    python evaluate_v38.py --category numbers
    python evaluate_v38.py --category words
"""
...
from train_ksl_v38 import KSLGraphNetV38, build_graph_distance


def load_v38_models(ckpt_dir, classes, device):
    """Load v38 DSTA-SLR GroupNorm multi-stream models."""
    ...
    for sname in ["joint", "bone", "velocity"]:
        ...
        # Build graph distance for this model
        graph_dist = torch.FloatTensor(build_graph_distance(num_nodes)).to(device)
        ...
        model = KSLGraphNetV38(
            ...
            num_subsets=config.get("num_subsets", 8),
            num_edge_features=config.get("num_edge_features", 6),
            temporal_kernels=tuple(config.get("temporal_kernels", [5, 7])),
...
def main():
    parser = argparse.ArgumentParser(description="V38 Real-Tester Evaluation (DSTA-SLR)")
    parser.add_argument("--category", type=str, default="both",
                        choices=["numbers", "words", "both"])
    parser.add_argument("--checkpoint-base", type=str,
                        default="/scratch/alpine/hama5612/ksl-dir-2/data/checkpoints/v38")
    ...
    print("=" * 70)
    print("V38 Real-Tester Evaluation")
    print("DSTA-SLR backbone + GroupNorm + SupCon (no AdaBN needed)")
    ...
    for cat_name, cat_videos, classes in categories:
        ...
        print(f"\n  Loading v38 DSTA-SLR models from {ckpt_dir}...")
        models, fusion_weights = load_v38_models(ckpt_dir, classes, device)
        ...
        result = evaluate_method(f"v38_{cat_name}", cat_videos, predict_fn, classes)
        all_results[cat_name] = result

    # --- Summary ---
    print(f"\n{'='*70}")
    print("FINAL SUMMARY - V38 (DSTA-SLR)")
    print(f"{'='*70}")
    print(f"\n  Reference baselines:")
    ...
    out_path = os.path.join(results_dir, f"v38_eval_{ts_str}.json")
    save_data = {
        "version": "v38",
        "backbone": "dsta-slr",
