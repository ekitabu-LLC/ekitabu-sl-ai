#!/usr/bin/env python3
"""
V45_expr4 Real-Tester Evaluation — LLM Semantic Supervision.

Same architecture as V43 (BatchNorm, no projection head, SupCon w=0.05).
Semantic projection head is NOT needed at inference — only the classifier head.

Expected:
  Numbers: ~67.8% (R&R + semantic anchor)
  Words:   ~65.4% (semantic anchor, no R&R)

Usage:
    python evaluate_v45_expr4.py --category both
    python evaluate_v45_expr4.py --category numbers
    python evaluate_v45_expr4.py --category words
"""
...
from train_ksl_v45_expr4 import KSLGraphNetV45Expr4, build_adj
...
def load_v45_expr4_models(ckpt_dir, classes, device):
    """Load V45_expr4 models (same arch as V43, semantic head not needed at inference)."""
...
def main():
    parser = argparse.ArgumentParser(
        description="V45_expr4 Real-Tester Evaluation (LLM Semantic Supervision)")
    parser.add_argument("--category", type=str, default="both",
                        choices=["numbers", "words", "both"])
    parser.add_argument("--checkpoint-base", type=str,
                        default="/scratch/alpine/hama5612/ksl-dir-2/data/checkpoints/v45_expr4")
    ...
    print("=" * 70)
    print("V45_expr4 Real-Tester Evaluation")
    print("LLM Semantic Supervision + BatchNorm + SupCon (exp5 arch)")
    print("  Numbers: R&R + semantic supervision (target ~67.8%)")
    print("  Words:   Semantic supervision only (target ~65.4%)")
    ...
    for cat_name, cat_videos, classes in categories:
        ...
        print(f"  R&R applied during training: {'YES' if cat_name == 'numbers' else 'NO'}")
        print(f"  Semantic supervision: YES (CLIP text embeddings)")
        print(f"{'='*70}")

        print(f"\n  Loading V45_expr4 models from {ckpt_dir}...")
        models, fusion_weights = load_v45_expr4_models(ckpt_dir, classes, device)
        ...
        # AdaBN
        print(f"\n  Applying AdaBN to {len(cat_videos)} test videos...")
        adabn_data = preextract_test_data(cat_videos)
        for sname, model in models.items():
            adapt_bn_stats(model, adabn_data, device, stream_name=sname)
        print(f"  AdaBN applied using {len(adabn_data)} samples")

        predict_fn = make_predict_fn(models, fusion_weights, classes, device)
        result = evaluate_method(f"v45_expr4_{cat_name}", cat_videos, predict_fn, classes)
        all_results[cat_name] = result

    # Summary
    print(f"\n{'='*70}")
    print("FINAL SUMMARY - V45_expr4 LLM Semantic Supervision")
    print(f"{'='*70}")
    print(f"\n  Reference baselines:")
    print(f"    v43 (best single combined):  Numbers=66.1%, Words=65.4%, Combined=65.7%")
    print(f"    exp5 SupCon:                 Numbers=61.0%, Words=65.4%")
    print(f"    v41 R&R (uniform):           Numbers=67.8%, Words=55.6%")
    ...
    os.makedirs(os.path.join(results_dir, "v45_expr4"), exist_ok=True)
    out_path = os.path.join(results_dir, "v45_expr4", "real_tester_results.json")
