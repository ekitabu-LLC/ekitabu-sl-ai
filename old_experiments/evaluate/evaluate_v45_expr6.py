#!/usr/bin/env python3
"""
V45 Expr6 Real-Tester Evaluation — Self-Knowledge Distillation.

Uses KSLGraphNetV45Expr6 (same as V43 at inference — aux heads inactive in eval mode).
BatchNorm architecture, AdaBN applied at test time.

Usage:
    python evaluate_v45_expr6.py --category both
    python evaluate_v45_expr6.py --category numbers
    python evaluate_v45_expr6.py --category words
"""
...
from train_ksl_v45_expr6 import KSLGraphNetV45Expr6, build_adj
...
def load_v45_expr6_models(ckpt_dir, classes, device):
    """Load V45 Expr6 self-KD models (BatchNorm arch, aux heads ignored at eval)."""
    ...
    def predict(raw, true_class):
        ...
        with torch.no_grad():
            for sname, model in models.items():
                ...
                # In eval mode, model returns (logits, signer_logits, embedding)
...
def main():
    parser = argparse.ArgumentParser(
        description="V45 Expr6 Real-Tester Evaluation (Self-Knowledge Distillation)")
    parser.add_argument("--category", type=str, default="both",
                        choices=["numbers", "words", "both"])
    parser.add_argument("--checkpoint-base", type=str,
                        default="/scratch/alpine/hama5612/ksl-dir-2/data/checkpoints/v45_expr6")
    ...
    print("=" * 70)
    print("V45 Expr6 Real-Tester Evaluation")
    print("Self-Knowledge Distillation + BatchNorm + SupCon + R&R(numbers)")
    ...
    for cat_name, cat_videos, classes in categories:
        ...
        print(f"\n  Loading V45 Expr6 models from {ckpt_dir}...")
        models, fusion_weights = load_v45_expr6_models(ckpt_dir, classes, device)
        ...
        # AdaBN: adapt BatchNorm stats to test distribution
        ...
        result = evaluate_method(f"v45_expr6_{cat_name}", cat_videos, predict_fn, classes)
        all_results[cat_name] = result

    # Summary
    print(f"\n{'='*70}")
    print("FINAL SUMMARY - V45 Expr6 Self-Knowledge Distillation")
    print(f"{'='*70}")
    print(f"\n  Reference baselines:")
    print(f"    v43 (best single):          Numbers=66.1%, Words=65.4%, Combined=65.7%")
    ...
    os.makedirs(os.path.join(results_dir, "v45_expr6"), exist_ok=True)
    out_path = os.path.join(results_dir, "v45_expr6", "real_tester_results.json")
