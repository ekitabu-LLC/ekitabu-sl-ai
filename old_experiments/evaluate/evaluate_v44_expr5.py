#!/usr/bin/env python3
"""
V44 Expr5 Real-Tester Evaluation — HA-GCN Hand-Body Sub-Graph Architecture.

V44 Expr5 uses GroupNorm throughout — no AdaBN needed.

Usage:
    python evaluate_v44_expr5.py --category both
    python evaluate_v44_expr5.py --category numbers
    python evaluate_v44_expr5.py --category words
"""
...
from train_ksl_v44_expr5 import KSLGraphNetHA, build_hand_adj, build_body_adj
...
def load_ha_models(ckpt_dir, classes, device):
    """Load HA-GCN models (GroupNorm, hand-body sub-graphs)."""
...
def main():
    parser = argparse.ArgumentParser(
        description="V44 Expr5 Real-Tester Evaluation (HA-GCN Hand-Body Sub-Graphs)")
    parser.add_argument("--category", type=str, default="both",
                        choices=["numbers", "words", "both"])
    parser.add_argument("--checkpoint-base", type=str,
                        default="/scratch/alpine/hama5612/ksl-dir-2/data/checkpoints/v44_expr5")
    ...
    print("=" * 70)
    print("V44 Expr5 Real-Tester Evaluation")
    print("HA-GCN: Hand-Body Sub-Graph Architecture + GroupNorm + SupCon")
    ...
    for cat_name, cat_videos, classes in categories:
        ...
        print(f"\n  Loading HA-GCN models from {ckpt_dir}...")
        models, fusion_weights = load_ha_models(ckpt_dir, classes, device)
        ...
        # GroupNorm models: no AdaBN needed
        print(f"\n  GroupNorm model — no AdaBN adaptation needed")

        predict_fn = make_predict_fn(models, fusion_weights, classes, device)
        result = evaluate_method(f"v44_expr5_{cat_name}", cat_videos, predict_fn, classes)
        all_results[cat_name] = result

    # --- Summary ---
    print(f"\n{'='*70}")
    print("FINAL SUMMARY - V44 Expr5 HA-GCN (Hand-Body Sub-Graphs)")
    print(f"{'='*70}")
    print(f"\n  Reference baselines:")
    ...
    os.makedirs(os.path.join(results_dir, "v44_expr5"), exist_ok=True)
    out_path = os.path.join(results_dir, "v44_expr5", "real_tester_results.json")
