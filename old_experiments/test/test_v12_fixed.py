"""
Test KSL v12 using pre-extracted features (same extraction as training).
This eliminates train/test extraction mismatch.

Usage:
    1. First run: modal run extract_test_features.py
    2. Then run: modal run test_v12_fixed.py
"""

import modal
import os

app = modal.App("ksl-test-v12-fixed")
...
def test_with_extracted_features():
    import torch
    import torch.nn as nn
    import numpy as np

    print("=" * 70)
    print("KSL v12 Testing (Pre-extracted Features)")
    print("=" * 70)
    ...
    def preprocess(data, mf=90):
        ...
        # Extract pose (upper body) and hands from 549-dim data
        if data.shape[1] >= 225:
    ...
    if os.path.exists(test_dir):
        ...
        print("  No pre-extracted test data found! Run extract_test_features.py first.")
...
def main():
    r = test_with_extracted_features.remote()
