#!/usr/bin/env python3
"""
V44_expr3 Real-Tester Evaluation -- Skeleton-CutMix + SupCon (exp5 arch).

V44_expr3 uses BatchNorm (not GroupNorm) -- same architecture as exp5/V43.
No projection head. Skeleton-CutMix applied during training for cross-signer
hand exchange. R&R applied during training for numbers only.

Usage:
    python evaluate_v44_expr3.py --category both
    python evaluate_v44_expr3.py --category numbers
    python evaluate_v44_expr3.py --category words
"""
...
from train_ksl_v44_expr3 import KSLGraphNetV44Expr3, build_adj
...
def load_v44_expr3_models(ckpt_dir, classes, device):
    """Load V44_expr3 models (exp5 arch: BatchNorm, no projection head)."""
...
def main():
    parser = argparse.ArgumentParser(
        description="V44_expr3 Real-Tester Evaluation (Skeleton-CutMix + SupCon)")
    parser.add_argument("--category", type=str, default="both",
                        choices=["numbers", "words", "both"])
    parser.add_argument("--checkpoint-base", type=str,
                        default="/scratch/alpine/hama5612/ksl-dir-2/data/checkpoints/v44_expr3")
    ...
    print("=" * 70)
    print("V44_expr3 Real-Tester Evaluation")
    print("Skeleton-CutMix + BatchNorm + SupCon (exp5 arch)")
    ...
    for cat_name, cat_videos, classes in categories:
        ...
        print(f"\n  Loading V44_expr3 models from {ckpt_dir}...")
        models, fusion_weights = load_v44_expr3_models(ckpt_dir, classes, device)
        ...
        result = evaluate_method(f"v44_expr3_{cat_name}", cat_videos, predict_fn, classes)
        all_results[cat_name] = result

    # --- Summary ---
    print(f"\n{'='*70}")
    print("FINAL SUMMARY - V44_expr3 Skeleton-CutMix (exp5 arch)")
    print(f"{'='*70}")
    print(f"\n  Reference baselines:")
    print(f"    exp5 SupCon:                Numbers=61.0%, Words=65.4%")
    print(f"    v41 R&R (uniform):          Numbers=67.8%, Words=55.6%")
    print(f"    v43 (cond R&R+exp5 arch):   Numbers=66.1%, Words=65.4%, Combined=65.7%")
    ...
    os.makedirs(os.path.join(results_dir, "v44_expr3"), exist_ok=True)
    out_path = os.path.join(results_dir, "v44_expr3", "real_tester_results.json")
