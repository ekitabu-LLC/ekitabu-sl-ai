#!/usr/bin/env python3
"""
V42 Real-Tester Evaluation — Category-Conditional R&R + SupCon.

V42 uses GroupNorm throughout — no AdaBN needed at inference.
Same architecture as V41 (GroupNorm + SupCon + projection head) but trained
with R&R augmentation only for numbers (not words), giving balanced performance:
  Numbers: target ~67.8% (benefits from R&R, same as v41)
  Words:   target ~65.4% (no R&R, same as exp5)

Usage:
    python evaluate_v42.py --category both
    python evaluate_v42.py --category numbers
    python evaluate_v42.py --category words
"""
...
from train_ksl_v42 import KSLGraphNetV42, build_adj
...
def load_v42_models(ckpt_dir, classes, device):
    """Load V42 category-conditional R&R models (same arch as V41)."""
...
def main():
    parser = argparse.ArgumentParser(
        description="V42 Real-Tester Evaluation (Category-Conditional R&R + SupCon)")
    parser.add_argument("--category", type=str, default="both",
                        choices=["numbers", "words", "both"])
    parser.add_argument("--checkpoint-base", type=str,
                        default="/scratch/alpine/hama5612/ksl-dir-2/data/checkpoints/v42")
    ...
    print("=" * 70)
    print("V42 Real-Tester Evaluation")
    print("Category-Conditional R&R + GroupNorm + SupCon (no AdaBN needed)")
    ...
    for cat_name, cat_videos, classes in categories:
        ...
        print(f"\n  Loading V42 models from {ckpt_dir}...")
        models, fusion_weights = load_v42_models(ckpt_dir, classes, device)
        ...
        result = evaluate_method(f"v42_{cat_name}", cat_videos, predict_fn, classes)
        all_results[cat_name] = result

    # --- Summary ---
    print(f"\n{'='*70}")
    print("FINAL SUMMARY - V42 Category-Conditional R&R")
    print(f"{'='*70}")
    print(f"\n  Reference baselines:")
    ...
    os.makedirs(os.path.join(results_dir, "v42"), exist_ok=True)
    out_path = os.path.join(results_dir, "v42", "real_tester_results.json")
