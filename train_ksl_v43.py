#!/usr/bin/env python3
"""
KSL Training v43 - Category-Conditional Recover-and-Resample

Based on v31_exp5 (GroupNorm=False/BatchNorm, SupCon weight=0.05, KSLGraphNetV43
architecture — no projection head).

Key insight: R&R (Recover-and-Resample) helps numbers (hand-pose/position dominant)
but hurts words (motion/trajectory dominant) because Stage 3 (per-segment speed warp)
disrupts kinematic coherence for motion-based signs.

V43 fix: apply R&R only when training the numbers model; skip for words.

Key changes from v31_exp5:
  1. **R&R augmentation** added (from v41) — applied ONLY when apply_rr=True (numbers)
  2. **category-conditional**: numbers training gets R&R, words training does NOT
  3. Model class renamed KSLGraphNetV43 (identical architecture to KSLGraphNetV43)
  4. Checkpoint dir: v43

Expected: Numbers ~67.8% (same as v41), Words ~65.4% (same as exp5)
           → Combined ~66.5%, better diversity for ensemble

Usage:
    python train_ksl_v43.py --model-type numbers
    python train_ksl_v43.py --model-type words
"""
...
# Import all helpers and classes from v28 (exact copy)
# We'll paste the necessary code here to keep it self-contained
...
# ---------------------------------------------------------------------------
# Config (v43 - exp5 base + R&R for numbers only)
# ---------------------------------------------------------------------------
...
V43_CONFIG = {
    ...
    "cutmix_alpha": 1.0,
    "cutmix_prob": 0.3,
    # NEW: R&R augmentation (applied to numbers only via apply_rr flag)
    "rr_prob": 0.7,             # probability of applying R&R per sample
...
# ---------------------------------------------------------------------------
# Recover-and-Resample Temporal Augmentation (NEW in v43, from v41)
# ---------------------------------------------------------------------------
...
class KSLMultiStreamDataset(Dataset):
    ...
    def __init__(self, data_dirs, classes, config, aug=False, apply_rr=False):
        ...
        self.aug = aug
        self.config = config
        self.apply_rr = apply_rr  # NEW v43: apply R&R only for numbers
    ...
    def __getitem__(self, idx):
        ...
        # NEW v43: Recover-and-Resample (only when apply_rr=True, i.e., numbers)
...
# ---------------------------------------------------------------------------
# Model (KSLGraphNetV43 architecture)
# ---------------------------------------------------------------------------
...
class KSLGraphNetV43(nn.Module):
    """ST-GCN with aux branch, GRL, SupCon."""
...
# ---------------------------------------------------------------------------
# Train One Stream (adapted from v28)
# ---------------------------------------------------------------------------
...
def main():
    parser = argparse.ArgumentParser(
        description="KSL ST-GCN Training v31 Exp5 - Cross-Signer Contrastive",
    ...
    parser.add_argument("--checkpoint-dir", type=str, default=os.path.join(base_data, "checkpoints", "v43"))
    ...
    print("=" * 70)
    print(f"KSL Training v43 - Category-Conditional R&R + SupCon")
    print(f"Started: {ts()}")
    ...
    print(f"  Streams: {V43_CONFIG['streams']}")
    print(f"  In channels per stream: {V43_CONFIG['in_channels']}")
    print(f"  Batch size: {V43_CONFIG['batch_size']}")
    print(f"  SupCon weight: {V43_CONFIG['supcon_weight']}")
    print(f"  R&R: applied to NUMBERS only (not words)")

    print(f"\n[{ts()}] V43 Config:")
    print(json.dumps(V43_CONFIG, indent=2, default=str))
    ...
    for cat_name, classes, use_focal, apply_rr in categories:
        ...
        # Train each stream
        stream_results = {}
        for stream_name in V43_CONFIG["streams"]:
            result = train_stream(
                stream_name, cat_name, classes, V43_CONFIG,
        ...
        fusion_weights, fused_acc = learn_fusion_weights(
            cat_name, classes, V43_CONFIG, val_dir, ckpt_dir, device
        )
    ...
    # Summary
    print(f"\n[{ts()}] {'=' * 70}")
    print(f"[{ts()}] SUMMARY V43 - Category-Conditional R&R + SupCon")
    print(f"[{ts()}] {'=' * 70}")
    ...
    results_path = os.path.join(
        args.results_dir,
        f"v43_{args.model_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    )
    results_out = {
        "version": "v43",
        ...
        "timestamp": ts(),
        "changes_from_exp5": [
            "1. R&R augmentation added (from v41)",
            "2. R&R applied ONLY to numbers training (apply_rr=True for numbers, False for words)",
            "3. Architecture identical to KSLGraphNetV25 (BatchNorm, no projection head)",
            "4. Checkpoint dir: v43",
        ],
    }
