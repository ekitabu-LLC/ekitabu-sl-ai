"""
Test KSL v14 using pre-extracted features.

Usage:
    modal run test_v14.py
"""

import modal
import os

app = modal.App("ksl-test-v14")
...
@app.function(gpu="T4", volumes={"/data": volume}, timeout=3600, image=image)
def test_v14():
    import torch
    ...
    print("=" * 70)
    print("KSL v14 Testing (Extreme Dropout)")
    print("=" * 70)
    ...
    num_ckpt_path = "/data/checkpoints_v14_numbers/best_model.pt"
    word_ckpt_path = "/data/checkpoints_v14_words/best_model.pt"

    if not os.path.exists(num_ckpt_path):
        print("v14 models not found! Run train_ksl_v14.py first.")
    ...
    if os.path.exists(test_dir):
        for class_name in sorted(os.listdir(test_dir)):
            ...
            for fn in sorted(os.listdir(class_dir)):
                ...
                ok = pred == class_name
                print(f"{class_name}: Pred={pred} ({conf*100:.1f}%) {'OK' if ok else 'WRONG'}")
